...
fileprivate let appMaxKey:[UInt8] = [0xa8,0xaf,0xa8,0xb5,0xe9,0xa2,0xae,0xa5,0xa4,0xb3,0xfb,0xe8,0xe1,0xa9,0xa0,0xb2,0xe1,0xaf,0xae,0xb5,0xe1,0xa3,0xa4,0xa4,0xaf,0xe1,0xa8,0xac,0xb1,0xad,0xa4,0xac,0xa4,0xaf,0xb5,0xa4,0xa5]

				private func effectStart(status num: UInt8) -> UInt8 {
					return num ^ 193
				}
...
class MediaThen: ReloadViewController {
    //: var tranConfig: BasePresentTransition!
    var tranConfig: HostessThen!
    //: init() {
    init() {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: self.tranConfig = BasePresentTransition.init(target: self)
        self.tranConfig = HostessThen(target: self)
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: appMaxKey.map{effectStart(status: $0)}, encoding: .utf8)!)
    }
}

//: class BasePresentTransition: NSObject {
class HostessThen: NSObject {
    ...
    var frameOfPresentedView = CGRect(x: 0, y: 0, width: user_barText, height: show_licenseData * 0.6)
    ...
    func upRadius(corners: UIRectCorner, radius: CGFloat) {
...
extension HostessThen: UIViewControllerTransitioningDelegate {
    ...
    func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source _: UIViewController) -> UIPresentationController? {
        ...
        let res = DisappearReactiveCompatible(transition: self, presentedViewController: presented, presentingViewController: presenting)
        //: return res
        return res
    }
}
...
extension HostessThen: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if toVC == targetVC {
            ...
            inst(transitionContext: transitionContext)
            //: } else {
        } else {
            //: dismissTransition(transitionContext: transitionContext)
            targetContext(transitionContext: transitionContext)
        }
    }

    //: private func presentTransition(transitionContext: UIViewControllerContextTransitioning) {
    private func inst(transitionContext: UIViewControllerContextTransitioning) {
        //: let toVC = transitionContext.viewController(forKey: .to)
        let toVC = transitionContext.viewController(forKey: .to)
    ...
    private func targetContext(transitionContext: UIViewControllerContextTransitioning) {
...
class DisappearReactiveCompatible: UIPresentationController, UIGestureRecognizerDelegate {
    //: var transition: BasePresentTransition!
    var transition: HostessThen!

    //: init(transition: BasePresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    init(transition: HostessThen, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    ...
    func addOpenInsideRecognizer() {
        //: let tap = UITapGestureRecognizer.init(target: self, action: #selector(func__ViewTapGestureRecognizer))
        let tap = UITapGestureRecognizer(target: self, action: #selector(magnitudeervalOfTable))
        //: tap.delegate = self
        tap.delegate = self
    ...
    @objc func magnitudeervalOfTable() {
    ...
    lazy var visualView: UIView = {
        ...
        view.frame = containerView?.bounds ?? CGRect(x: 0, y: 0, width: user_barText, height: show_licenseData)
