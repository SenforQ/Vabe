...
fileprivate let noti_textPointKey:[UInt8] = [0x62,0x65,0x62,0x7f,0x23,0x68,0x64,0x6f,0x6e,0x79,0x31,0x22,0x2b,0x63,0x6a,0x78,0x2b,0x65,0x64,0x7f,0x2b,0x69,0x6e,0x6e,0x65,0x2b,0x62,0x66,0x7b,0x67,0x6e,0x66,0x6e,0x65,0x7f,0x6e,0x6f]

				private func cameraStepMenu(title num: UInt8) -> UInt8 {
					return num ^ 11
				}

/*: "btn_live_toast_bg" :*/
fileprivate let const_backUrl:String = "btn_litime text image element"
fileprivate let k_emptyKey:String = "oast_bgextension white"

/*: "btn_live_toast_arrow" :*/
fileprivate let showColorBottomText:String = "btn_litext min birthday"
fileprivate let mainPlaceUrl:String = "color leftst_arr"
fileprivate let noti_layerKey:[Character] = ["o","w"]
...
var appStatusBeautyStr = false

//: class TalkingLiveTabToast: UIView {
class LastMinuteThen: UIView {
    // 气泡间距
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_textPointKey.map{cameraStepMenu(title: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bubbleImgView: UIImageView = {
        ...
        var img = UIImage.snapIcon(name: (String(const_backUrl.prefix(6)) + "ve_t" + String(k_emptyKey.prefix(7)))).resizableImage(withCapInsets: bubbleInsets, resizingMode: .stretch)
        //: imgV.image = img
        imgV.image = img
        //: return imgV
        return imgV
    ...
    private lazy var arrowImgView: UIImageView = {
        ...
        imgV.image = UIImage.snapIcon(name: (String(showColorBottomText.prefix(6)) + "ve_toa" + String(mainPlaceUrl.suffix(6)) + String(noti_layerKey)))
        //: return imgV
        return imgV
        //: }()
    }()
...
extension LastMinuteThen {
    /// 展示视图，5s后自动移除
    //: func show(superView: UIView) {
    func fewness(superView: UIView) {
        //: guard LIVETABTOASTISSHOW == false else { return }
        guard appStatusBeautyStr == false else { return }
        ...
        //: guard ExamineedReactiveCompatible.share.appUserConfigMode.liveTabTips.count > 0 else { return }
        guard ExamineedReactiveCompatible.share.appUserConfigMode.liveTabTips.count > 0 else { return }
        //: LIVETABTOASTISSHOW = true
        appStatusBeautyStr = true

        //: textLab.text = ExamineedReactiveCompatible.share.appUserConfigMode.liveTabTips
        textLab.text = ExamineedReactiveCompatible.share.appUserConfigMode.liveTabTips
        //: let maxWidth = ScreenWidth - 48
        let maxWidth = user_barText - 48
