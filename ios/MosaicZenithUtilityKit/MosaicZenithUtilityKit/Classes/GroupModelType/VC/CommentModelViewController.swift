...
fileprivate let data_giftName:String = "key time time background deadlineicon_k"
fileprivate let kShowTitle:String = "ng_defas text data gift"
...
// __CLOSE_PRINT__
//
//  CommentModelViewController.swift
//  AbroadTalking
//
...
class CommentModelViewController: ReloadViewController {
    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var postDataArr = [TalkingUserInfoMomentItemModel]()
    private var postDataArr = [ViewTransformable]()
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: appToMinChangeUrl, right: 0)
        //: collectionView.register(TalkingPostItemCell.self, forCellWithReuseIdentifier: TalkingPostItemCell.className())
        collectionView.register(NameThen.self, forCellWithReuseIdentifier: NameThen.className())
    ...
    private lazy var emptyView: UIView = {
        ...
        img.image = UIImage.snapIcon(name: (String(data_giftName.suffix(6)) + "ong_ko" + String(kShowTitle.prefix(6)) + "ault"))
...
extension CommentModelViewController {
    ...
    private func atChange() {
        ...
        GroupViewToolReactiveCompatible.fireExtinguisherCompletion(params: params) { [weak self] succeed, result, _ in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            if let resultArr = resultDict["list"] as? [Any] {
                ...
                for dict in resultArr {
                    ...
                    if let model = LanthanonMeasurable.deserialize(from: dict as? Dictionary) {
...
extension CommentModelViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: NameThen.className(), for: indexPath) as! NameThen
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = VisualCommunicationViewController(model: detailModel)
...
extension CommentModelViewController: JXPagingViewListViewDelegate {
...
extension CommentModelViewController {
    ...
    func labelStatus(_ userModel: WithLabTransformable) {
