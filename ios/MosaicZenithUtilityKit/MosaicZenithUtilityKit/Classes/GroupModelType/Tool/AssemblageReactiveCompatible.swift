...
fileprivate let k_gameKey:[UInt8] = [0x3b,0x7,0x6,0x1c,0x4f,0x9,0xa,0xe,0x1b,0x1a,0x1d,0xa,0x4f,0x6,0x1c,0x4f,0x1,0x0,0x1b,0x4f,0xe,0x19,0xe,0x6,0x3,0xe,0xd,0x3,0xa,0x4f,0xb,0x1a,0x1d,0x6,0x1,0x8,0x4f,0xe,0x4f,0xc,0xe,0x3,0x3]

				private func popCorner(player num: UInt8) -> UInt8 {
					return num ^ 111
				}
...
protocol MediaTampingBarToolDelegate: NSObject {
    /// 录音时间
...
enum EasingDefaultsSerializable: Int {
    //: case PlayerFinish = 0
...
class AssemblageReactiveCompatible: NSObject {
    //: open weak var delegate: TalkingAudioToolDelegate?
    open weak var delegate: MediaTampingBarToolDelegate?
    ...
    static let shared = AssemblageReactiveCompatible()
    ...
    override private init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(fail(notification:)), name: AVAudioSession.interruptionNotification, object: nil)
    }

    //: func getVoiceRecordLocalPathWithFileType(path: NSString) -> String {
    func voiceModel(path: NSString) -> String {
...
extension AssemblageReactiveCompatible: AVAudioRecorderDelegate, AVAudioPlayerDelegate {
    /// 录制、播放过程中中断
    //: @objc func interruptionChange(notification: NSNotification) -> Void {
    @objc func fail(notification: NSNotification) {
        ...
        if changeReason?.intValue ?? 0 == AVAudioSession.InterruptionType.began.rawValue {
            ...
            self.heatherMixtureMoment(showMsg: String(bytes: k_gameKey.map{popCorner(player: $0)}, encoding: .utf8)!.localized)
            //: if self.isRecording {
            if self.isRecording {
                //: self.delegate?.func__recordFinishWithFile(filePath: self.filePath)
                self.delegate?.buttonPath(filePath: self.filePath)
...
//                destroyRecordTimer()
                ...
                self.delegate?.addEqual(status: .PlayerInterruption)
            }
        }
    }
    ...
    func audioPlayerDidFinishPlaying(_: AVAudioPlayer, successfully _: Bool) {
        ...
        self.delegate?.addEqual(status: .PlayerFinish)
    }
}
...
extension AssemblageReactiveCompatible {
    ...
    func imageTimer() {
        ...
        recordTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(groupTime(time:)), userInfo: nil, repeats: true)
    ...
    @objc func groupTime(time _: Timer) {
        //: recorder?.updateMeters()
        recorder?.updateMeters()
        ...
        self.delegate?.barConvertImage(currentTime: interval)
    }
    ...
    func equalAccept() {
        ...
        self.delegate?.wood(currentTime: currentTime)
    }
    ...
    func timer() {
        ...
        playTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(tictacTime(time:)), userInfo: nil, repeats: true)
        //: if let curTime: Timer = recordTimer {
        if let curTime: Timer = recordTimer {
            //: RunLoop.main.add(curTime, forMode: .common)
            RunLoop.main.add(curTime, forMode: .common)
    ...
    @objc func tictacTime(time _: Timer) {
        //: if self.player != nil {
        if self.player != nil {
