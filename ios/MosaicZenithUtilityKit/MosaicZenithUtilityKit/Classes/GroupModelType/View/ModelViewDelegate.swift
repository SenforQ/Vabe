...
fileprivate let notiColorMessage:[UInt8] = [0x37,0x30,0x37,0x2a,0x76,0x3d,0x31,0x3a,0x3b,0x2c,0x64,0x77,0x7e,0x36,0x3f,0x2d,0x7e,0x30,0x31,0x2a,0x7e,0x3c,0x3b,0x3b,0x30,0x7e,0x37,0x33,0x2e,0x32,0x3b,0x33,0x3b,0x30,0x2a,0x3b,0x3a]

				private func shareFilter(share num: UInt8) -> UInt8 {
					return num ^ 94
				}

/*: "Edit Title" :*/
fileprivate let user_femaleLicenseControlData:String = "Edit self implement gift type to"

/*: "Finish" :*/
fileprivate let noti_managerKey:[Character] = ["F"]
fileprivate let show_contentTitle:String = "iniview"

/*: "btn_delete" :*/
fileprivate let show_renderMsg:String = "image action size view addbtn_d"
fileprivate let kLabelName:[Character] = ["e","l","e","t","e"]
...
class ModelViewDelegate: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(digitiser))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiColorMessage.map{shareFilter(share: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleLabel: UILabel = {
        ...
        lab.font = UIFont.font(type: .Semibold, fontSize: 20)
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
        //: lab.text = "Edit Title".localized
        lab.text = (String(user_femaleLicenseControlData.prefix(5)) + "Title").localized
    ...
    lazy var inputTView: SongTitleView = {
    ...
    private lazy var finishBtn: UIButton = {
        ...
        btn.setTitle((String(noti_managerKey) + show_contentTitle.replacingOccurrences(of: "view", with: "sh")).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.resourceIn(colors: UIColor.appSub(), size: CGSize(width: 267, height: 50)), for: .normal)
        //: btn.layer.cornerRadius = 25
        btn.layer.cornerRadius = 25
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(finishBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(makeShow), for: .touchUpInside)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.snapIcon(name: (String(show_renderMsg.suffix(5)) + String(kLabelName))), for: .normal)
...
extension ModelViewDelegate: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > lengthLimit {
            ...
            if textNum > lengthLimit && lengthLimit > 0 {
                ...
                textView.text = nextTitle(index: lengthLimit, text: textContent)
            }
        }
    ...
    private func nextTitle(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
