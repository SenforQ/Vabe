...
fileprivate let kModelShouldValue:[UInt8] = [0x57,0x50,0x57,0x4a,0x16,0x5d,0x51,0x5a,0x5b,0x4c,0x4,0x17,0x1e,0x56,0x5f,0x4d,0x1e,0x50,0x51,0x4a,0x1e,0x5c,0x5b,0x5b,0x50,0x1e,0x57,0x53,0x4e,0x52,0x5b,0x53,0x5b,0x50,0x4a,0x5b,0x5a]

				private func shareLabel(share num: UInt8) -> UInt8 {
					return num ^ 62
				}

/*: "#333333" :*/
fileprivate let const_deviceId:String = "#333333"

/*: "#999999" :*/
fileprivate let noti_numberMsg:String = "#999999"

/*: "isCupid" :*/
fileprivate let userDropData:[Character] = ["i"]
fileprivate let data_midMessage:String = "content user self varsCupid"

/*: "autoGreetAuth" :*/
fileprivate let user_viewName:String = "autoGrefirst hidden"
fileprivate let kColorSizeMessage:[Character] = ["e"]
fileprivate let kLabelUrl:[Character] = ["t","A","u","t","h"]

/*: "isLeaderboardAnonymous" :*/
fileprivate let userViewFormat:[Character] = ["i","s","L","e","a","d","e","r","b","o","a","r","d","A","n","o","n","y"]
fileprivate let data_curveText:String = "moufilter"

/*: "isSameGendered" :*/
fileprivate let const_topBottomText:String = "isSameGview size"
fileprivate let app_observerData:String = "eninputereinput"
...
class IconViewCell: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(progressMake), name: UIApplication.didBecomeActiveNotification, object: nil)
    }

    //: required init?(coder aDecoder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: kModelShouldValue.map{shareLabel(share: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleBLB: UILabel = {
        ...
        lb.textColor = UIColor(hex: (const_deviceId.capitalized))
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var switchView: UISwitch = {
        ...
        swit.addTarget(self, action: #selector(userChange), for: .valueChanged)
        //: return swit
        return swit
        //: }()
    }()
...
extension IconViewCell {
    ...
    func location(titile: String, row: Int, isLast: Bool) {
...
//        if titile == SettingsSizeBreedText.General.rawValue {
//            SwitchicialPermissionTool.checkPushNotification { isShow in
...
//        }
    ...
    func queryedName(detail: String) {
    ...
    @objc func userChange() {
        ...
        if titleBLB.text == SkipColumnConvertible.Turnon_Notif.rawValue.localized {
        ...
        } else if titleBLB.text == SkipColumnConvertible.Cupid.rawValue.localized {
            //: setConfig(type: "isCupid", value: self.switchView.isOn)
            config(type: (String(userDropData) + String(data_midMessage.suffix(6))), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Automatic.rawValue.localized {
        } else if titleBLB.text == SkipColumnConvertible.Automatic.rawValue.localized {
            //: setConfig(type: "autoGreetAuth", value: self.switchView.isOn)
            config(type: (String(user_viewName.prefix(7)) + String(kColorSizeMessage) + String(kLabelUrl)), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Ranking.rawValue.localized {
        } else if titleBLB.text == SkipColumnConvertible.Ranking.rawValue.localized {
            //: setConfig(type: "isLeaderboardAnonymous", value: self.switchView.isOn)
            config(type: (String(userViewFormat) + data_curveText.replacingOccurrences(of: "filter", with: "s")), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Same_gendered.rawValue.localized {
        } else if titleBLB.text == SkipColumnConvertible.Same_gendered.rawValue.localized {
            //: setConfig(type: "isSameGendered", value: self.switchView.isOn)
            config(type: (String(const_topBottomText.prefix(7)) + app_observerData.replacingOccurrences(of: "input", with: "d")), value: self.switchView.isOn)
        }
    }
    ...
    func config(type: String, value: Bool) {
        ...
        CenterReloadThen.settingStart(params: dict) { succeed, _, _ in
            //: ProgressHUD.dismiss()
    ...
    @objc func progressMake(notification _: NSNotification) {
//        if titleBLB.text == SettingsSizeBreedText.General.rawValue {
//            SwitchicialPermissionTool.checkPushNotification { isShow in
//                DispatchQueue.main.sync {
//                    self.switchView.isOn = isShow
//                }
