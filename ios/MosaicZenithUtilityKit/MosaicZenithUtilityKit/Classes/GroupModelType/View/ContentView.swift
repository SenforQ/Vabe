...
fileprivate let data_requestText:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "btn_me_edit" :*/
fileprivate let showManagerEqualViewMsg:[Character] = ["b","t","n","_","m","e","_","e","d","i","t"]

/*: "Program" :*/
fileprivate let constLoadModeData:String = "Programgift talk under equal"
...
class ContentView: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        self.backgroundColor = .addChoice()
        //: selectionStyle = .none
        selectionStyle = .none
        //: setupSubviews()
        titleSubviews()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_requestText.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(moveShow), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var editImg: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.snapIcon(name: (String(showManagerEqualViewMsg))))
        //: imag.isUserInteractionEnabled = false
        imag.isUserInteractionEnabled = false
        //: return imag
        return imag
...
extension ContentView {
    ...
    func section(title: String, icon: String) {
        //: iconImg.image = UIImage.BundleImageNamed(name: icon)
        iconImg.image = UIImage.snapIcon(name: icon)
        //: titleLab.text = title.localized
        titleLab.text = title.localized
        //: if title == "Program" {
        if title == (String(constLoadModeData.prefix(7))) {
    ...
    func sinceSendHidden(isHidden: Bool) {
    ...
    func enterPan() {
        ...
        backBtn.areaRecordRadii(width: user_barText - 30, height: 50, corners: [.topLeft, .topRight], cornerRadii: .init(width: 8, height: 8))
    }
}
