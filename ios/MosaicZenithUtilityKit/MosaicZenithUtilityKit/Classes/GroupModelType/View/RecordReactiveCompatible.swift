...
fileprivate let dataMakeName:[UInt8] = [0x6e,0x73,0x6e,0x79,0x2d,0x68,0x74,0x69,0x6a,0x77,0x3f,0x2e,0x25,0x6d,0x66,0x78,0x25,0x73,0x74,0x79,0x25,0x67,0x6a,0x6a,0x73,0x25,0x6e,0x72,0x75,0x71,0x6a,0x72,0x6a,0x73,0x79,0x6a,0x69]

				fileprivate func featureM(custom num: UInt8) -> UInt8 {
					let value = Int(num) - 5
					if value < 0 {
...
fileprivate let appShareMessage:[Character] = ["A","d","d"," ","m","o","r","e"," ","d","e","t","a","i"]
fileprivate let appSnapFormat:[Character] = ["l","s"," ","o","f"]
fileprivate let dataPostUrl:String = "size data from yourself"

/*: "#999999" :*/
fileprivate let data_moreFormat:String = "#showshowshow"

/*: "Tip:Don't post content that induces others to send gifts or other money-relatsed content." :*/
fileprivate let noti_sendStr:[UInt8] = [0x2e,0x13,0xa,0x40,0x3e,0x15,0x14,0x5d,0xe,0x5a,0xa,0x15,0x9,0xe,0x5a,0x19,0x15,0x14,0xe,0x1f,0x14,0xe,0x5a,0xe,0x12,0x1b,0xe,0x5a,0x13,0x14,0x1e,0xf,0x19,0x1f,0x9,0x5a,0x15,0xe,0x12,0x1f,0x8,0x9,0x5a,0xe,0x15,0x5a,0x9,0x1f,0x14,0x1e,0x5a,0x1d,0x13,0x1c,0xe,0x9,0x5a,0x15,0x8,0x5a,0x15,0xe,0x12,0x1f,0x8,0x5a,0x17,0x15,0x14,0x1f,0x3,0x57,0x8,0x1f,0x16,0x1b,0xe,0x9,0x1f,0x1e,0x5a,0x19,0x15,0x14,0xe,0x1f,0x14,0xe,0x54]

				private func intervaleractionState(key num: UInt8) -> UInt8 {
					return num ^ 122
				}
...
class RecordReactiveCompatible: UITableViewCell {
    //: let limitCount = 500
    let limitCount = 500
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(stack(notification:)), name: UIResponder.keyboardWillHideNotification, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(date(notification:)), name: UIResponder.keyboardWillShowNotification, object: nil)
    }

    //: required init?(coder aDecoder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: dataMakeName.map{featureM(custom: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleLB: UILabel = {
        ...
        lb.text = (String(appShareMessage) + String(appSnapFormat) + String(dataPostUrl.suffix(9))).localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var numberLB: UILabel = {
        ...
        LB.textColor = UIColor(hex: (data_moreFormat.replacingOccurrences(of: "show", with: "99")))
    ...
    lazy var textView: SongTitleView = {
        ...
        //: let sign = ExamineedReactiveCompatible.share.loginUserMode.signature
        let sign = ExamineedReactiveCompatible.share.loginUserMode.signature
        //: textView.text = sign
        textView.text = sign
        //: textView.placeholder = "Tip:Don't post content that induces others to send gifts or other money-relatsed content.".localized
        textView.placeholder = String(bytes: noti_sendStr.map{intervaleractionState(key: $0)}, encoding: .utf8)!.localized
        //: textView.placeholderColor = UIColor.appValueDetailColor()
        textView.placeholderColor = UIColor.aftColor()
...
extension RecordReactiveCompatible {
    ...
    @objc func stack(notification _: NSNotification) {
    ...
    @objc func date(notification: NSNotification) {
...
extension RecordReactiveCompatible: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > limitCount {
            ...
            if textNum > limitCount && limitCount > 0 {
                ...
                textView.text = quantitativeRelation(index: limitCount, text: textContent)
            }
        }
        //: numberLB.text =  "\(textView.text.count)/\(limitCount)"
        numberLB.text = "\(textView.text.count)/\(limitCount)"
    ...
    private func quantitativeRelation(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
