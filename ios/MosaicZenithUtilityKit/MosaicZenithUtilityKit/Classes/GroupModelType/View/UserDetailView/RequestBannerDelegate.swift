...
fileprivate let userViewPath:[UInt8] = [0x89,0x8e,0x89,0x94,0x48,0x83,0x8f,0x84,0x85,0x92,0x5a,0x49,0x40,0x88,0x81,0x93,0x40,0x8e,0x8f,0x94,0x40,0x82,0x85,0x85,0x8e,0x40,0x89,0x8d,0x90,0x8c,0x85,0x8d,0x85,0x8e,0x94,0x85,0x84]

				fileprivate func priceLab(title num: UInt8) -> UInt8 {
...
fileprivate let user_whiteData:String = "icleadingn"
fileprivate let k_videoMessage:String = "RDET"
fileprivate let showManagerValue:String = "tip"
fileprivate let app_showKey:[Character] = ["r","_","s","h","a","d","o","w"]

/*: "icon_lounge" :*/
fileprivate let data_snapMessage:[Character] = ["i","c","o","n"]
fileprivate let app_borderText:[Character] = ["_","l","o","u","n","g","e"]

/*: "Official" :*/
fileprivate let showGiftData:String = "to background game mainOffic"
fileprivate let appAdjustKey:[Character] = ["i","a","l"]

/*: "Reviewing" :*/
fileprivate let mainSizeName:String = "Reviattention at ting"

/*: "#FF506D" :*/
fileprivate let app_managerHiddenUrl:[Character] = ["#","F","F","5","0","6","D"]

/*: "icon_user_detail_intimate_header" :*/
fileprivate let const_pointContent:[UInt8] = [0xf,0x5,0x9,0x8,0x39,0x13,0x15,0x3,0x14,0x39,0x2,0x3,0x12,0x7,0xf,0xa,0x39,0xf,0x8,0x12,0xf,0xb,0x7,0x12,0x3,0x39,0xe,0x3,0x7,0x2,0x3,0x14]

				private func messageBottomCell(choice num: UInt8) -> UInt8 {
					return num ^ 102
				}

/*: "LiveReactiveCompatible" :*/
fileprivate let user_touchName:[Character] = ["T","U","s","e","r","D","e","t","a","i","l","H","e","a","d","e"]
fileprivate let dataLiveName:String = "rItembirth model content key"

/*: "#FFFFFF" :*/
fileprivate let data_atValue:String = "#FFFFFFremove invite user"

/*: "#BDB8B5" :*/
fileprivate let dataPathMessageName:[Character] = ["#","B","D","B","8","B","5"]

/*: "head_detail_bottom" :*/
fileprivate let userTapId:String = "hearea"
fileprivate let show_yourValue:[Character] = ["o","t","t","o","m"]

/*: "#2DF2FF" :*/
fileprivate let data_countUrl:String = "item corner#2DF2F"
fileprivate let k_statusButtonPath:String = "f"

/*: "#48FF1E" :*/
fileprivate let const_backMessage:[Character] = ["#","4","8","F","F","1","E"]

/*: "Online" :*/
fileprivate let user_toKey:String = "normal"
fileprivate let show_shadowCellName:String = "nlinsubmit"

/*: "#FF4B7F" :*/
fileprivate let app_underUrl:[Character] = ["#","F","F","4","B","7","F"]
...
let mainSceneUrl = (302.0 + app_versionItemStr)

//: class TalkingUserDetailHeaderView: UIView {
class RequestBannerDelegate: UIView {
    //: var VideoCallBlock: (() -> Void)?
    var VideoCallBlock: (() -> Void)?
    //: private var userInfoModel = TalkingUserInfoModel()
    private var userInfoModel = WithLabTransformable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userViewPath.map{priceLab(title: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bannerView: JXBanner = {
        ...
        view.placeholderImgView.image = UIImage.anArti()
    ...
    private lazy var shadowBgView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.snapIcon(name: (user_whiteData.replacingOccurrences(of: "leading", with: "o") + "_use" + k_videoMessage.lowercased() + "ail_head" + showManagerValue.replacingOccurrences(of: "tip", with: "e") + String(app_showKey))))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var vipBgView: UIImageView = {
        ...
        v.frame = CGRect(x: 0, y: mainSceneUrl - 15, width: user_barText, height: 15)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var nicknameLab: UILabel = {
        ...
        lab.font = UIFont.font(type: .Medium, fontSize: 19)
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.snapIcon(name: (String(data_snapMessage) + String(app_borderText)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var officalImgV: UIButton = {
        ...
        img.setTitle((String(showGiftData.suffix(5)) + String(appAdjustKey)).localized, for: .normal)
    ...
    private lazy var onlineImgV: GroupVideoControl = {
    ...
    private lazy var liveView: DataThen = {
        ...
        v.tapGestureBlock = { [weak self] in
            ...
            guard let self = self else { return }
            //: AppPushManager.share.func_audiencePushToLiveRoomVC(uid: self.userInfoModel.uid, enterType: .userDetail)
            AppPushManager.share.recordShow(uid: self.userInfoModel.uid, enterType: .userDetail)
        }
        //: return v
        return v
        //: }()
    ...
    private lazy var reViewingLB: UILabel = {
        ...
        lb.text = (String(mainSizeName.prefix(4)) + "ewing").localized
        //: lb.textColor = .white
        lb.textColor = .white
        //: lb.backgroundColor = UIColor.init(hex: "#FF506D")
        lb.backgroundColor = UIColor(hex: (String(app_managerHiddenUrl)))
    ...
    lazy var lastLoginLab: UILabel = {
        ...
        lb.font = .font(type: .Medium, fontSize: 12)
        //: lb.textColor = .white
        lb.textColor = .white
        //: return lb
        return lb
    ...
    private lazy var intimateBtn: GroupVideoControl = {
        //: let btn = TalkingButton()
        let btn = GroupVideoControl()
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_user_detail_intimate_header"), for: .normal)
        btn.setImage(UIImage.snapIcon(name: String(bytes: const_pointContent.map{messageBottomCell(choice: $0)}, encoding: .utf8)!), for: .normal)
        //: btn.isHidden = true
        btn.isHidden = true
        //: btn.addTarget(self, action: #selector(intimateBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(visualizationBody), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension RequestBannerDelegate {
    /// 查看视频
    //: private func goToVideo() {
    private func stopRank() {
        //: let vc = TalkingMomentVideoVC.init(videoPath: self.userInfoModel.videoUrl)
        let vc = ValueTableRecognizerDelegate(videoPath: self.userInfoModel.videoUrl)
        //: if ExamineedReactiveCompatible.share.loginUid != self.userInfoModel.uid {
        if ExamineedReactiveCompatible.share.loginUid != self.userInfoModel.uid {
    ...
    private func suite(_ index: Int) {
        ...
        let vc = PublisherDataSource(momentModel: currenModel, index: index, type: .normal)
        //: self.currentViewController()?.navigationController?.pushViewController(vc, animated: true)
        self.announcement()?.navigationController?.pushViewController(vc, animated: true)
    }

    //: @objc func intimateBtnClick() {
    @objc func visualizationBody() {
        //: let url = TalkingWebManager.getFullUrl(urlType: .Intimate) + "?toUid=\(self.userInfoModel.uid)"
        let url = ListHostManager.colorOf(urlType: .Intimate) + "?toUid=\(self.userInfoModel.uid)"
...
extension RequestBannerDelegate: JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: LiveReactiveCompatible.self, reuseIdentifier: (String(user_touchName) + String(dataLiveName.prefix(5)) + "Cell"))
    }

    //: func jxBanner(_ banner: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        //: let bannerCell = cell as! TUserDetailHeaderItemCell
        let bannerCell = cell as! LiveReactiveCompatible
        //: let urlStr = self.dataArray[index]
        let urlStr = self.dataArray[index]

        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == CommentRefCompatibleValue.female.rawValue && userInfoModel.videoUrl.count > 1 { // 视频
            //: bannerCell.setVideoView()
            bannerCell.colorTo()
            //: bannerCell.configModel(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            bannerCell.capacity(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            ...
            bannerCell.mentalRepresentation()
            //: bannerCell.bgImgView.setUrlImage(urlStr: urlStr, placeImg: UIImage.placeSquareBigImgBanner())
            bannerCell.bgImgView.applicatorEqual(urlStr: urlStr, placeImg: UIImage.withBanner())
        }
    ...
    func jxBanner(_: JXBannerType, centerIndex: Int, centerCell _: UICollectionViewCell) {
        //: guard userInfoModel.uid == ExamineedReactiveCompatible.share.loginUid else {
        guard userInfoModel.uid == ExamineedReactiveCompatible.share.loginUid else {
            //: return
            return
        }
    ...
    func jxBanner(pageControl _: JXBannerType, numberOfPages _: Int, coverView: UIView, builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder {
        ...
        pageControl.activeColor = UIColor(hex: (String(data_atValue.prefix(7))))!
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(dataPathMessageName)))!
...
extension RequestBannerDelegate: JXBannerDelegate {
    ...
    public func jxBanner(_: JXBannerType, didSelectItemAt index: Int) {
        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == CommentRefCompatibleValue.female.rawValue && userInfoModel.videoUrl.count > 1 {
            //: goToVideo()
            stopRank()
            ...
            if userInfoModel.sex == CommentRefCompatibleValue.female.rawValue && userInfoModel.videoUrl.count > 1 && curren > 0 {
                //: curren -= 1
                curren -= 1
            }
            //: goToPhoto(curren)
...
extension RequestBannerDelegate {
    ...
    func digitizer(_ userModel: WithLabTransformable) {
        ...
        if userModel.sex == CommentRefCompatibleValue.female.rawValue && userModel.videoUrl.count > 1 {
        ...
        let normalImg = UIImage.snapIcon(name: (userTapId.replacingOccurrences(of: "area", with: "ad") + "_detail_b" + String(show_yourValue))).withTintColor(.white)
        //: if userModel.loungePlus {
        if userModel.loungePlus {
            //: if let vipModel = ExamineedReactiveCompatible.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userModel.vipSkinId}).first {
            if let vipModel = ExamineedReactiveCompatible.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userModel.vipSkinId }).first {
                //: vipBgView.setUrlImage(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
                vipBgView.applicatorEqual(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
            }
            //: } else {
        } else {
        ...
        self.nicknameLab.textColor = userModel.loungePlus ? .utilizerForBodyCovering() : .white
        ...
        if userModel.intimateUrl.count > 0 {
            ...
            intimateImgV.inclose(urlStr: userModel.intimateUrl)
        }
        // 在线状态
        ...
        if userModel.livePlayUrl.count > 0 {
            ...
            let image = UIImage.imageContent(color: UIColor(hex: (String(data_countUrl.suffix(6)) + k_statusButtonPath.uppercased()))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.sitting(), for: .normal)
            onlineImgV.setImage(image.sitting(), for: .normal)
            ...
            let image = UIImage.imageContent(color: UIColor(hex: (String(const_backMessage)))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.sitting(), for: .normal)
            onlineImgV.setImage(image.sitting(), for: .normal)
            //: onlineImgV.setTitle("Online".localized, for: .normal)
            onlineImgV.setTitle((user_toKey.replacingOccurrences(of: "normal", with: "O") + show_shadowCellName.replacingOccurrences(of: "submit", with: "e")).localized, for: .normal)
            ...
            let image = UIImage.imageContent(color: UIColor(hex: (String(app_underUrl)))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.sitting(), for: .normal)
            onlineImgV.setImage(image.sitting(), for: .normal)
        ...
        // 直播
        //: if ExamineedReactiveCompatible.share.loginUid != userModel.uid &&
        if ExamineedReactiveCompatible.share.loginUid != userModel.uid,
        ...
        if liveView.isHidden == false {
            ...
            liveView.viewDoing(liveUrl: userModel.livePlayUrl)
        }
    }
    ...
    private func operationMatch() {
        //: liveView.snp.makeConstraints { make in
        liveView.snp.makeConstraints { make in
            //: make.trailing.equalToSuperview()
            make.trailing.equalToSuperview()
            //: make.top.equalTo(StatusBarHeight+80)
            make.top.equalTo(app_versionItemStr + 80)
        ...
        reViewingLB.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(app_versionItemStr + 15)
