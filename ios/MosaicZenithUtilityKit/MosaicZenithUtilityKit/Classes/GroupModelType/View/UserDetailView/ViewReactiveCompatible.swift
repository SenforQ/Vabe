...
fileprivate let userNameTextId:[UInt8] = [0x63,0x64,0x63,0x7e,0x22,0x69,0x65,0x6e,0x6f,0x78,0x30,0x23,0x2a,0x62,0x6b,0x79,0x2a,0x64,0x65,0x7e,0x2a,0x68,0x6f,0x6f,0x64,0x2a,0x63,0x67,0x7a,0x66,0x6f,0x67,0x6f,0x64,0x7e,0x6f,0x6e]

				private func modelCenterTable(item num: UInt8) -> UInt8 {
					return num ^ 10
				}

/*: "icon_details_arrow" :*/
fileprivate let noti_iconMainKey:String = "self at let any itemicon_"
fileprivate let mainTimeUrl:String = "namerrow"

/*: "btn_dynamic_stop_nor" :*/
fileprivate let noti_beautyKey:String = "btn_dcomponent var"
fileprivate let kBeMessage:String = "count letic_st"
...
class ViewReactiveCompatible: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userNameTextId.map{modelCenterTable(item: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var maskMoreView: UIView = {
        ...
        view.backgroundColor = .underReadingRgba(0, 0, 0, 0.4)
    ...
    lazy var moreButton: GroupVideoControl = .init().then {
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 17)
        $0.titleLabel?.font = .font(type: .Regular, fontSize: 17)
        //: $0.setTitle("More".localized, for: .normal)
        $0.setTitle("More".localized, for: .normal)
        //: $0.setImage(UIImage(named: "icon_details_arrow"), for: .normal)
        $0.setImage(UIImage(named: (String(noti_iconMainKey.suffix(5)) + "details_" + mainTimeUrl.replacingOccurrences(of: "name", with: "a"))), for: .normal)
    ...
    lazy var pushBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(warlikeInstalmentDuring), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ViewReactiveCompatible {
    ...
    func addResourceHidden(userModel: WithLabTransformable) {
        ...
        colorA = [UIColor.equalTarget(), UIColor.componentColor()]
        //: fontA = [UIFont.pingfangMediumFont(fontSize: 18), UIFont.pingfangMediumFont(fontSize: 16)]
        fontA = [UIFont.dismissSharedSize(fontSize: 18), UIFont.dismissSharedSize(fontSize: 16)]
        ...
        for index in 0 ..< itemCount {
            ...
            if index == 0 {
                ...
                let model: ViewTransformable = (userModel.momentPic?.list)![index]
                ...
                firstImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.parentArrange())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: firstImgView)
                    replyWith(btn: firstImgView)
                }
                ...
                secondImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.parentArrange())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: secondImgView)
                    replyWith(btn: secondImgView)
                }
                //: } else {
            } else {
                //: let model: TalkingUserInfoMomentItemModel = (userModel.momentPic?.list)![index]
                ...
                thirdImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.parentArrange())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: thirdImgView)
                    replyWith(btn: thirdImgView)
                }
            }
        }
    }
    ...
    @objc func warlikeInstalmentDuring() {
        //: let vc = TalkingUserMomentVC.init(uid: userId)
        let vc = MomentWaitViewController(uid: userId)
    ...
    func replyWith(btn: UIImageView) {
        //: btn.subviews.map {
        btn.subviews.map {
            //: (view) -> () in
            view in
        ...
        let imag = UIImageView(image: UIImage(named: (String(noti_beautyKey.prefix(5)) + "ynam" + String(kBeMessage.suffix(5)) + "op_nor")))
...
extension ViewReactiveCompatible {
    ...
    private func viewsOk() {
        ...
        let height = (user_barText - actualWidth(w: 15 * 4)) / 3.0
