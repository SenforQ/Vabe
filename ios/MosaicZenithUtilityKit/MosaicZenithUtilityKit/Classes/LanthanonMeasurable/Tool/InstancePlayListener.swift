...
/*: "/TXCache" :*/
fileprivate let mainPathMsg:String = "view snap finish push view/TX"
fileprivate let kKeyText:String = "frame guardCache"
...
protocol VoicePlayerDelegate: NSObject {
    ...
    func actual(player: InstancePlayListener, status: EnablePlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func pointInTime(player: InstancePlayListener, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func sessionAcross(player: InstancePlayListener, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class InstancePlayListener: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: VoicePlayerDelegate?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: EnablePlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = InstancePlayListener()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(mainPathMsg.suffix(3)) + String(kKeyText.suffix(5)))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(compartmentRun), userInfo: nil, repeats: true)
...
extension InstancePlayListener {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func upholdQuantityerest(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: k_scaleName.fromUser(), with: k_descriptionName)
    ...
    @objc func compartmentRun() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.sessionAcross(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func femaleAddMode(renderMode: ImpressionRenderMode) {
    ...
    func someFrom(time: Float) {
        //: self.player.seek(time)
        self.player.seek(time)
    }

    //: func height() ->Int {
    func voiceHeight() -> Int {
        //: return Int(self.player.height())
        return Int(self.player.height())
    }

    //: func width() ->Int {
    func behindPlayer() -> Int {
    ...
    func sumEnable(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isServer == true {
        if LoadSocketDelegate.shared.isServer == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func hotbed() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.actual(player: self, status: self.playStatus!)
        }
    }

    //: func isPlayings() -> Bool {
    func currentPlayings() -> Bool {
...
extension InstancePlayListener: TXVodPlayListener {
    ...
    func onPlayEvent(_: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.pointInTime(player: self, duration: Int(duration), currentTime: Int(currentTime))
