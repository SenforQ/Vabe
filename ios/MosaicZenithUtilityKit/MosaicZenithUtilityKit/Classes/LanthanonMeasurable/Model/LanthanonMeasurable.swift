...
struct LanthanonMeasurable: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
...
struct SizePhotoModel: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
enum TimeSchemaConvertible: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct ModelMeasurable: HandyJSON {
    ...
    var status: TapTitleNumbererpolation?
}

///
//: enum MomentVideoStatus: String {
enum TapTitleNumbererpolation: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension LanthanonMeasurable {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func objectEqual(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(HostThen.sum(text: content!, size: CGSize(width: user_barText - 54, height: CGFloat(MAXFLOAT)), font: UIFont.font(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == TimeSchemaConvertible.Photo.rawValue || momentType == TimeSchemaConvertible.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
