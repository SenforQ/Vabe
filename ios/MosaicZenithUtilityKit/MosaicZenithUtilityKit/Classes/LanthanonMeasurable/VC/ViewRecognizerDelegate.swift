...
/*: "Cancel" :*/
fileprivate let showViewData:String = "Canceltry item self new equal"

/*: "Upload failed" :*/
fileprivate let dataMakePath:String = "model raw iconUplo"
fileprivate let const_startModelKey:String = "sendled"

/*: "imgUrl" :*/
fileprivate let show_toUrl:String = "bottom label detail content elseimgUrl"

/*: "videoUrl" :*/
fileprivate let notiUserPath:String = "model add app screenvideoUrl"

/*: "content" :*/
fileprivate let const_tableName:[Character] = ["c","o","n","t","e","n","t"]

/*: "location" :*/
fileprivate let mainSnapStr:String = "lpathation"

/*: "Video review in progress" :*/
fileprivate let mainColorEmptyTitle:String = "request"
fileprivate let dataEqualText:String = "idetoday"
fileprivate let dataMagnitudeName:String = "return in tablew in "

/*: "Shoot video at least 5 seconds" :*/
fileprivate let showGestureTotalName:String = "Shoot month clip suite"
fileprivate let mainPicMessage:String = " at lekey add"
fileprivate let mainPlayerStr:String = "make color model view make secon"
fileprivate let main_feeModelId:String = "dvideo"

/*: "Camera" :*/
fileprivate let notiErrorValue:String = "return title component withCamera"
...
protocol LightControllerDelegate: NSObjectProtocol {
    // 发布成功
    //: func ShareSucceed()
    func clickSucceed()
}
...
class ViewRecognizerDelegate: ReloadViewController {
    //: weak var delegate: TalkingFreeViewControllerDelegate?
    weak var delegate: LightControllerDelegate?
    ...
    private lazy var reqManager: GroupViewToolReactiveCompatible = //: return GroupViewToolReactiveCompatible()
        .init()
    //: }()
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((String(showViewData.prefix(6))).localized, for: .normal)
    ...
    private lazy var postBtn: UIButton = {
        ...
        btn.setBackgroundImage(UIImage.resourceIn(colors: UIColor.appSub(), size: CGSize(width: 70, height: 30)), for: .normal)
        //: btn.layer.cornerRadius = 15
        btn.layer.cornerRadius = 15
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(postBtnClickEvent), for: .touchUpInside)
        btn.addTarget(self, action: #selector(finishPost), for: .touchUpInside)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
    ...
    private lazy var collectionView: TableDataSource = {
        //: let collectionView = FreeCollectionView(frame: CGRect.zero, collectionViewLayout: UICollectionViewFlowLayout())
        let collectionView = TableDataSource(frame: CGRect.zero, collectionViewLayout: UICollectionViewFlowLayout())
        ...
        collectionView.deleteActionBlock = { [weak self] in
            ...
            if self.collectionView.imageArray.count == 0 {
                ...
                self.collectionView.contentType = TimeSchemaConvertible.nones
                //: self.videoPath = ""
                self.videoPath = ""
                //: self.coverPath = ""
                self.coverPath = ""
...
extension ViewRecognizerDelegate {
    ...
    func alongWill() {
        ...
        reqManager.mailName(photos: self.collectionView.imageArray, content: content) { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
            ...
            NotificationCenter.default.post(name: k_adjustData, object: self, userInfo: nil)
        }
    }
    ...
    func sizeVideo() {
        ...
        PicThen.shared.humankindCompletion(type: 1, coverPath: self.coverPath, videoPath: self.videoPath) { coverUrl, videoUrl, succeed in
            //: guard succeed else {
            guard succeed else {
                //: ProgressHUD.dismiss()
                CenterProgressHUD.showAcross()
                //: self.func__showStatusBarErrorMsg(showMsg: "Upload failed".localized)
                self.heatherMixtureMoment(showMsg: (String(dataMakePath.suffix(4)) + "ad fa" + const_startModelKey.replacingOccurrences(of: "send", with: "i")).localized)
                //: return
                return
            }
            ...
            dict[(String(show_toUrl.suffix(6)))] = coverUrl
            //: dict["videoUrl"] = videoUrl
            dict[(String(notiUserPath.suffix(8)))] = videoUrl
            //: dict["content"] = content
            dict[(String(const_tableName))] = content
            //: dict["location"] = ""
            dict[(mainSnapStr.replacingOccurrences(of: "path", with: "oc"))] = ""

            //: TalkingMomentRequestTool.releaseVideoMoment(params: dict) {[weak self] succeed, result, errorModel in
            GroupViewToolReactiveCompatible.roundMoment(params: dict) { [weak self] succeed, _, _ in
                //: ProgressHUD.dismiss()
                CenterProgressHUD.showAcross()
                //: guard let self = self else { return }
                guard let self = self else { return }
                ...
                self.statusWith(showMsg: (mainColorEmptyTitle.replacingOccurrences(of: "request", with: "V") + dataEqualText.replacingOccurrences(of: "today", with: "o") + " revie" + String(dataMagnitudeName.suffix(5)) + "progress").localized)
                //: self.navigationController?.popViewController(animated: true)
                self.navigationController?.popViewController(animated: true)
                //: self.delegate?.ShareSucceed()
                self.delegate?.clickSucceed()
                //: NotificationCenter.default.post(name: POST_EXPLORE_SUCCESS_NOTIFICATION, object: self, userInfo: nil)
                NotificationCenter.default.post(name: k_adjustData, object: self, userInfo: nil)
            }
        }
...
extension ViewRecognizerDelegate {
    ...
    @objc func split() { // 返回
        ...
        PicThen.shared.compressBackground()
        //: self.navigationController?.popViewController(animated: true)
        self.navigationController?.popViewController(animated: true)
    }

    //: @objc func postBtnClickEvent() {
    @objc func finishPost() { // 发布
    ...
    func upAlbum() {
        ...
        SwitchicialPermissionTool.blockText(true) { (isOpen: Bool) in
            ...
            let vc = MessageStreamPickTool.toSign(maxCount: count, allowPhoto: true, allowVideo: false)
            ...
            vc.didFinishPickingVideoHandle = { [weak self] (_: UIImage?,
                    ...
                    MessageStreamPickTool.outsideErase(asset: assets) { filePath in
                        ...
                        if seconds >= 5 {
                            ...
                            self.pothouse(url: filePath!)
                            //: } else {
                        } else {
                            //: DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) {
                            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) {
                                //: self.func__showStatusBarErrorMsg(showMsg: "Shoot video at least 5 seconds".localized)
                                self.heatherMixtureMoment(showMsg: (String(showGestureTotalName.prefix(6)) + "video" + String(mainPicMessage.prefix(6)) + "ast 5" + String(mainPlayerStr.suffix(6)) + main_feeModelId.replacingOccurrences(of: "video", with: "s")).localized)
                            }
                        }
                    }
            }
    ...
    func pothouse(url: URL) {
        //: let videoVC = TalkingVideoEditorController(maxDuration: 60.0, videoURL: url) { videoInfo in
        let videoVC = PictureEditorController(maxDuration: 60.0, videoURL: url) { videoInfo in
...
extension ViewRecognizerDelegate: UIImagePickerControllerDelegate & UINavigationControllerDelegate {
        ...
        self.dismiss(animated: true) {
            ...
            if mediaType == kUTTypeMovie as String {
                ...
                if seconds >= 5 {
                    ...
                    self.pothouse(url: info[.mediaURL]! as! URL)
...
// MARK: - QueryedViewDelegate

//: extension TalkingFreeViewController: FreeCollectionViewDelegate {
extension ViewRecognizerDelegate: QueryedViewDelegate {
    /// 选择图片
    ...
    func corner() {
        ...
        if self.collectionView.contentType == .Photo {
            ...
            vc.zapTime(cellTitleList: [(String(notiErrorValue.suffix(6))).localized, "Photo".localized])
            //: }else {
        } else {
            //: vc.initwithList(cellTitleList: ["Camera".localized, "Photo".localized, "Video".localized])
            vc.zapTime(cellTitleList: [(String(notiErrorValue.suffix(6))).localized, "Photo".localized, "Video".localized])
        }
    ...
    func scienceLaboratory(index: IndexPath) {
        ...
        let vc = PublisherDataSource(images: self.collectionView.imageArray, index: index.row, type: .publish)
        // 删除按钮block
    ...
    func cover() {
        ...
        let vc = ValueTableRecognizerDelegate(videoPath: self.videoPath)
        //: vc.modalPresentationStyle = .fullScreen
        vc.modalPresentationStyle = .fullScreen
        //: self.present(vc, animated: true)
        self.present(vc, animated: true)
...
extension ViewRecognizerDelegate {
    ...
    func lessView() {
        ...
        cancelBtn.snp.makeConstraints { make in
            ...
            make.top.equalTo(app_versionItemStr + 7)
        ...
        collectionView.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(main_infoId)
