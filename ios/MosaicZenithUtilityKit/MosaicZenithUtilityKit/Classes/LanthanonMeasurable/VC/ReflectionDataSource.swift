...
/*: "There's no feed yet." :*/
fileprivate let app_withValue:String = "return view coverThere"
fileprivate let data_ofMessage:String = "feed yewith size"
fileprivate let noti_milkValue:[Character] = ["t","."]
...
class ReflectionDataSource: UIViewController {
    //: var pageIndex = 0
    var pageIndex = 0
    ...
    lazy var mainTableView: UITableView = {
        ...
        table.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: mainHomeTitle, right: 0)
        //: return table
        return table
...
extension ReflectionDataSource {
    //: func reqData() {
    func stigmatise() {
        ...
        GroupViewToolReactiveCompatible.clickOn(params: dict) { succeed, result, errorModel in
...
extension ReflectionDataSource {
    ...
    @objc func cookie(notification: NSNotification) {
    ...
    @objc func enableNotification(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: LanthanonMeasurable = userinfo["model"] as! LanthanonMeasurable
        //: for i in 0..<self.dataSourceArr.count {
        for i in 0 ..< self.dataSourceArr.count {
    ...
    @objc func deletenotif(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: LanthanonMeasurable = userinfo["model"] as! LanthanonMeasurable
...
extension ReflectionDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = RefreshRefView.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingMomentItemCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? RefreshRefView
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingMomentItemCell(style: .default, reuseIdentifier: identifier)
            cell = RefreshRefView(style: .default, reuseIdentifier: identifier)
        }
    ...
    func tableView(_: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if self.dataSourceArr.count > indexPath.row {
            ...
            if indexPath.row == 0 && self.isfirstvideoCell == true {
                ...
                if model.momentType == TimeSchemaConvertible.Video.rawValue {
                    //: guard let tempcell: TalkingMomentItemCell = cell as? TalkingMomentItemCell  else {
                    guard let tempcell: RefreshRefView = cell as? RefreshRefView else {
                        //: return
                        return
                    }
                    //: tempcell.playVideo()
...
extension ReflectionDataSource: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: scrollViewEndScrollPlayer(scrollView: scrollView)
        clickScroll(scrollView: scrollView)
    }
    ...
    func clickScroll(scrollView: UIScrollView) {
        ...
        if let middleIndexPath = mainTableView.indexPathForRow(at: CGPoint(x: 0, y: scrollView.contentOffset.y + mainTableView.frame.size.height / 2)) {
            ...
            if dataSourceArr.count > middleIndexPath.row {
                ...
                let cell: RefreshRefView = mainTableView.cellForRow(at: middleIndexPath) as! RefreshRefView
...
extension ReflectionDataSource {
    ...
    private func linksman() {
        ...
        style.TipsTitle = (String(app_withValue.suffix(5)) + "\'s no " + String(data_ofMessage.prefix(7)) + String(noti_milkValue)).localized
    ...
    private func through() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(enableNotification(notification:)), name: kStreamName, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(cookie(notification:)), name: constSoundTitle, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(deletenotif(notification:)), name: kLikeStr, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(specs), name: k_adjustData, object: nil)
    }
}
