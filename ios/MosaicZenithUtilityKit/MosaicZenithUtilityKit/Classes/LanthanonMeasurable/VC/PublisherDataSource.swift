...
fileprivate let constManagerValue:[UInt8] = [0x91,0x96,0x91,0x9c,0x50,0x8b,0x97,0x8c,0x8d,0x9a,0x62,0x51,0x48,0x90,0x89,0x9b,0x48,0x96,0x97,0x9c,0x48,0x8a,0x8d,0x8d,0x96,0x48,0x91,0x95,0x98,0x94,0x8d,0x95,0x8d,0x96,0x9c,0x8d,0x8c]

				fileprivate func nameInput(title num: UInt8) -> UInt8 {
...
/*: "Save successfully" :*/
fileprivate let appEqualName:String = "Save left self current frame tap"
fileprivate let constSendUrl:[Character] = ["f","u","l","l","y"]
...
class PublisherDataSource: LoadReactiveCompatible {
    ...
    init(momentModel: LanthanonMeasurable, index: Int, type: RefPhotoType) {
        ...
        if SizeLanguageManager.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(user_barText), y: 0)
                //: self.pageIndex = index
                self.pageIndex = index
                //: self.updataPageLabel()
                self.releaseWithinStigmatise()
            }
        }
    }
    ...
    init(images: [UIImage], index: Int, type: RefPhotoType) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constManagerValue.map{nameInput(title: $0)}, encoding: .utf8)!)
    }
    ...
    override func viewDidLoad() {
        ...
        collecView.reloadData()
        //: updataPageLabel()
        releaseWithinStigmatise()
        //: crushBtnTool(modle: currenModel)
        lockModle(modle: currenModel)
//        if myType == .normal {
//            setDownloadView()
//        }
    }
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: user_barText, height: show_licenseData)
        ...
        view.frame = CGRect(x: 0, y: 0, width: user_barText, height: show_licenseData)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(user_barText), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(user_barText), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(user_barText), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: TalkingMomentPhotosPreviewCell.className())
        view.register(RequestViewDelegate.self, forCellWithReuseIdentifier: RequestViewDelegate.className())
...
//    lazy var downloadBtn: GroupVideoControl = {
//        let btn = GroupVideoControl.init()
...
//        SwitchicialPermissionTool.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
//                guard let self = self else {return}
//                guard  self.pageIndex >= 0 && self.pageIndex < self.currenModel.pic.count  else { return }
//                let model: SizePhotoModel = self.currenModel.pic[self.pageIndex]
...
//    }
    ...
    func mentalRepresentationUser(image: UIImage) {
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.statusWith(showMsg: (String(appEqualName.prefix(5)) + "success" + String(constSendUrl)).localized)
                }
                //: } else {
            } else {
...
extension PublisherDataSource: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func bottomForView() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func toView() {
        //: if  currenModel.sex==ExamineedReactiveCompatible.share.loginUserMode.sex {
        if currenModel.sex == ExamineedReactiveCompatible.share.loginUserMode.sex {
            //: return
            return
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: RequestViewDelegate = collectionView.dequeueReusableCell(withReuseIdentifier: RequestViewDelegate.className(), for: indexPath) as! RequestViewDelegate
        //: if myType == .normal {
        if myType == .normal {
            //: let model: TalkingMomentPhotoModel = currenModel.pic[indexPath.row]
            let model: SizePhotoModel = currenModel.pic[indexPath.row]
            //: if model.imgUrl?.count ?? 0 > 1 {
            if model.imgUrl?.count ?? 0 > 1 {
                //: cell.configUrl(url: model.imgUrl! )
                cell.videoTo(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.picTip(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.picTip(image: photos[indexPath.row])
        }
    ...
    func collectionView(_: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt _: IndexPath) {
        ...
        let tcell: RequestViewDelegate = cell as! RequestViewDelegate
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / user_barText)
        //: if myType == .normal {
        if myType == .normal {
