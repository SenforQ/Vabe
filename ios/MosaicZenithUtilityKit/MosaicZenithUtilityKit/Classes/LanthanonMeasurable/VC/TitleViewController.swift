...
/*: "News Feed" :*/
fileprivate let const_sectionPath:[Character] = ["N","e","w","s"," ","F","e","e","d"]

/*: "There's no feed yet." :*/
fileprivate let const_addText:String = "make letThere"
fileprivate let data_startValue:String = " feeedit add let error under"
fileprivate let kUserRawUrl:String = "d yet.count by"

/*: "The post has been deleted ~" :*/
fileprivate let main_sectionData:[Character] = ["T","h","e"," ","p","o","s","t"," ","h","a","s"," ","b","e","e","n"," ","d","e","l","e","t"]
fileprivate let main_makeContent:String = "ed ~equal button equal for mutual"
...
class TitleViewController: ReloadViewController {
    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var dataArr: [TalkingRemindListModel] = []
    private var dataArr: [MessageModelType] = []
    ...
    override func viewDidLoad() {
        ...
        title = (String(const_sectionPath)).localized
        //: setupSubviews()
        vertical()
    ...
    lazy var emptyView: EmptyView = {
        ...
        style.TipsTitle = (String(const_addText.suffix(5)) + "\'s no" + String(data_startValue.prefix(4)) + String(kUserRawUrl.prefix(6))).localized
    ...
    lazy var tableView: UITableView = {
        ...
        table.register(ModelThen.self, forCellReuseIdentifier: ModelThen.className())
        //: return table
        return table
        //: }()
    }()
...
extension TitleViewController {
    ...
    private func behindShouldVoice() {
        //: pageIndex = 0
        pageIndex = 0
        //: loadListData()
        drawUpData()
    ...
    private func drawUpData() {
        //: TalkingMomentRequestTool.req_getRemindList(page: pageIndex) { data in
        GroupViewToolReactiveCompatible.roundConstraint(page: pageIndex) { data in
...
extension TitleViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: ModelThen.className(), for: indexPath) as! ModelThen
        //: cell.refreshCell(dataArr[indexPath.row])
        cell.restorePathCellModel(dataArr[indexPath.row])
        //: return cell
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if model.status == 0 {
            ...
            heatherMixtureMoment(showMsg: (String(main_sectionData) + String(main_makeContent.prefix(4))).localized)
            //: return
            return
        }
        //: var detailModel = TalkingMomentModel()
        var detailModel = LanthanonMeasurable()
        //: detailModel.mid = model.momentId
        detailModel.mid = model.momentId
        //: detailModel.uid = String(model.uid)
        detailModel.uid = String(model.uid)
        //: let detailVC = TalkingMomentDetailVC(model: detailModel)
        let detailVC = VisualCommunicationViewController(model: detailModel)
