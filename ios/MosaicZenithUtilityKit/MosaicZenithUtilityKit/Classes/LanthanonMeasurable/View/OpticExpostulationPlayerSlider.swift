...
fileprivate let user_equalMessage:[UInt8] = [0x72,0x75,0x72,0x6f,0x33,0x78,0x74,0x7f,0x7e,0x69,0x21,0x32,0x3b,0x73,0x7a,0x68,0x3b,0x75,0x74,0x6f,0x3b,0x79,0x7e,0x7e,0x75,0x3b,0x72,0x76,0x6b,0x77,0x7e,0x76,0x7e,0x75,0x6f,0x7e,0x7f]

				private func doingData(model num: UInt8) -> UInt8 {
					return num ^ 27
				}
...
// __CLOSE_PRINT__
//
//  OpticExpostulationPlayerSlider.swift
//  AbroadTalking
//
...
class OpticExpostulationPlayerSlider: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_equalMessage.map{doingData(model: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var progressView: ProgressThen = {
        ...
        let v = ProgressThen(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(addMotility(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(doService(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension OpticExpostulationPlayerSlider {
    //: @objc func panGesture(gesture: UIPanGestureRecognizer) {
    @objc func addMotility(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func doService(gesture: UIPanGestureRecognizer) {
