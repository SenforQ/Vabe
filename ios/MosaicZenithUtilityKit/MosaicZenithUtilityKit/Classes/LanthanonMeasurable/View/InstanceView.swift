...
fileprivate let dataEachMessage:[UInt8] = [0x81,0x86,0x81,0x8c,0x40,0x7b,0x87,0x7c,0x7d,0x8a,0x52,0x41,0x38,0x80,0x79,0x8b,0x38,0x86,0x87,0x8c,0x38,0x7a,0x7d,0x7d,0x86,0x38,0x81,0x85,0x88,0x84,0x7d,0x85,0x7d,0x86,0x8c,0x7d,0x7c]

				fileprivate func showStorage(add num: UInt8) -> UInt8 {
...
fileprivate let app_sharedFormat:String = "btn_dyraw in"
fileprivate let const_pathStr:String = "show for viewp_nor"

/*: "btnwOblEh_tdj_oediv_stop" :*/
fileprivate let noti_onUrl:[Character] = ["b","t","n","w","O","b","l"]
fileprivate let main_showViewName:String = "normal share lab varEh_tdj"
fileprivate let show_norStr:String = "model let self net size_stop"

/*: "btnCmjoS7_tdj_oediv_play" :*/
fileprivate let mainWithId:[Character] = ["b","t","n","C","m","j","o"]
fileprivate let user_viewTitle:String = "path7"
fileprivate let noti_noMsg:String = "_tdj_osize title model"
...
protocol ViewDelegate: NSObject {
    ...
    func giftLeftHandednessInsert(view: InstanceView, status: VoiceViewStatus)

    //: func func__interactionViewSlideValue(view: TalkingVideoPlayerInteractionView, value: CGFloat)
    func path(view: InstanceView, value: CGFloat)
}
...
class InstanceView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        super.init(frame: frame)
        //: self.setupSubviews()
        self.packageSubviews()
        //: self.setupSubViewsConstraint()
        self.pointConstraint()
        //: self.bindInteraction()
        self.stickToWithoutCounteraction()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: dataEachMessage.map{showStorage(add: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var centerPlayButton: UIButton = {
        ...
        btn.setImage(UIImage.snapIcon(name: (String(app_sharedFormat.prefix(6)) + "namic_sto" + String(const_pathStr.suffix(5)))), for: .normal)
        //: btn.addTarget(self, action: #selector(centerButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(totalryToAdd(sender:)), for: .touchUpInside)
        //: btn.isHidden = true
        btn.isHidden = true
        //: return btn
        return btn
    ...
    private lazy var playButton: UIButton = {
        ...
        btn.setImage(UIImage.snapIcon(name: (String(noti_onUrl) + String(main_showViewName.suffix(6)) + "_oediv" + String(show_norStr.suffix(5)))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btnCmjoS7_tdj_oediv_play"), for: .selected)
        btn.setImage(UIImage.snapIcon(name: (String(mainWithId) + user_viewTitle.replacingOccurrences(of: "path", with: "S") + String(noti_noMsg.prefix(6)) + "ediv_play")), for: .selected)
        //: btn.addTarget(self, action: #selector(playButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(readingVideo(sender:)), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var failImageView: UIImageView = {
        ...
        imag.image = UIImage.snapIcon(name: "")
        //: imag.isHidden = true
        imag.isHidden = true
    ...
    private lazy var slider: OpticExpostulationPlayerSlider = {
...
extension InstanceView {
    ...
    private func stickToWithoutCounteraction() {
        ...
        self.slider.getValueBlock = { [weak self] value in
            ...
            self.delegate?.path(view: self, value: value)
        }
