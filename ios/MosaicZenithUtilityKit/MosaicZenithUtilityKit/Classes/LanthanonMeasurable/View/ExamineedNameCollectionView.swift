...
fileprivate let kMethodMsg:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
// __CLOSE_PRINT__
//
//  ExamineedNameCollectionView.swift
//  AbroadTalking
//
...
class ExamineedNameCollectionView: UICollectionView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kMethodMsg.reversed(), encoding: .utf8)!)
    }

    //: func configModel(model: TalkingMomentModel) {
    func tableModel(model: LanthanonMeasurable) {
        //: cunrrenModel = model
        cunrrenModel = model
        //: self.reloadData()
        self.reloadData()
...
extension ExamineedNameCollectionView: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, insetForSectionAt _: Int) -> UIEdgeInsets {
        ...
        if cunrrenModel.pic.count == 4 {
            ...
            return UIEdgeInsets(top: 0, left: 0, bottom: 0, right: user_barText - 200 - 30)
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, sizeForItemAt _: IndexPath) -> CGSize {
        ...
        if cunrrenModel.pic.count == 1 {
            ...
            var width = isMyhost ? (user_barText - 30 - 24 - 30) / 3 : (user_barText - 30 - 24) / 3
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, referenceSizeForHeaderInSection _: Int) -> CGSize {
        ...
        return CGSize(width: user_barText, height: 1)
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: GroupThen = collectionView.dequeueReusableCell(withReuseIdentifier: GroupThen.className(), for: indexPath) as! GroupThen
        //: let model: TalkingMomentPhotoModel = cunrrenModel.pic[indexPath.row]
        let model: SizePhotoModel = cunrrenModel.pic[indexPath.row]
        //: cell.configUrl(url: model.thumbUrl ?? "")
        cell.ping(url: model.thumbUrl ?? "")
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = PublisherDataSource(momentModel: cunrrenModel, index: indexPath.row, type: .normal)
...
extension ExamineedNameCollectionView {
    // 添加视图
    //: private func setupSubviews() {
    private func compartmentSend() {
        //: backgroundColor = .clear
        backgroundColor = .clear
        //: register(TalkingMomentPhotosCell.self, forCellWithReuseIdentifier: TalkingMomentPhotosCell.className())
        register(GroupThen.self, forCellWithReuseIdentifier: GroupThen.className())
        //: dataSource = self
        dataSource = self
        //: delegate = self
        delegate = self
