...
fileprivate let noti_errorId:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "F5F5F5" :*/
fileprivate let constDomainTitle:String = "Fmakemake5"
...
class RefreshRefView: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_errorId.reversed(), encoding: .utf8)!)
    }
    ...
    func flagWill(isListTableCell: Bool) {
    ...
    lazy var userInfoView: ImageVideoReactiveCompatible = {
        //: let  view = TalkingMomentUserInfoView.init()
        let view = ImageVideoReactiveCompatible()
    ...
    lazy var textContentView: LiveSectionTextView = {
        //: let  view = TalkingMomentTextContentView.init()
        let view = LiveSectionTextView()
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var photoView: ExamineedNameCollectionView = {
        ...
        let view = ExamineedNameCollectionView(frame: CGRect(x: 0, y: 0, width: 0, height: 0), collectionViewLayout: flayout)
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var bottomView: ItemViewSideMomentComponentPlayerDelegate = {
        //: let  view = TalkingMomentItemBottomView.init()
        let view = ItemViewSideMomentComponentPlayerDelegate()
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var lineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (constDomainTitle.replacingOccurrences(of: "make", with: "5F")))
        //: self.contentView.addSubview(view)
        self.contentView.addSubview(view)
        //: return view
        return view
...
extension RefreshRefView {
    //: func configCell(model: TalkingMomentModel) {
    func someModel(model: LanthanonMeasurable) {
        //: cunrrenModel = model
        cunrrenModel = model
        //: userInfoView.isHidden = false
        userInfoView.isHidden = false
        //: userInfoView.configInfo(model: model)
        userInfoView.statisticalTable(model: model)
        //: photoView.isMyhost = self.isMyhost
        photoView.isMyhost = self.isMyhost
        //: textContentView.configText(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)
        textContentView.installment(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)

        //: if model.momentType == MomentType.Photo.rawValue || model.momentType == MomentType.Live.rawValue {
        if model.momentType == TimeSchemaConvertible.Photo.rawValue || model.momentType == TimeSchemaConvertible.Live.rawValue {
...
//            }
            ...
            photoView.tableModel(model: cunrrenModel)
...
//            }
        ...
        bottomView.gamut(model: cunrrenModel)
...
//        }
    ...
    @objc func errorEqual() {
        //: let vc = TalkingMomentVideoVC.init(videoPath: cunrrenModel.videoInfo!.videoUrl)
        let vc = ValueTableRecognizerDelegate(videoPath: cunrrenModel.videoInfo!.videoUrl)
        //: if cunrrenModel.sex != ExamineedReactiveCompatible.share.loginUserMode.sex {
        if cunrrenModel.sex != ExamineedReactiveCompatible.share.loginUserMode.sex {
    ...
    func imageUp() {
        ...
        if cunrrenModel.momentType == TimeSchemaConvertible.Video.rawValue {
...
extension RefreshRefView {
    ...
    private func conversion() {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(errorEqual))
