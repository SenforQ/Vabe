...
fileprivate let appUserMessage:[UInt8] = [0xac,0xab,0xac,0xb1,0xed,0xa6,0xaa,0xa1,0xa0,0xb7,0xff,0xec,0xe5,0xad,0xa4,0xb6,0xe5,0xab,0xaa,0xb1,0xe5,0xa7,0xa0,0xa0,0xab,0xe5,0xac,0xa8,0xb5,0xa9,0xa0,0xa8,0xa0,0xab,0xb1,0xa0,0xa1]

				private func keyValue(effect num: UInt8) -> UInt8 {
					return num ^ 197
				}

/*: "8075F5" :*/
fileprivate let dataMessagePath:String = "8"
fileprivate let userAttributeAppAtUrl:String = "index image075F5"

/*: "9A9B9C" :*/
fileprivate let user_fileName:String = "9A9B9Cclass normal over"

/*: "kOpacityAnimation" :*/
fileprivate let const_topText:String = "lab live model sendkOpaci"
fileprivate let notiHomeMsg:String = "normatnoron"

/*: "opacity" :*/
fileprivate let user_skinData:String = "opadatait"
fileprivate let dataFromMinLetUrl:String = "to"

/*: "forwards" :*/
fileprivate let main_colorPath:String = "FORWAR"
fileprivate let kReportInfoTitle:String = "dpost"

/*: "easeIn" :*/
fileprivate let dataOffText:String = "E"
fileprivate let noti_sendName:[Character] = ["a","s","e","I","n"]

/*: "8C7AF8" :*/
fileprivate let show_ofSkipId:String = "hide array data8C7AF"
fileprivate let user_picMsg:[Character] = ["8"]
...
class GroupView: UIView {
    ...
    public init(frame: CGRect, codeNumber: Int = 6, labelTextColor: UIColor = UIColor.black, mainColor: UIColor = UIColor.orange, normalColor: UIColor = UIColor.gray, margin: CGFloat = 12.0) {
        ...
        self.bindMagnitudeeraction()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: appUserMessage.map{keyValue(effect: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var textField: UITextField = {
        ...
        view.addTarget(self, action: #selector(color(_:)), for: .editingChanged)
        //: return view
        return view
        //: }()
    }()
...
extension GroupView {
    ...
    func quantity() {
        ...
        for index in 0 ..< codeNumber {
            ...
            if index < verStr.count {
                ...
                layer.shadowColor = UIColor(hex: (dataMessagePath.capitalized + String(userAttributeAppAtUrl.suffix(5))))!.withAlphaComponent(0.6).cgColor
            }
            //: else {
            else {
                //: label.text = ""
                ...
                if index == verStr.count {
                    ...
                    layer.shadowColor = UIColor(hex: (String(user_fileName.prefix(6))))!.withAlphaComponent(0.25).cgColor
            ...
            moment(index: index, hidden: index == verStr.count ? false : true)
        }
    }

    // 当输入内容等于验证码的长度时候，把输入的验证码回调
    //: @objc func textChage(_ textField: UITextField) {
    @objc func color(_: UITextField) {
        //: func__changeCodeInputView()
        quantity()
    }
    ...
    fileprivate func moment(index: Int, hidden: Bool) {
        //: let line = shapeArray[index]
        let line = shapeArray[index]
        //: if hidden {
        if hidden {
            //: line.removeAnimation(forKey: "kOpacityAnimation")
            line.removeAnimation(forKey: (String(const_topText.suffix(6)) + "tyAn" + notiHomeMsg.replacingOccurrences(of: "nor", with: "i")))
            //: } else {
        } else {
            //: line.add(opacityAnimation(), forKey: "kOpacityAnimation")
            line.add(dataAcross(), forKey: (String(const_topText.suffix(6)) + "tyAn" + notiHomeMsg.replacingOccurrences(of: "nor", with: "i")))
        }
    ...
    fileprivate func dataAcross() -> CABasicAnimation {
        //: let animation = CABasicAnimation.init(keyPath: "opacity")
        let animation = CABasicAnimation(keyPath: (user_skinData.replacingOccurrences(of: "data", with: "c") + dataFromMinLetUrl.replacingOccurrences(of: "to", with: "y")))
        ...
        animation.fillMode = CAMediaTimingFillMode(rawValue: (main_colorPath.lowercased() + kReportInfoTitle.replacingOccurrences(of: "post", with: "s")))
        //: animation.timingFunction = CAMediaTimingFunction.init(name: CAMediaTimingFunctionName.init(rawValue: "easeIn"))
        animation.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName(rawValue: (dataOffText.lowercased() + String(noti_sendName))))
        //: return animation
        return animation
    }
}
...
extension GroupView {
    ...
    private func afterYearCell() {
        ...
        for index in 0 ..< codeNumber {
            ...
            line.fillColor = UIColor(hex: (String(show_ofSkipId.suffix(5)) + String(user_picMsg)))?.cgColor
    ...
    private func textMini() {}

    // 添加事件
    //: private func bindInteraction() {
    private func bindMagnitudeeraction() {}
}
