...
/*: "recharge" :*/
fileprivate let k_collectionPath:String = "recenterarge"

/*: "recharge_ :*/
fileprivate let notiWorldGenderName:[Character] = ["r","e","c","h","a","r","g","e","_"]
...
enum RequestImageAlignment: NSInteger {
    //: case left = 0
    case left = 0
    //: case top
    case top
...
class GroupVideoControl: UIButton {
    ...
    var imageAlignment: RequestImageAlignment = .left
...
extension UIButton {
    ...
    func exaggerateName(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        analogDigitalConverterMake(interval: interval, topOffset: 0)
    }
    ...
    func analogDigitalConverterMake(interval: CGFloat, topOffset: CGFloat) {
    ...
    func nameDown(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func errorHeritage(color: UIColor, forState: UIControl.State) {
    ...
    func publicTransportSum(urlStr: String,
                            //: placeImg: UIImage? = UIImage.placeImgSquare(),
                            placeImg: UIImage? = UIImage.parentArrange(),
                            ...
                            loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: k_scaleName.fromUser(), with: k_descriptionName)
    ...
    func showUp(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((k_collectionPath.replacingOccurrences(of: "center", with: "ch"))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (k_collectionPath.replacingOccurrences(of: "center", with: "ch")), with: (String(notiWorldGenderName)) + "\(mainStatusHalfMessage)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.publicTransportSum(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains(".png") {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(mainStatusHalfMessage).png")

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains(".jpg") {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(mainStatusHalfMessage).jpg")
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.publicTransportSum(urlStr: newUrl, placeImg: nil)
    }
