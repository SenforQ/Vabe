...
/*: "RSA加密失败，str:  :*/
fileprivate let dataNearbyManagerPath:String = "RSA加密view mask if"
fileprivate let constShareUrl:String = "color"

/*: "weixin://" :*/
fileprivate let userViewName:String = "effect"
fileprivate let main_labelPerformPath:String = "left path true return equalixin://"

/*: "dingtalk://" :*/
fileprivate let data_atPath:String = "dincontentt"
fileprivate let notiExtraPath:[Character] = ["/","/"]

/*: "wxwork://" :*/
fileprivate let kSnapStr:[Character] = ["w","x","w","o","r","k",":"]
fileprivate let constInstanceText:[Character] = ["/","/"]

/*: "deviceId" :*/
fileprivate let main_byName:[UInt8] = [0x57,0x56,0x45,0x5a,0x50,0x56,0x7a,0x57]

				private func broadcastLab(collection num: UInt8) -> UInt8 {
					return num ^ 51
				}

/*: "weixin" :*/
fileprivate let show_layerData:String = "WEIXIN"

/*: "wxwork" :*/
fileprivate let show_quickTitle:[Character] = ["w","x","w","o","r"]
fileprivate let const_textName:String = "icon"

/*: "dingtalk" :*/
fileprivate let userChangeTitle:[Character] = ["d","i","n","g","t"]
fileprivate let notiImageData:String = "auserk"
...
extension UIDevice {
    ...
    class func labBlock(IdentifierBlock: @escaping (_ str: String) -> Void) {
        ...
        if #available(iOS 14.0, *) {
            ...
            ATTrackingManager.requestTrackingAuthorization(completionHandler: { status in
                ...
                if status == .authorized {
                    ...
                    IdentifierBlock(UIDevice.getTakeApp())
                    //: } else {
                } else {
                    //: IdentifierBlock("")
                    IdentifierBlock("")
...
//            let mnc = carrier?.mobileNetworkCode  //移动网络代码
    ...
    static func awakeDoing(str: String) -> String {
        //: var encryptedStr = ""
        var encryptedStr = ""
        //: do {
        do {
            ...
            printLog(message: (String(dataNearbyManagerPath.prefix(5)) + "失败，str:" + constShareUrl.replacingOccurrences(of: "color", with: " ")) + "\(str).")
        }
        //: return encryptedStr
        return encryptedStr
    }
...
public extension UIDevice {
    ...
    static var isOpenWX: Bool {
        ...
        let appSchema = (userViewName.replacingOccurrences(of: "effect", with: "we") + String(main_labelPerformPath.suffix(7)))
    ...
    static var isOpenDD: Bool {
        ...
        let appSchema = (data_atPath.replacingOccurrences(of: "content", with: "g") + "alk:" + String(notiExtraPath))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenWXWork: Bool {
        ...
        let appSchema = (String(kSnapStr) + String(constInstanceText))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var customDeviceNum: String {
        ...
        if let value = key.get(show_messageSeatData) {
            //: return value
            return value
            //: }else{
        } else {
            ...
            key.set(value, forKey: show_messageSeatData)
            //: return value
            return value
        }
    }
    ...
    static var deviceInfoDic: [String: Any] {
        ...
        let usedVpn = RecordAddrTool.allTap() || RecordAddrTool.activeVpn()
        ...
        if let paramsStr = String(data: data!, encoding: String.Encoding.utf8) {
            ...
            let deviceIdStr = awakeDoing(str: adid)
            //: return ["deviceId": deviceIdStr,
            return [String(bytes: main_byName.map{broadcastLab(collection: $0)}, encoding: .utf8)!: deviceIdStr,
                    //: "p0": paramsStr]
                    "p0": paramsStr]
        }
        //: return [:]
    ...
    static var getApps: String? {
        ...
        if UIDevice.isOpenWX {
            ...
            appsArr.append((show_layerData.lowercased()))
        }
        //: if UIDevice.isOpenWXWork {
        if UIDevice.isOpenWXWork {
            //: appsArr.append("wxwork")
            appsArr.append((String(show_quickTitle) + const_textName.replacingOccurrences(of: "icon", with: "k")))
        }
        //: if UIDevice.isOpenDD {
        if UIDevice.isOpenDD {
            //: appsArr.append("dingtalk")
            appsArr.append((String(userChangeTitle) + notiImageData.replacingOccurrences(of: "user", with: "l")))
        }
