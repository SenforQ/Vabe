...
/*: "add_header_cookies" :*/
fileprivate let data_labelStr:[Character] = ["a","d","d","_","h","e","a","d","e","r","_","c","o","o","k"]
fileprivate let noti_pathUnderValue:[Character] = ["i","e","s"]
...
public extension UIImageView {
    ...
    @objc func inclose(urlStr: String,
    ...
    internal func applicatorEqual(urlStr: String,
                                  //: placeImg: UIImage = UIImage.placeImgSquare(),
                                  placeImg: UIImage = UIImage.parentArrange(),
    ...
    @objc func methodSet(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: k_scaleName.fromUser(), with: k_descriptionName)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.detailGift())
    }
    ...
    @objc func imbecility(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: RecordAddrTool.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(data_labelStr) + String(noti_pathUnderValue)))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.parentArrange(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
