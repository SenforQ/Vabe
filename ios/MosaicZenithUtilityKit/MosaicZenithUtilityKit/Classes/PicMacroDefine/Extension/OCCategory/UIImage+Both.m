...
- (UIImage *)filter {
    ...
    return [UIImage imageWithData:[self videoTo]];
}


//: - (UIImage *)wcSessionCompress {
- (UIImage *)sessionBecome {
    //: return [self wcCompres:YES];
    return [self indicator:YES];
}

//: - (UIImage *)wcTimelineCompress {
- (UIImage *)margin {
    //: return [self wcCompres:NO];
    return [self indicator:NO];
}
...
- (UIImage *)indicator:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self level:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self message:size];
...
- (CGSize)level:(Boolean)isSession {
...
+ (NSData *)input:(UIImage *)image trademarkWhite:(NSInteger)fileSize{
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG noTarget:scalesize connectionForScale:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG icon:(fileSize*1024)];
    }
...
- (UIImage*)theory:(CGSize)targetSize{
...
- (UIImage *)noTarget:(CGSize)targetSize connectionForScale:(BOOL)isScale{
...
- (NSData *)icon:(NSInteger)fileSize{
...
- (UIImage *)message:(CGSize)newSize {
