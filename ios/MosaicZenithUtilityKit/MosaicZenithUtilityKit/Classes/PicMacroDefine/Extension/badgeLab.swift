...
class ViewBadgeLab: UILabel {
    ...
    //: private func setup() {
    private func spokeModel() {
        //: translatesAutoresizingMaskIntoConstraints = false
        translatesAutoresizingMaskIntoConstraints = false
        //: layer.backgroundColor = UIColor.msgTipsColor().cgColor
        layer.backgroundColor = UIColor.cellMore().cgColor
        //: textColor = .white
        textColor = .white
        //: font = UIFont.pingfangFont(type: .Medium, fontSize: 11)
        font = UIFont.font(type: .Medium, fontSize: 11)
...
class LimitedThen: UILabel {
    ...
    func mastheadCorner(isTop:Bool) {
        //: if isTop {
        if isTop {
