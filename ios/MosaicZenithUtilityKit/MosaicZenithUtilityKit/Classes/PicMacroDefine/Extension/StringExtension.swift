...
/*: "ZUhWdWQzVnNiM1ps" :*/
fileprivate let user_centerStr:String = "import videoZUhWdW"
fileprivate let show_listUrl:[Character] = ["M","1","p","s"]

/*: "OrKulDHBYyjUHZTy1sS7dw==" :*/
fileprivate let userCornerKey:String = "title color sizeOrKul"
fileprivate let data_itemMakePath:String = "yjUHZTy1table link"
fileprivate let notiMaxUrl:[Character] = ["s","S","7","d","w","=","="]

/*: "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMnopQRSTUVWXYZ0123456789" :*/
fileprivate let notiQuitContent:[UInt8] = [0x14,0x15,0x16,0x17,0x18,0x19,0x1a,0x1b,0x1c,0x1d,0x1e,0x1f,0x20,0x21,0x22,0x23,0x24,0x25,0x26,0x27,0x28,0x29,0x2a,0x2b,0x2c,0x2d,0xf4,0xf5,0xf6,0xf7,0xf8,0xf9,0xfa,0xfb,0xfc,0xfd,0xfe,0xff,0x0,0x21,0x22,0x23,0x4,0x5,0x6,0x7,0x8,0x9,0xa,0xb,0xc,0xd,0xe3,0xe4,0xe5,0xe6,0xe7,0xe8,0xe9,0xea,0xeb,0xec]

				fileprivate func colorData(window num: UInt8) -> UInt8 {
...
let mainBarStr = (String(user_centerStr.suffix(6)) + "QzVnNi" + String(show_listUrl))

//: let UrlDomin = "OrKulDHBYyjUHZTy1sS7dw=="
let k_scaleName = (String(userCornerKey.suffix(5)) + "DHBY" + String(data_itemMakePath.prefix(8)) + String(notiMaxUrl))
...
extension String {
    ...
    func readBy(_ matchStr: String) -> [NSRange] {
...
extension String {
    ...
    static func labCheck(length: Int) -> String {
        //: let letters: NSString = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMnopQRSTUVWXYZ0123456789"
        let letters: NSString = String(bytes: notiQuitContent.map{colorData(window: $0)}, encoding: .utf8)! as NSString
    ...
    func have(key: String) -> String? {
        ...
        let EncryptData = (inputData as NSData).tactilePropertyTitle(key)
        //: return NSData.base64String(from: EncryptData, length: UInt((EncryptData as NSData).length))
        return NSData.age(EncryptData, at: UInt((EncryptData as NSData).length))
    }

    //: func aes256Decrypt(key: String) -> String? {
    func equalBeauty(key: String) -> String? {
        //: let encryData = NSData.base64Data(from: self)
        let encryData = NSData.gift(self)
        //: let decryData = (encryData as NSData).aes256Decrypt(withKey: key)
        let decryData = (encryData as NSData).harvestMoon(key)
    ...
    func buttonShow() -> String? {
        //: printLog(message: "加密参数= \(self)")
        printLog(message: "加密参数= \(self)")
        //: guard let inputData: Data = data(using: .utf8) else { return nil }
        guard let inputData: Data = data(using: .utf8) else { return nil }
        //: let EncryptData = (inputData as NSData).aes256Encrypt(withKey: UrlKey)
        let EncryptData = (inputData as NSData).tactilePropertyTitle(mainBarStr)
        //: return NSData.base64String(from: EncryptData, length: UInt((EncryptData as NSData).length))
        return NSData.age(EncryptData, at: UInt((EncryptData as NSData).length))
    }
    ...
    func fromUser() -> String {
        ...
        let decryData = (encryData as NSData).harvestMoon(mainBarStr)
        //: let decryStr = String(data: decryData, encoding: .utf8)
        let decryStr = String(data: decryData, encoding: .utf8)
        //: return decryStr ?? ""
        return decryStr ?? ""
    ...
    static func unicodeNowTo(str: String, from: Int, to: Int) -> String {
