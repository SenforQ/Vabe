...
fileprivate let kGiftStr:[UInt8] = [0xd8,0xee,0xf4,0x9f,0xe0,0xf1,0xe4,0x9f,0xe2,0xf4,0xf1,0xf1,0xe4,0xed,0xf3,0xeb,0xf8,0x9f,0xee,0xed,0x9f,0xe0,0x9f,0xe2,0xe0,0xeb,0xeb,0xab,0x9f,0xef,0xeb,0xe4,0xe0,0xf2,0xe4,0x9f,0xf3,0xf1,0xf8,0x9f,0xe0,0xe6,0xe0,0xe8,0xed,0x9f,0xeb,0xe0,0xf3,0xe4,0xf1]

				fileprivate func sharedMake(wrap num: UInt8) -> UInt8 {
...
/*: "onCallResume" :*/
fileprivate let user_modelData:[Character] = ["o","n","C","a","l","l","R","e"]
fileprivate let showMViewName:[Character] = ["s","u","m","e"]
...
// __CLOSE_PRINT__
//
//  ExamineedReactiveCompatible+VideoChat.swift
//  AbroadTalking
//
...
extension ExamineedReactiveCompatible {
    ...
    func taskSituation(info: [String: Any], situation: LimitedChatSituation = .Start) {
        //: let json = JSON(info)
        let json = JSON(info)
...
extension ExamineedReactiveCompatible {
    ...
    private func capabilityAdd(info: [String: Any], resumeTalk: Bool) {
        //: if !resumeTalk && TalkingSocketManager.shared.isServer {
        if !resumeTalk && LoadSocketDelegate.shared.isServer {
            //: self.func__showStatusBarErrorMsg(showMsg: "You are currently on a call, please try again later".localized)
            self.heatherMixtureMoment(showMsg: String(bytes: kGiftStr.map{sharedMake(wrap: $0)}, encoding: .utf8)!.localized)
            //: return
            return
        }
        ...
        if announcement()!.isKind(of: ComponentErrorDelegate.self) ||
            //: currentViewController()!.isKind(of: TalkingVoiceChatViewController.self) ||
            announcement()!.isKind(of: SeedViewController.self) ||
            //: currentViewController()!.isKind(of: TalkingVideoInitivCallVC.self) {
            announcement()!.isKind(of: VideoEqualVc.self)
        {
            //: return
            return
        }
        ...
        if type == 0 { // 视频通话
            ...
            (vc as! ComponentErrorDelegate).chatModel = HumankindChatModel.master(info: info, situation: resumeTalk ? .Resume : .Recieved)
            //: } else {
        } else { // 语音通话
            //: vc = TalkingVoiceChatViewController()
            vc = SeedViewController()
            ...
            (vc as! SeedViewController).chatModel = HumankindChatModel.master(info: info, situation: resumeTalk ? .Resume : .Recieved)
        }

        //: let callResume = json["onCallResume"].stringValue
        let callResume = json[(String(user_modelData) + String(showMViewName))].stringValue
        ...
        if resumeTalk, callResume.count > 0, callResume == "1" { // 恢复通话
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
                ...
                self.isMessage(roomVC: vc!, isVideoTalk: type == 0 ? true : false, resumeTalk: resumeTalk, resumeData: info)
            }
            //: } else {
        } else { // 构建音视频通话
            //: self.tryToPushTalkRoom(roomVC: vc!, isVideoTalk: (type == 0 ? true : false), resumeTalk: resumeTalk, resumeData: info)
    ...
    private func isMessage(roomVC: UIViewController, isVideoTalk: Bool, resumeTalk: Bool, resumeData: [String: Any]) {
        //: guard var currentVc = currentViewController() else { return }
        guard var currentVc = announcement() else { return }
        ...
        if currentVc.isKind(of: UINavigationController.self) {
            ...
            //: let rootVc = PicMacroDefine.getWindow().rootViewController as! UITabBarController
            let rootVc = PicMacroDefine.statusDown().rootViewController as! UITabBarController
        ...
        if !isVideoTalk {
            ...
            talkClass = NSStringFromClass(SeedViewController.self)
            //: } else {
        } else {
            //: talkClass = NSStringFromClass(TalkingVideoChatViewController.self)
            talkClass = NSStringFromClass(ComponentErrorDelegate.self)
        }
        //: var chatIndex = -1
        var chatIndex = -1
        //: var index = -1
        ...
        if isVideoTalk {
            ...
            var currentChatVc: ComponentErrorDelegate?
            //: let pushChatVc = roomVC as! TalkingVideoChatViewController
            let pushChatVc = roomVC as! ComponentErrorDelegate

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVideoChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? ComponentErrorDelegate
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    ...
                    currentChatVc?.nextAggregation(resumeData: resumeData)
                }
                //: return
                return
            }
            ...
            if navVc!.isKind(of: ModelNavigationController.self) {
                //: let nav = navVc as! TalkingNavigationController
                let nav = navVc as! ModelNavigationController
            ...
            if topVc != nil, topVc!.isKind(of: SkipViewController.self) {
            ...
            var currentChatVc: SeedViewController?
            //: let pushChatVc = roomVC as! TalkingVoiceChatViewController
            let pushChatVc = roomVC as! SeedViewController

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVoiceChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? SeedViewController
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    //: currentChatVc?.updateInfoViaResumeTalkData(resumeData: resumeData)
                    currentChatVc?.dataNameEvent(resumeData: resumeData)
                }
                //: return
                return
            }
