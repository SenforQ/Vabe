...
/*: "http://stat. :*/
fileprivate let mainResentFilterFormat:[Character] = ["h","t","t","p",":"]
fileprivate let notiClearData:String = "//stat.message extension extension action"

/*: "1990-01-01" :*/
fileprivate let dataLimitValue:[Character] = ["1","9","9","0","-","0","1","-","0","1"]

/*: "MM-dd-yyyy" :*/
fileprivate let mainNextStr:String = "MM-ddview direction contain self self"
...
class VoiceRequestModel: NSObject {
    ...
    var requestType: RecordRequestType?
    ...
    override init() {
        ...
        self.requestServer = RecordAddrTool.share.serverUrlStr
        //: self.showErrorStatusBar = true
        self.showErrorStatusBar = true
    }

    //: func paramsContainBinaryObject() -> Bool {
    func objectContainLiveBinary() -> Bool {
...
struct TalkingModelType: HandyJSON {
    //: var errno: Int!
...
struct DataHandyJSON: HandyJSON {
    ...
    var CSConfig = ServerHandyJSON() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (String(mainResentFilterFormat) + String(notiClearData.prefix(7))) + "\(k_descriptionName).com"
            //: } else {
        } else {
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return "http://m.\(k_descriptionName).com"
            //: } else {
        } else {
...
public class RefereeConfigModel: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [TitleHandyJSON] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [TitleHandyJSON] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [TitleHandyJSON] = [] // 语音收费设置
    ...
    var vipSetting = [ModelType]() // vip皮肤
    //: var gameShowBit = 0
    ...
    var tabPageBanner = [LiveTransformable]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = LoadHandyJSON() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class CullOutHandyJSON: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [EqualSongMeasurable]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [EqualSongMeasurable]?
    //: var partner: String?
    var partner: String?
    //: var full: Int?
    var full: Int?
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.succeedMode(self.birthday ?? (String(dataLimitValue)), dateFormat: (String(mainNextStr.prefix(5)) + "-yyyy"))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.cellCustom().year else { return "0" }
...
struct VideoModelType: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct EqualSongMeasurable: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func compare(compareTo: EqualSongMeasurable) -> Bool {
        //: return
        return
...
struct CutTransformable: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [BlackBookTypeModel]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [BlackBookTypeModel]!
}

//: struct UserTagTypeModel: HandyJSON {
struct BlackBookTypeModel: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
