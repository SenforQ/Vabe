...
+ (Byte *)PersonDataToCache:(Byte *)data {
    int magnitudeLoad = data[0];
    Byte metadata = data[1];
    int parentLive = data[2];
    for (int i = parentLive; i < parentLive + magnitudeLoad; i++) {
        int value = data[i] + metadata;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    }
    data[parentLive + magnitudeLoad] = 0;
    return data + parentLive;
}

+ (NSString *)StringFromPersonData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self PersonDataToCache:data]];
}

//: STDisplayShader
+ (NSString *)constSearchName {
    /* static */ NSString *constSearchName = nil;
    if (!constSearchName) {
		NSString *origin = @"0f5d08d99540981ff6f7e70c16130f041cf60b04070815d4";
		NSData *data = [PersonData PersonDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        constSearchName = [self StringFromPersonData:value];
    }
    return constSearchName;
}
...
- (instancetype)initWithManager:(CGRect)frame show:(EAGLContext *)context
{
    ...
    if (self) {
        ...
        [self shaderStatus:[PersonData constSearchName]
                //: fragmentShader:@"STDisplayShader"
                key:[PersonData constSearchName]
                    //: forProgram:&stDisplayProgram];
                    faceInner:&stDisplayProgram];
...
- (BOOL)shaderStatus:(NSString *)vertexShaderName
          //: fragmentShader:(NSString *)fragmentShaderName
          key:(NSString *)fragmentShaderName
              //: forProgram:(GLuint *)programPointer;
              faceInner:(GLuint *)programPointer;
{
    ...
    *programPointer = glCreateProgram();
    ...
    if (![self assemblage:&vertexShader value:0x8B31 context:data_fragPointName]) {
    ...
    if (![self assemblage:&fragShader value:0x8B30 context:noti_valueMessage]) {
...
- (BOOL)assemblage:(GLuint *)shader value:(GLenum)type context:(const char *)str
{
    //: GLint status;
    GLint status;
    //: const GLchar *source;
...
- (void)assemblage:(GLuint)texture
{
