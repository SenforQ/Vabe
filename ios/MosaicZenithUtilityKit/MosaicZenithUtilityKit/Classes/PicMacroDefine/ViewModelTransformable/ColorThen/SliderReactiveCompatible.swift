...
fileprivate let userTitleData:[UInt8] = [0x9b,0x9c,0x9b,0x86,0xda,0x91,0x9d,0x96,0x97,0x80,0xc8,0xdb,0xd2,0x9a,0x93,0x81,0xd2,0x9c,0x9d,0x86,0xd2,0x90,0x97,0x97,0x9c,0xd2,0x9b,0x9f,0x82,0x9e,0x97,0x9f,0x97,0x9c,0x86,0x97,0x96]

				private func pointPath(vertical num: UInt8) -> UInt8 {
					return num ^ 242
				}
...
// __CLOSE_PRINT__
//
//  SliderReactiveCompatible.swift
//  AbroadTalking
//
...
typealias SlidingHandler = (_ direction: DisappearSlideDirection) -> Void
...
class SliderReactiveCompatible: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (app_tablePath - self.contentView.contentOffset.x)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userTitleData.map{pointPath(vertical: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.factorStyle(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: ColorThen = {
        ...
        let overlay = ColorThen(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension SliderReactiveCompatible {
    /// 重置位置
    //: func resetPostion() {
    func upPostion() {
    ...
    private func factorStyle(point: CGFloat) -> UIImage {
...
extension SliderReactiveCompatible: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = app_tablePath - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.notAdd(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.notAdd(isShow: true)
    }
...
extension SliderReactiveCompatible {
    ...
    private func acceptSubviews() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(app_tablePath)
