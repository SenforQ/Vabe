...
fileprivate let user_actionEqualValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "FFD855" :*/
fileprivate let notiBackgroundData:String = "bag"
fileprivate let constViewStr:[Character] = ["F","D","8","5","5"]
...
class ColorThen: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - data_noProgressValue
                //: self.rightSlider.frame = CGRect(x: minX,
                self.rightSlider.frame = CGRect(x: minX,
                                                //: y: self.rightSlider.frame.origin.y,
                                                y: self.rightSlider.frame.origin.y,
                                                //: width: kSliderWidth,
                                                width: data_noProgressValue,
                ...
                self.slider(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_actionEqualValue.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (notiBackgroundData.replacingOccurrences(of: "bag", with: "F") + String(constViewStr)))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: app_tablePath, y: 0, width: data_noProgressValue, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - data_noProgressValue, y: 0, width: data_noProgressValue, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.underReadingRgba(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: app_tablePath, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.underReadingRgba(0, 0, 0, 0.6)
...
extension ColorThen {
    ...
    func withBar() {
        ...
        self.leftSilder.frame = CGRect(x: app_tablePath, y: 0, width: data_noProgressValue, height: self.frame.size.height)
        //: self.rightSlider.frame = CGRect(x: self.frame.size.width - kSliderWidth, y: 0, width: kSliderWidth, height: self.frame.size.height)
        self.rightSlider.frame = CGRect(x: self.frame.size.width - data_noProgressValue, y: 0, width: data_noProgressValue, height: self.frame.size.height)
        //: resetIndicator()
        visualPropertyIndicator()
    }

    //: func needShowIndicator(isShow: Bool) {
    func notAdd(isShow: Bool) {
        //: self.indicatorView.isHidden = !isShow
        self.indicatorView.isHidden = !isShow
    }

    //: func refreshSliderValidScope(startX: CGFloat) {
    func slider(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - data_noProgressValue,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - data_noProgressValue)
    }
}
...
extension ColorThen {
    ...
    @objc func breakUpGesture(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.slider(startX: videoStartX)
    ...
    @objc func betweenColor(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension ColorThen {
    ...
    private func hustle() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(breakUpGesture(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(betweenColor(gesture:)))
    ...
    private func visualPropertyIndicator() {
    ...
    func secondXposition(_ xposition: CGFloat) {
