...
protocol RecordNavigationDelegate: AnyObject {
...
class PositionViewController: UIViewController {
    ...
    weak var delegate: RecordNavigationDelegate?
    ...
    private lazy var controllerScrollView: UIScrollView = {
        ...
        let count = min(self.controllerItems.count, self.titleItems.count)
        //: var height = PicMacroDefine.getScreenHeight() - (topDistance + kTitle_H)
        var height = PicMacroDefine.rowHeight() - (topDistance + kTitle_H)
        //: if self.hasBttomTabBar {
        if self.hasBttomTabBar {
            //: height -= TabBarViewHeight
            height -= mainHomeTitle
        }
        ...
        //: let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: PicMacroDefine.getScreenWidth(), height: height)
        let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: PicMacroDefine.sharedObject(), height: height)
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        //: scrollView.contentSize = CGSize(width: count * Int(PicMacroDefine.getScreenWidth()), height: 0)
        scrollView.contentSize = CGSize(width: count * Int(PicMacroDefine.sharedObject()), height: 0)
        ...
        scrollView.semanticContentAttribute = SizeLanguageManager.shared.direction == .leftToRight ? .forceLeftToRight:.forceRightToLeft
        //: return scrollView
        return scrollView
    //: }()
    }()
    ...
    private lazy var titleScrollView: UIScrollView = {
        //: var rect = CGRect(x: 15, y: topDistance, width: PicMacroDefine.getScreenWidth()-rightSpace, height: kTitle_H)
        var rect = CGRect(x: 15, y: topDistance, width: PicMacroDefine.sharedObject()-rightSpace, height: kTitle_H)
        //: if isBisect {
        if isBisect {
            //: rect = CGRect(x: 0, y: topDistance, width: PicMacroDefine.getScreenWidth()-rightSpace, height: kTitle_H)
            rect = CGRect(x: 0, y: topDistance, width: PicMacroDefine.sharedObject()-rightSpace, height: kTitle_H)
        }
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        ...
        var originX: CGFloat = SizeLanguageManager.shared.direction == .leftToRight ? 0:25
        ...
        self.titleItems.enumerated().forEach { [weak self] index, obj in
            ...
            titleBtn.addTarget(self, action: #selector(listCommit), for: .touchDown)
            //: var titleWidth = 0.0
            var titleWidth = 0.0
            //: if isBisect {
            if isBisect {
                //: titleWidth = PicMacroDefine.getScreenWidth() / Double(self.titleItems.count)
                titleWidth = PicMacroDefine.sharedObject() / Double(self.titleItems.count)
            //: } else {
            } else {
                //: titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
                titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
            }
            //: let x = LanguageManager.shared.direction == .leftToRight ? originX:(PicMacroDefine.getScreenWidth()-originX-titleWidth)
            let x = SizeLanguageManager.shared.direction == .leftToRight ? originX:(PicMacroDefine.sharedObject()-originX-titleWidth)
...
extension PositionViewController {
    ...
    @objc private func listCommit(_ sender: UIButton) {
        //: currentSelectedTitleButton(sender)
        cellUser(sender)
        //: let index = sender.tag - 1000
        let index = sender.tag - 1000
        //: let offset_x = CGFloat(index) * PicMacroDefine.getScreenWidth()
        let offset_x = CGFloat(index) * PicMacroDefine.sharedObject()
        //: controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        //: addControllerView(withIndex: index)
        titledrop(withIndex: index)
        //: seletedViewController(withIndex: index)
        tickIndex(withIndex: index)
    }
    
    //: private func addControllerView(withIndex index: Int) {
    private func titledrop(withIndex index: Int) {
        //: if index >= children.count {
        if index >= children.count {
            //: return
            return
        }
        //: let origin_x = CGFloat(index) * PicMacroDefine.getScreenWidth()
        let origin_x = CGFloat(index) * PicMacroDefine.sharedObject()
        ...
        //: vc.view.frame = CGRect(x: origin_x, y: 0, width: PicMacroDefine.getScreenWidth(), height: controllerScrollView.frame.size.height)
        vc.view.frame = CGRect(x: origin_x, y: 0, width: PicMacroDefine.sharedObject(), height: controllerScrollView.frame.size.height)
        //: controllerScrollView.addSubview(vc.view)
        controllerScrollView.addSubview(vc.view)
    }
    
    //: private func currentSelectedTitleButton(_ button: UIButton) {
    private func cellUser(_ button: UIButton) {
    ...
    func imageCenter(_ sender: UIButton) {
        //: if titleScrollView.contentSize.width <= PicMacroDefine.getScreenWidth() {
        if titleScrollView.contentSize.width <= PicMacroDefine.sharedObject() {
            //: return
            return
        }
        //: var offset_x = sender.center.x - PicMacroDefine.getScreenWidth() * 0.5
        var offset_x = sender.center.x - PicMacroDefine.sharedObject() * 0.5
        //: if offset_x < 0 {
        if offset_x < 0 {
            //: offset_x = 0
            offset_x = 0
        }
        //: let maxOffset = titleScrollView.contentSize.width - PicMacroDefine.getScreenWidth()
        let maxOffset = titleScrollView.contentSize.width - PicMacroDefine.sharedObject()
...
extension PositionViewController {
    ...
    func tickIndex(withIndex index: Int) {
        //: guard index < children.count else { return }
        guard index < children.count else { return }
        //: delegate?.selectedViewController(children[index], index: index)
        delegate?.state(children[index], index: index)
    }
}

// MARK: - UIScrollViewDelegate
//: extension QYSlideNavigationViewController: UIScrollViewDelegate {
extension PositionViewController: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: let index = Int(controllerScrollView.contentOffset.x / PicMacroDefine.getScreenWidth())
        let index = Int(controllerScrollView.contentOffset.x / PicMacroDefine.sharedObject())
        //: if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
        if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
            //: currentSelectedTitleButton(selectedBtn)
            cellUser(selectedBtn)
        }
        //: addControllerView(withIndex: index)
        titledrop(withIndex: index)
        //: seletedViewController(withIndex: index)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        //: let index = Int(scrollView.contentOffset.x / PicMacroDefine.getScreenWidth())
        let index = Int(scrollView.contentOffset.x / PicMacroDefine.sharedObject())
        ...
        //: let scaleR = scrollView.contentOffset.x / PicMacroDefine.getScreenWidth() - CGFloat(index)
        let scaleR = scrollView.contentOffset.x / PicMacroDefine.sharedObject() - CGFloat(index)
        //: let scaleL = 1 - scaleR
        let scaleL = 1 - scaleR
