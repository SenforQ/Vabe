...
/*: "Select a date" :*/
fileprivate let const_managerColorValue:[Character] = ["S","e","l","e","c","t"," ","a"," ","d","a","t","e"]
...
protocol VideoObjectProtocol: NSObjectProtocol {
...
class PublisherPickView: HandPickReactiveCompatible, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    override func quote() {
        ...
        titleString = (String(const_managerColorValue)).localized
        ...
        hour(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func hour(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func confirm(button: UIButton) {
        ...
        if let day = selectDay {
            ...
            delegate?.overView(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.overView(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return revoke(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func revoke(year: Int, month: Int) -> Int {
