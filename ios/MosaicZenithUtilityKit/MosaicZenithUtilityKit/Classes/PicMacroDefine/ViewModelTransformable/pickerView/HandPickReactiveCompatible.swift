...
fileprivate let noti_topBagTrackFormat:[UInt8] = [0x17,0x10,0x17,0xa,0x56,0x1d,0x11,0x1a,0x1b,0xc,0x44,0x57,0x5e,0x16,0x1f,0xd,0x5e,0x10,0x11,0xa,0x5e,0x1c,0x1b,0x1b,0x10,0x5e,0x17,0x13,0xe,0x12,0x1b,0x13,0x1b,0x10,0xa,0x1b,0x1a]

				private func textMenu(wrap num: UInt8) -> UInt8 {
					return num ^ 126
				}

/*: "Cancel" :*/
fileprivate let noti_viewValue:[Character] = ["C","a","n","c","e","l"]
...
// __CLOSE_PRINT__
//
//  HandPickReactiveCompatible.swift
//  AbroadTalking
//
...
class HandPickReactiveCompatible: UIView {
    // 背景视图
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_topBagTrackFormat.map{textMenu(wrap: $0)}, encoding: .utf8)!)
    }
    ...
    private func quantityimate() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(brushEnableStart))
        ...
        cancelButton.setTitle((String(noti_viewValue)).localized, for: .normal)
        //: cancelButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        cancelButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        //: cancelButton.setTitleColor(UIColor.black, for: .normal)
        cancelButton.setTitleColor(UIColor.black, for: .normal)
        //: cancelButton.addTarget(self, action: #selector(cancelClick(button:)), for: .touchUpInside)
        cancelButton.addTarget(self, action: #selector(clearSize(button:)), for: .touchUpInside)
    ...
    @objc func clearSize(button _: UIButton) {
        //: dismissView()
        brushEnableStart()
    }

    //: @objc func confirmClick(button: UIButton) {
    @objc func confirm(button _: UIButton) {
        //: dismissView()
        brushEnableStart()
    }
