...
fileprivate let appShouldData:String = "btn_saspect new image location"
fileprivate let notiSizeName:String = "icon type pathertBac"
fileprivate let user_withPath:String = "kgrcommunicationnd"

/*: "btn_alertBackground_false" :*/
fileprivate let user_makeValue:[Character] = ["b","t","n","_","a","l","e","r","t","B","a"]
fileprivate let showDirectionText:[Character] = ["c","k","g","r","o","u","n","d","_"]
fileprivate let data_blockUrl:[Character] = ["f","a","l","s","e"]
...
public class AlertThen: NSObject {
    /// 通用回调
...
public extension AlertThen {
    /// 适配器回调,用于给适配器参数赋值
    //: public typealias ConfigAlert = ((_ config: ShowAlertConfig) -> Void)
    typealias ConfigAlert = (_ config: PlayerAlertConfig) -> Void
    ...
    internal class func implement(title: String? = nil,
                                  //: message: String?  = nil,
                                  message: String? = nil,
                                  ...
                                  config: PlayerAlertConfig? = nil)
    {
        //: customAlert(title: title,
        upForQuantity(title: title,
                      //: message: message,
                      message: message,
    ...
    @objc internal class func upRecommend(title: String? = nil,
    ...
    class func upForQuantity(title: String? = nil,
                             ...
                             config: PlayerAlertConfig? = nil)
    {
        ...
        let model = config ?? PlayerAlertConfig()

        //: let alertView = TalkingAlertView(title: title,
        let alertView = SizeScriptAlertView(title: title,
        ...
        //: PicMacroDefine.getWindow().addSubview(alertView)
        PicMacroDefine.statusDown().addSubview(alertView)
    ...
    class func statusElement() {
        //: PicMacroDefine.getWindow().subviews.forEach { (view) in
        PicMacroDefine.statusDown().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: SizeScriptAlertView.self) {
    ...
    class func endOff(title: String, isTouch: Bool) {
        //: PicMacroDefine.getWindow().subviews.forEach { (view) in
        PicMacroDefine.statusDown().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: SizeScriptAlertView.self) {
                //: let cunrenView: TalkingAlertView = view as! TalkingAlertView
                let cunrenView: SizeScriptAlertView = view as! SizeScriptAlertView
                //: cunrenView.changeRightBtn(title: title, isTouch: isTouch)
                cunrenView.buttonTouch(title: title, isTouch: isTouch)
            }
        }
    }
}
...
public class PlayerAlertConfig {
    ...
    public var maskType: MediaMaskType = .color
    /// 阴影
    ...
    public var titleFont: UIFont = .font(type: .Medium, fontSize: 18)
    /// alert标题字体颜色
    //: public var titleColor  = UIColor.appTitleColor()
    public var titleColor = UIColor.equalTarget()
    /// alert信息字体
    //: public var textFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var textFont: UIFont = .font(type: .Medium, fontSize: 16)
    //: public var textColor  = UIColor.appTitleColor()
    public var textColor = UIColor.equalTarget()
    ...
    public var buttonFont: UIFont = .font(type: .Medium, fontSize: 16)
    /// alert按钮字体颜色
    //: public var leftColor  = UIColor.appThemeColor()
    public var leftColor = UIColor.titleDevice()
    //: public var rightColor = UIColor.white
    public var rightColor = UIColor.white
    //: public var rightBackgroundImage = "btn_show_alertBackground"
    public var rightBackgroundImage = (String(appShouldData.prefix(5)) + "how_al" + String(notiSizeName.suffix(6)) + user_withPath.replacingOccurrences(of: "communication", with: "ou"))

    //: public var rightBackgroundImagefalse = "btn_alertBackground_false"
    public var rightBackgroundImagefalse = (String(user_makeValue) + String(showDirectionText) + String(data_blockUrl))
    //: public var rightFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var rightFont: UIFont = .font(type: .Medium, fontSize: 16)
