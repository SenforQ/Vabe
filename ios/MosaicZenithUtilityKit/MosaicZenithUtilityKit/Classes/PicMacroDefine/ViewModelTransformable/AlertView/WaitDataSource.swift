...
/*: "cellID" :*/
fileprivate let k_toFormat:String = "item clickcellID"

/*: "init(coder:) has not been implemented" :*/
fileprivate let appDataViewPath:[UInt8] = [0xf1,0xf6,0xf1,0xec,0xb0,0xfb,0xf7,0xfc,0xfd,0xea,0xa2,0xb1,0xb8,0xf0,0xf9,0xeb,0xb8,0xf6,0xf7,0xec,0xb8,0xfa,0xfd,0xfd,0xf6,0xb8,0xf1,0xf5,0xe8,0xf4,0xfd,0xf5,0xfd,0xf6,0xec,0xfd,0xfc]

				private func addCurrent(make num: UInt8) -> UInt8 {
					return num ^ 152
				}

/*: "Video Call" :*/
fileprivate let app_parentFirstVisualUrl:[Character] = ["V","i","d","e","o"," ","C","a","l","l"]

/*: "Video Call (%@ Points/Min)" :*/
fileprivate let main_partyData:String = "regular selectVideo "
fileprivate let k_indicatorValue:String = "%@ Poself video intimate view"
fileprivate let main_topPath:String = "self for intervalin)"

/*: "Voice Call" :*/
fileprivate let constApplicationErrorId:[Character] = ["V","o","i","c","e"," ","C"]
fileprivate let data_countStr:String = "agiftgift"

/*: "Voice Call (%@ Points/Min)" :*/
fileprivate let showShareStr:String = "target re in letVoice "
fileprivate let userFromStr:String = "%@ Pointleft equal"
fileprivate let data_enableErrorStr:String = "s/Min)title bar"

/*: "#F8F8F8" :*/
fileprivate let app_viewDataUrl:[Character] = ["#"]
fileprivate let const_unknownText:[Character] = ["F","8","F","8","F","8"]

/*: "Cancel" :*/
fileprivate let kMakeTopPlayTitle:[Character] = ["C","a","n","c","e","l"]

/*: "#F5F5F5" :*/
fileprivate let const_buttonMessage:String = "#F5F5F5state event string to"

/*: "icon_lounge" :*/
fileprivate let dataTotalPath:[Character] = ["i","c","o","n","_","l","o","u","n","g","e"]
...
class WaitDataSource: UIView {
    //: var munuBlock: MunuBlock?
    var munuBlock: MunuBlock?

    //: private var type: MunuPopType = .Unknown
    private var type: ResumePopType = .Unknown // 当前展示类型
    ...
    var cellTitleColor = UIColor.equalTarget()
    /// cell字体大小
    //: var cellTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cellTitleFont = UIFont.font(type: .Regular, fontSize: 16)
    /// 是否有"Cancel"菜单
    //: var hasCancel = true
    var hasCancel = true
    /// cancel字体颜色
    //: var cancelColor = UIColor.appValueDetailColor()
    var cancelColor = UIColor.aftColor()
    /// cancel字体大小
    //: var cancelTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cancelTitleFont = UIFont.font(type: .Regular, fontSize: 16)
    /// 标题
    //: var title: String?
    var title: String?
    /// 标题颜色
    //: var titleColor = UIColor.appThemeColor()
    var titleColor = UIColor.titleDevice()
    /// 标题字体大小
    //: var titleFont = UIFont.pingfangMediumFont(fontSize: 16)
    var titleFont = UIFont.dismissSharedSize(fontSize: 16)

    //: var tableView = UITableView()
    var tableView = UITableView()
    //: let cellID = "cellID"
    let cellID = (String(k_toFormat.suffix(6)))
    ...
    fileprivate var footerViewHight: CGFloat {
        ...
        return hasCancel ? (footerViewLineHight + 50 + dataShowText) : dataShowText
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appDataViewPath.map{addCurrent(make: $0)}, encoding: .utf8)!)
    }

    //: func initwithList(cellTitleList: [String]!) {
    func zapTime(cellTitleList: [String]!) {
        // 初始化
    ...
    func counteractionDifferentialCost(type: ResumePopType, videoPrice: String?, voicePrice: String?) {
        //: self.type = type
        self.type = type
        //: if self.type == .Call {
        if self.type == .Call { // 音视频通话
            ...
            if ExamineedReactiveCompatible.share.appStatus == InstanceHashableRepresentation.normal.rawValue {
                ...
                if ExamineedReactiveCompatible.share.loginUserMode.sex == CommentRefCompatibleValue.male.rawValue {
                    ...
                    let videoAttrStr = String.bestowal(VideoPrice: Int(videoPrice ?? "0") ?? 0)
                    //: let voiceAttrStr = String.setVoicePriceStr(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    let voiceAttrStr = String.starEffect(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    ...
                    let attributes = [.foregroundColor: UIColor.titleDevice(), .font: UIFont.font(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
                    //: let VideoCall = "Video Call".localized
                    let VideoCall = (String(app_parentFirstVisualUrl)).localized
                    //: let videoStr = "Video Call (%@ Points/Min)".localizedArguments(videoPrice ?? "0")
                    let videoStr = (String(main_partyData.suffix(6)) + "Call (" + String(k_indicatorValue.prefix(5)) + "ints/M" + String(main_topPath.suffix(3))).equalKey(videoPrice ?? "0")
                    ...
                    let VoiceCall = (String(constApplicationErrorId) + data_countStr.replacingOccurrences(of: "gift", with: "l")).localized
                    //: let voiceStr = "Voice Call (%@ Points/Min)".localizedArguments(voicePrice ?? "0")
                    let voiceStr = (String(showShareStr.suffix(6)) + "Call (" + String(userFromStr.prefix(8)) + String(data_enableErrorStr.prefix(6))).equalKey(voicePrice ?? "0")
                ...
                self.cellTitleList = [(String(app_parentFirstVisualUrl)).localized, (String(constApplicationErrorId) + data_countStr.replacingOccurrences(of: "gift", with: "l")).localized]
            }
        }
    ...
    public func errorDown() {
        ...
        tableView = UITableView(frame: CGRect(x: 0, y: show_licenseData - tableViewHight, width: user_barText, height: tableViewHight), style: .plain)
        ...
        tableView.register(FileView.self, forCellReuseIdentifier: cellID)
        ...
        if title == nil {
            ...
            tableView.areaRecordRadii(width: user_barText,
...
extension WaitDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var cell = tableView.dequeueReusableCell(withIdentifier: cellID) as? FileView
        //: if cell == nil {
        if cell == nil {
            //: cell = TitleCell(style: .default, reuseIdentifier: cellID)
            cell = FileView(style: .default, reuseIdentifier: cellID)
        }

        //: if ExamineedReactiveCompatible.share.loginUserMode.sex == Gender.male.rawValue,
        if ExamineedReactiveCompatible.share.loginUserMode.sex == CommentRefCompatibleValue.male.rawValue,
           //: ExamineedReactiveCompatible.share.loginUserMode.loungePlus == true,
           ExamineedReactiveCompatible.share.loginUserMode.loungePlus == true,
           //: type == .Call {
           type == .Call
        {
        ...
        cell!.titleLab.font = cellTitleFont
        //: if self.type == .Call && ExamineedReactiveCompatible.share.appStatus == AppSkinStatus.normal.rawValue {
        if self.type == .Call && ExamineedReactiveCompatible.share.appStatus == InstanceHashableRepresentation.normal.rawValue {
    ...
    func tableView(_: UITableView, viewForFooterInSection _: Int) -> UIView? {
        ...
        let footerView = UIView(frame: CGRect(x: 0, y: 0, width: user_barText, height: footerViewHight))
        //: let lineView = UIView(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: footerViewLineHight))
        let lineView = UIView(frame: CGRect(x: 0, y: 0, width: user_barText, height: footerViewLineHight))
        //: lineView.backgroundColor = UIColor(hex: "#F8F8F8")
        lineView.backgroundColor = UIColor(hex: (String(app_viewDataUrl) + String(const_unknownText)))
        ...
        btn.frame = CGRect(x: 0, y: footerViewLineHight, width: user_barText, height: 50)
        //: footerView.addSubview(btn)
        footerView.addSubview(btn)
        //: btn.setTitle("Cancel".localized, for: .normal)
        btn.setTitle((String(kMakeTopPlayTitle)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(clickSize), for: .touchUpInside)
        //: return footerView
        return footerView
    }
    ...
    func tableView(_: UITableView, viewForHeaderInSection _: Int) -> UIView? {
        ...
        let headerView = UIView(frame: CGRect(x: 0, y: 0, width: user_barText, height: headerViewHight))

        //: let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: ScreenWidth-30, height: headerViewHight))
        let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: user_barText - 30, height: headerViewHight))
        ...
        let lineView = UIView(frame: CGRect(x: 0, y: headerViewHight - 1, width: user_barText, height: 1))
        //: lineView.backgroundColor = UIColor.init(hex: "#F5F5F5")
        lineView.backgroundColor = UIColor(hex: (String(const_buttonMessage.prefix(7))))
...
class FileView: UITableViewCell {
    ...
    lazy var vipIcon: UIImageView = {
        ...
        img.image = UIImage.snapIcon(name: (String(dataTotalPath)))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    lazy var lineLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(const_buttonMessage.prefix(7))))
        //: return lab
        return lab
        //: }()
    }()
