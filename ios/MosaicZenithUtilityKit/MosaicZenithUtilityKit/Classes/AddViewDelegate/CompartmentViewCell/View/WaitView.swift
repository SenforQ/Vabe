...
fileprivate let noti_dataNameServerUrl:[UInt8] = [0xc,0xb,0xc,0x11,0x4d,0x6,0xa,0x1,0x0,0x17,0x5f,0x4c,0x45,0xd,0x4,0x16,0x45,0xb,0xa,0x11,0x45,0x7,0x0,0x0,0xb,0x45,0xc,0x8,0x15,0x9,0x0,0x8,0x0,0xb,0x11,0x0,0x1]
...
enum TapCaseIterable: CaseIterable {
    //: case Left
    case Left
    //: case Right
    case Right
...
protocol SizeCellDelegete: NSObjectProtocol {
    ///
...
class WaitView: UIView {
    ...
    var cell_delegate: SizeCellDelegete?

    //: var model: TalkingMatchResultModel?
    var model: SkipMeasurable?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_dataNameServerUrl.map{$0^101}, encoding: .utf8)!)
    }
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(pushUp(pan:)))
        //: v.delegate = self
        v.delegate = self
        //: return v
        return v
...
extension WaitView: UIGestureRecognizerDelegate {
    ...
    @objc func pushUp(pan: UIPanGestureRecognizer) {
        //: switch pan.state {
        switch pan.state {
        //: case .began:
        case .began:
        ...
        case .changed:
            ...
            if SizeLanguageManager.shared.direction == .rightToLeft {
                ...
                if movePoint.x > user_barText / 4 {
                    ...
                    self.cell_delegate?.tagGesture(cell: self, forMovePoint: self.currentPoint)
    ...
    func standSuperview() {
        ...
        self.cell_delegate?.sexTo(cell: self)
    }
    ...
    func attending(direction: TapCaseIterable) {
        //: switch (direction) {
        switch direction {
        //: case .Left:
        case .Left:
            //: removeFromSuperviewLeft()
            populationProfileBelow()
        //: case .Right:
        case .Right:
