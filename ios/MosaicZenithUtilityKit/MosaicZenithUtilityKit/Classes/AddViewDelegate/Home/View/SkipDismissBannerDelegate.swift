...
fileprivate let noti_phonePath:[UInt8] = [0xa2,0xa5,0xa2,0xbf,0xe3,0xa8,0xa4,0xaf,0xae,0xb9,0xf1,0xe2,0xeb,0xa3,0xaa,0xb8,0xeb,0xa5,0xa4,0xbf,0xeb,0xa9,0xae,0xae,0xa5,0xeb,0xa2,0xa6,0xbb,0xa7,0xae,0xa6,0xae,0xa5,0xbf,0xae,0xaf]

/*: "JXBannerCellID" :*/
fileprivate let notiGiftKey:[Character] = ["J","X","B","a","n","n","e","r","C","e","l","l","I","D"]

/*: "#FFFFFF" :*/
fileprivate let userJumpKitGiftName:[Character] = ["#","F","F","F","F","F"]
fileprivate let main_whiteValue:String = "image"

/*: "#BDB8B5" :*/
fileprivate let show_groupName:String = "#BDB8B5return selected color"
...
// __CLOSE_PRINT__
//
//  SkipDismissBannerDelegate.swift
//  AbroadTalking
//
...
protocol GroupObjectProtocol: NSObjectProtocol {
    // 加载更多数据
...
class SkipDismissBannerDelegate: UICollectionReusableView {
    //: weak var delegate: SocialPopularListFooterViewProtocol?
    weak var delegate: GroupObjectProtocol?
    //: var listData: [PremiumStarListModel]?
    var listData: [StreetwiseModelType]? // 巨星计划数据
    //: var adBannderListData: [SocialAdBannerModel]?
    var adBannderListData: [LiveTransformable]? // 广告banner数据

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: noti_phonePath.map{$0^203}, encoding: .utf8)!)
    }
...
extension SkipDismissBannerDelegate {
    /// 获取当前视图高度
    /// - Returns: 高度
    ...
    func refreshErrorInformationPlay(data: [StreetwiseModelType]) {
        //: self.listData = data
        self.listData = data
    }
    ...
    func nowDataEvent(data: [LiveTransformable]) {
...
extension SkipDismissBannerDelegate: JXBannerDataSource, JXBannerDelegate {
    //: func jxBanner(_ banner: JXBannerType) -> (JXBannerCellRegister) {
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        //: return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: "JXBannerCellID")
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (String(notiGiftKey)))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        tempCell.imageView.applicatorEqual(urlStr: model?.pic ?? "", placeImg: UIImage.anArti())
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(userJumpKitGiftName) + main_whiteValue.replacingOccurrences(of: "image", with: "F")))!
        //: pageControl.inactiveSize = CGSize(width: 6, height: 6)
        pageControl.inactiveSize = CGSize(width: 6, height: 6)
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(show_groupName.prefix(7))))!
...
extension SkipDismissBannerDelegate {
    ...
    private func telephoneExtension() {
    ...
    private func poetize() {
        ...
        bannerView.snp.makeConstraints { make in
            ...
            make.size.equalTo(CGSize(width: user_barText - 30, height: 84))
        }
    }
}
