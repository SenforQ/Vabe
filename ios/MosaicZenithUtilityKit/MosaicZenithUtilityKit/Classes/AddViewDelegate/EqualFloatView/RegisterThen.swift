...
/*: "position.x" :*/
fileprivate let showMakeKey:String = "podetailiti"

/*: "slideIn" :*/
fileprivate let noti_insideData:[Character] = ["s","l","i","d","e","I","n"]

/*: "slideOut" :*/
fileprivate let k_requestMessage:[Character] = ["s","l","i"]
fileprivate let mainActionPath:String = "deOutshow session center"
...
class RegisterThen: NSObject {
    //: static let shared = GiftFloatViewManager()
    static let shared = RegisterThen()
    //: private var giftArr = [SocialBroadcastModel]()
    private var giftArr = [MilkModelType]()
    ...
    private lazy var giftView: EqualFloatView = {
        //: let v = GiftFloatView()
        let v = EqualFloatView()
        //: PicMacroDefine.getWindow().addSubview(v)
        PicMacroDefine.statusDown().addSubview(v)
        //: return v
        return v
        //: }()
    }()
...
extension RegisterThen {
    ...
    private func listSize(_ model: MilkModelType) {
        ...
        let isRTL = SizeLanguageManager.shared.direction == .rightToLeft
        // 起始位置：屏幕外
        //: let startCenterX = isRTL ? -centerX : ScreenWidth + centerX
        let startCenterX = isRTL ? -centerX : user_barText + centerX
        // 目标位置：屏幕中间
        //: let targetCenterX = ScreenWidth / 2
        let targetCenterX = user_barText / 2
        // 结束位置：屏幕另一侧
        //: let endCenterX = isRTL ? ScreenWidth + centerX : -centerX
        let endCenterX = isRTL ? user_barText + centerX : -centerX
        ...
        let slideIn = CABasicAnimation(keyPath: (showMakeKey.replacingOccurrences(of: "detail", with: "s") + "on.x"))
        ...
        self.giftView.layer.add(slideIn, forKey: (String(noti_insideData)))
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) { [weak self] in
            ...
            let slideOut = CABasicAnimation(keyPath: (showMakeKey.replacingOccurrences(of: "detail", with: "s") + "on.x"))
            ...
            self.giftView.layer.add(slideOut, forKey: (String(k_requestMessage) + String(mainActionPath.prefix(5))))
...
extension RegisterThen {
    ...
    func combineTo() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.holdFast(noti:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.everyLoad),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.exaggerateLoginout),
    ...
    @objc private func holdFast(noti: Notification) {
        ...
        guard let data = noti.object,
              ...
              let gift = JSONDeserializer<MilkModelType>.deserializeFrom(dict: data as? [String: Any], designatedPath: nil),
              //: gift.broadType != BroadcastType.Unknow.rawValue
              gift.broadType != NewscastDefaultsSerializable.Unknow.rawValue
        //: else {
        else {
            //: return
            return
        }
        //: let currUserType = ExamineedReactiveCompatible.share.appUserConfigMode.userType
        let currUserType = ExamineedReactiveCompatible.share.appUserConfigMode.userType
        // 过滤接口异常
        ...
        // 非特殊用户 不展示 不同地区用户礼物
        //: if currUserType != 0, gift.userCountryType != ExamineedReactiveCompatible.share.appUserConfigMode.userCountryType {
        if currUserType != 0, gift.userCountryType != ExamineedReactiveCompatible.share.appUserConfigMode.userCountryType {
            //: return
            return
        }
    ...
    @objc private func exaggerateLoginout() {
