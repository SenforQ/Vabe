...
/*: "Search" :*/
fileprivate let k_deviceStr:String = "Searchcolor view fill"

/*: "#F5F5F5" :*/
fileprivate let appSumeractionKey:String = "follow type#F5F5F5"

/*: "Search by UID" :*/
fileprivate let main_listStr:String = "Searcdevice edit observer"
fileprivate let k_frameStr:[Character] = ["h"]
fileprivate let show_titleBackFeeFormat:String = "cell self as for mode by UID"

/*: "icon_search_sea" :*/
fileprivate let app_viewPath:String = "icon_shared not transform to"
fileprivate let showSumMessage:String = "SEARCH"

/*: "Can’t find~" :*/
fileprivate let const_normalPathData:String = "return status view beauty receiveCan’"
fileprivate let showSectionPath:String = "cap var model stackt find~"

/*: "icon_kong_kong_default" :*/
fileprivate let data_giftId:String = "equal self self heighticon_"
fileprivate let app_nowBottomExtensionStr:String = "core name choice_kong"

/*: "Please enter the query UID" :*/
fileprivate let data_nameMsg:[Character] = ["P","l","e","a","s","e"," ","e","n","t","e","r"," ","t","h","e"," ","q","u"]
fileprivate let user_cosPathId:String = "ery UIDself mixture"
...
class WaitReactiveCompatible: ReloadViewController {
    ...
    private lazy var searchBtn: UIButton = {
        ...
        btn.setTitle((String(k_deviceStr.prefix(6))).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.resourceIn(colors: UIColor.appSub(), size: CGSize(width: 72, height: 32)), for: .normal)
        //: btn.layer.cornerRadius = 16
        btn.layer.cornerRadius = 16
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(searchButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(timeView), for: .touchUpInside)
        //: btn.alpha = 0.5
        btn.alpha = 0.5
    ...
    private lazy var searchTF: UITextField = {
        ...
        tf.placeholder = (String(main_listStr.prefix(5)) + String(k_frameStr) + String(show_titleBackFeeFormat.suffix(7))).localized
    ...
    private lazy var unlessEditingView: UIView = {
        ...
        let image = UIImage.snapIcon(name: (String(app_viewPath.prefix(5)) + showSumMessage.lowercased() + "_sea"))
    ...
    private lazy var tableView: UITableView = {
        ...
        tabV.register(PublisherItemCell.self, forCellReuseIdentifier: PublisherItemCell.className())
    ...
    private lazy var emptyV: EmptyView = {
        ...
        style.TipsTitle = (String(const_normalPathData.suffix(4)) + String(showSectionPath.suffix(7))).localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (String(data_giftId.suffix(5)) + "kong" + String(app_nowBottomExtensionStr.suffix(5)) + "_default")
        ...
        let v = EmptyView(frame: CGRect(x: 0, y: 14 + dataRecordTitle, width: user_barText, height: show_licenseData - (14 + dataRecordTitle + main_infoId)), style: style)
    ...
    private lazy var reqManager: ServerMessageManagingDirectorThen = //: return ServerMessageManagingDirectorThen()
        .init()
    //: }()
}
...
extension WaitReactiveCompatible {
    // 根据uid查询用户
    ...
    private func outcome() {
        ...
        guard searchTF.text?.count ?? 0 > 0 else {
            ...
            self.heatherMixtureMoment(showMsg: (String(data_nameMsg) + String(user_cosPathId.prefix(7))).localized)
            //: return
            return
        }
        ...
        reqManager.recount(uid: searchTF.text ?? "") { [weak self] _, _, _ in
            //: ProgressHUD.dismiss()
...
extension WaitReactiveCompatible: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: PublisherItemCell.className(), for: indexPath) as! PublisherItemCell
        //: let model = reqManager.searchListDataArr[indexPath.row]
        let model = reqManager.searchListDataArr[indexPath.row]
        //: cell.refreshCell(model: model)
        cell.premium(model: model)
...
extension WaitReactiveCompatible: UITextFieldDelegate {
...
extension WaitReactiveCompatible: JXPagingViewListViewDelegate {
...
extension WaitReactiveCompatible {
    ...
    private func chemistryLabState() {
