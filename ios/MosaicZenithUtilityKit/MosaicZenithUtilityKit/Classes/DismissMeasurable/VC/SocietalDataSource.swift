...
fileprivate let noti_userStr:[UInt8] = [0xd1,0xd6,0xd1,0xcc,0x90,0xdb,0xd7,0xdc,0xdd,0xca,0x82,0x91,0x98,0xd0,0xd9,0xcb,0x98,0xd6,0xd7,0xcc,0x98,0xda,0xdd,0xdd,0xd6,0x98,0xd1,0xd5,0xc8,0xd4,0xdd,0xd5,0xdd,0xd6,0xcc,0xdd,0xdc]

				private func itemVideo(tap num: UInt8) -> UInt8 {
					return num ^ 184
				}

/*: "icon_rank_left" :*/
fileprivate let main_maxPath:[Character] = ["i","c","o","n","_","r","a","n","k","_"]
fileprivate let data_cleanMsg:[Character] = ["l","e","f","t"]

/*: "icon_rank_right" :*/
fileprivate let app_viewUrl:String = "request log let mode makeicon_r"
fileprivate let showSelectedFormat:String = "ikeyht"

/*: "Yesterday" :*/
fileprivate let dataSectionKey:[Character] = ["Y","e","s","t","e","r","d","a","y"]
...
class SocietalDataSource: ReloadViewController {
    // MARK: params

    //: var titles = [String]()
    var titles = [String]()
    //: var rankPeriodModel = SocialPeriodModel()
    var rankPeriodModel = DoingTransformable()
    //: var type = 0
    var type = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_userStr.map{itemVideo(tap: $0)}, encoding: .utf8)!)
    }
    ...
    let leftImgView = UIImageView().then {
        ...
        $0.image = UIImage.snapIcon(name: (String(main_maxPath) + String(data_cleanMsg)))
    }

    //: let rightImgView = UIImageView().then {
    let rightImgView = UIImageView().then {
        //: $0.image = UIImage.BundleImageNamed(name: "icon_rank_right")
        $0.image = UIImage.snapIcon(name: (String(app_viewUrl.suffix(6)) + "ank_r" + showSelectedFormat.replacingOccurrences(of: "key", with: "g")))
    }
...
extension SocietalDataSource {
    ...
    private func resultRemoveBindings() {}
}
...
extension SocietalDataSource {
    ...
    private func sendTo(subTitle: String) -> [RefTransformable] {
        //: if subTitle == "Yesterday".localized {
        if subTitle == (String(dataSectionKey)).localized {
            //: return addEmptyModel(data: rankPeriodModel.yesterday?.res ?? [])
            return isothermal(data: rankPeriodModel.yesterday?.res ?? [])
            //: } else if subTitle == "Today".localized {
        } else if subTitle == "Today".localized {
            //: return addEmptyModel(data: rankPeriodModel.today?.res ?? [])
            return isothermal(data: rankPeriodModel.today?.res ?? [])
            //: } else {
        } else {
            //: return addEmptyModel(data: rankPeriodModel.weekly?.res ?? [])
            return isothermal(data: rankPeriodModel.weekly?.res ?? [])
        }
    }
    ...
    private func isothermal(data: [RefTransformable]) -> [RefTransformable] {
        //: var mData = data
        var mData = data
        //: if mData.count == 0 {
        if mData.count == 0 {
...
extension SocietalDataSource: JXSegmentedListContainerViewListDelegate {
...
extension SocietalDataSource: JXSegmentedListContainerViewDataSource {
...
extension SocietalDataSource {
    ...
    private func outFile() {
        ...
        segmentedDataSource.titleSelectedColor = UIColor.underReadingRgba(207, 72, 255, 1)
        //: segmentedDataSource.titleNormalFont = .pingfangFont(type: .Medium, fontSize: 15)
        segmentedDataSource.titleNormalFont = .font(type: .Medium, fontSize: 15)
        //: segmentedDataSource.titleSelectedFont = .pingfangFont(type: .Medium, fontSize: 15)
        segmentedDataSource.titleSelectedFont = .font(type: .Medium, fontSize: 15)
        ...
        segmentedView.backgroundColor = UIColor.underReadingRgba(212, 131, 255, 1)
        //: segmentedView.defaultSelectedIndex = 1
        segmentedView.defaultSelectedIndex = 1

        //: segmentedView.frame = CGRect(x: (ScreenWidth-270)/2, y: 10, width: totalWidth, height: 36)
        segmentedView.frame = CGRect(x: (user_barText - 270) / 2, y: 10, width: totalWidth, height: 36)
