...
struct DismissHandyJSON: HandyJSON {
    //: var activity: SocialPeriodModel?
    var activity: DoingTransformable?
    //: var charm: SocialPeriodModel?
    var charm: DoingTransformable?
}

//: struct SocialPeriodModel: HandyJSON {
struct DoingTransformable: HandyJSON {
    //: var yesterday: SocialResModel?
    var yesterday: AddResModel?
    //: var today: SocialResModel?
    var today: AddResModel?
    //: var weekly: SocialResModel?
    var weekly: AddResModel?
}

// 毫无意义的一层，等后端后期调整
//: struct SocialResModel: HandyJSON {
struct AddResModel: HandyJSON {
    //: var res: [SocialRankItemModel] = []
    var res: [RefTransformable] = []
}

//: struct SocialRankItemModel: HandyJSON {
struct RefTransformable: HandyJSON {
    //: var uid = 0
    var uid = 0
    //: var value = 0
    var value = 0
