...
fileprivate let appRenderText:[UInt8] = [0x9b,0x9e,0x92,0x90,0xa3,0x98,0x9e,0x9d,0x4f,0x90,0xa4,0xa3,0x97,0x9e,0xa1,0x98,0xa9,0x90,0xa3,0x98,0x9e,0x9d,0x82,0xa3,0x90,0xa3,0xa4,0xa2,0x4f,0x98,0xa2,0x4f]

				fileprivate func presentMessage(equal num: UInt8) -> UInt8 {
...
fileprivate let const_firstTitle:String = "self as featureget lo"
fileprivate let notiNameSmartStr:[Character] = ["n"," ","f","a","i","l","e","d","."," ","e","r","r","o","r",":"]
...
// __CLOSE_PRINT__
//
//  ModelPathManagerDelegate.swift
//  AbroadTalking
//
...
enum StreamAdditiveArithmetic: Int {
    //: case firstEntry = 0
    case firstEntry = 0
...
class ModelPathManagerDelegate: NSObject {
    ...
    var locationType: StreamAdditiveArithmetic?

    // singleton
    //: private static var _instance: TalkingLocationManager?
    private static var _instance: ModelPathManagerDelegate?
    //: class func shared() -> TalkingLocationManager {
    class func showOf() -> ModelPathManagerDelegate {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = TalkingLocationManager()
            _instance = ModelPathManagerDelegate()
            //: return _instance!
            return _instance!
        }
        //: return _instance!
    ...
    func tillFollow(block: @escaping Locationblock) {
    ...
    func titleWhen(block: @escaping Authblock) {
    ...
    func outerSpaceFromNationalService() -> Bool {
    ...
    func locationColorAllClear() -> StreamAdditiveArithmetic {
    ...
    func birthSodom() -> CLAuthorizationStatus {
        ...
        if #available(iOS 14.0, *) {
            ...
            printLog(message: String(bytes: appRenderText.map{presentMessage(equal: $0)}, encoding: .utf8)! + "\(status.rawValue)")
            //: return status
            return status
            //: } else {
        } else {
...
extension ModelPathManagerDelegate: CLLocationManagerDelegate {
...
//            guard abs(loction.timestamp.timeIntervalSinceNow) > 5 else { return }
    ...
    func locationManager(_: CLLocationManager, didFailWithError error: Error) {
        ...
        printLog(message: (String(const_firstTitle.suffix(6)) + "catio" + String(notiNameSmartStr)) + "\(error.localizedDescription)")
    }
}
