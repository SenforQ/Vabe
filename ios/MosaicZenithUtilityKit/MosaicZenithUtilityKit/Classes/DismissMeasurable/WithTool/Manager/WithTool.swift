...
extension WithTool {
    ...
    func afterShared(_ frontCamera: Bool) {
        ...
        self.camerVideo(front: frontCamera)
    }
    
    /// 摄像头开启与关闭
    /// - Parameter openCamera: 是否开启
    //: func useCamera(_ openCamera: Bool) {
    func panCamera(_ openCamera: Bool) {
        ...
        self.openEnable(isOpen: openCamera)
    }
    ...
    func bare() {
        ...
        DispatchQueue.main.async {
            ...
            self.isInfo(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
...
extension WithTool {
    ...
    private func camerVideo(front: Bool) {
    ...
    private func openEnable(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard data_appViewId == true else {
            //: if isOpen {
            if isOpen {
...
extension WithTool {
    ...
    func countermitGroup() {
        // 监听 App 切后台
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(appDidEnterBackground(notif:)),
                                               selector: #selector(notifEnterCommentHeritage(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(analogDigitalConverterWithValue(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(centerTarget(notif:)),
    ...
    @objc private func notifEnterCommentHeritage(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func analogDigitalConverterWithValue(notif: Notification?) {
        //: if notif == nil { 
    ...
    @objc func centerTarget(notif: Notification) {
