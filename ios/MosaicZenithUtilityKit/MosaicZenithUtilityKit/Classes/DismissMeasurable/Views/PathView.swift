...
fileprivate let appViewValue:[UInt8] = [0x81,0x86,0x81,0x9c,0xc0,0x8b,0x87,0x8c,0x8d,0x9a,0xd2,0xc1,0xc8,0x80,0x89,0x9b,0xc8,0x86,0x87,0x9c,0xc8,0x8a,0x8d,0x8d,0x86,0xc8,0x81,0x85,0x98,0x84,0x8d,0x85,0x8d,0x86,0x9c,0x8d,0x8c]

/*: "bg_tx_lb" :*/
fileprivate let showHomePath:[Character] = ["b","g","_","t","x","_","l","b"]

/*: "Edit Title" :*/
fileprivate let show_renderContent:[Character] = ["E","d","i","t"]
fileprivate let mainTempMessage:[Character] = [" ","T","i","t","l","e"]

/*: "Maybe next time" :*/
fileprivate let showSumValue:String = "Maybetender agent array"
fileprivate let userEqualTitle:[Character] = ["t","i","m","e"]
...
class PathView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appViewValue.map{$0^232}, encoding: .utf8)!)
    }
    ...
    lazy var contentView: UIImageView = {
        ...
        iamg.image = UIImage.snapIcon(name: "bg_tx")
    ...
    lazy var topIcon: UIImageView = {
        ...
        iamg.image = UIImage.snapIcon(name: (String(showHomePath)))
    ...
    lazy var titleLabel: UILabel = {
        ...
        lab.text = (String(show_renderContent) + String(mainTempMessage)).localized
    ...
    lazy var inputTView: UITextView = {
        ...
        let textView = UITextView()
        //: textView.text = ExamineedReactiveCompatible.share.appUserConfigMode.videoCover
        textView.text = ExamineedReactiveCompatible.share.appUserConfigMode.videoCover
    ...
    private lazy var finishBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(streamFinish), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setTitle((String(showSumValue.prefix(5)) + " next " + String(userEqualTitle)).localized, for: .normal)
        //: btn.setTitleColor(UIColor.appValueColor(), for: .normal)
        btn.setTitleColor(UIColor.componentColor(), for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 13)
        btn.titleLabel?.font = UIFont.easing(fontSize: 13)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(rubricShare), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension PathView {
    ...
    @objc func streamFinish() {
        //: dismiss()
        showBegin()
        //: guard TalkingSocketManager.shared.isCalling == false else {
        guard LoadSocketDelegate.shared.isCalling == false else {
        ...
        announcement()?.navigationController?.pushViewController(editvc, animated: true)
    }

    //: @objc func closeBtnClick() {
    @objc func rubricShare() {
        //: dismiss()
        showBegin()
    }
