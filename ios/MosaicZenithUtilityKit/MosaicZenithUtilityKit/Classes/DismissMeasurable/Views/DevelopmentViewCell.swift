...
fileprivate let app_objectTakeData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_rank_coin" :*/
fileprivate let dataLastMsg:[UInt8] = [0xb2,0xac,0xb8,0xb7,0xa8,0xbb,0xaa,0xb7,0xb4,0xa8,0xac,0xb8,0xb2,0xb7]

				fileprivate func responseLayer(status num: UInt8) -> UInt8 {
...
/*: "icon_lounge" :*/
fileprivate let const_listKey:[Character] = ["i","c","o","n","_","l","o","u","n"]
fileprivate let showValueTitle:String = "GE"

/*: "icon_rank_coin2" :*/
fileprivate let mainFlushVideoTitle:[Character] = ["i","c","o","n","_","r","a","n","k","_"]
fileprivate let showActionValue:String = "coininside"
...
class DevelopmentViewCell: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_objectTakeData.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var rankLabel = UILabel().then {
        ...
        $0.textColor = UIColor.aftColor()
        //: $0.font = .pingfangFont(type: .Semibold, fontSize: 20)
        $0.font = .font(type: .Semibold, fontSize: 20)
    }
    ...
    lazy var coinIconBtn = GroupVideoControl().then {
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin"), for: .normal)
        $0.setImage(UIImage.snapIcon(name: String(bytes: dataLastMsg.map{responseLayer(status: $0)}, encoding: .utf8)!), for: .normal)
        //: $0.setTitleColor(UIColor.white, for: .normal)
        $0.setTitleColor(UIColor.white, for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Medium, fontSize: 15)
        $0.titleLabel?.font = .font(type: .Medium, fontSize: 15)
        ...
        $0.setBackgroundImage(UIImage.resourceIn(colors: UIColor.appSub(), size: CGSize(width: actualWidth(w: 80), height: actualWidth(w: 33))), for: .normal)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.snapIcon(name: (String(const_listKey) + showValueTitle.lowercased()))
    }
}
...
extension DevelopmentViewCell {
    //: func updateRankCell(model: SocialRankItemModel, index: Int) {
    func random(model: RefTransformable, index: Int) {
        ...
        headPic.inclose(urlStr: model.headPic)
        ...
        if !model.headPicFrame.isEmptyString {
            ...
            iconBorder.showUp(urlStr: model.headPicFrame)
            //: }else {
        } else {
    ...
    func modifyArrayEqual(type: Int) {
        //: let imageStr = type == 0 ? "icon_rank_coin":"icon_rank_coin2"
        let imageStr = type == 0 ? String(bytes: dataLastMsg.map{responseLayer(status: $0)}, encoding: .utf8)! : (String(mainFlushVideoTitle) + showActionValue.replacingOccurrences(of: "inside", with: "2"))
        //: coinIconBtn.setImage(UIImage.BundleImageNamed(name: imageStr), for: .normal)
        coinIconBtn.setImage(UIImage.snapIcon(name: imageStr), for: .normal)
    }
}
