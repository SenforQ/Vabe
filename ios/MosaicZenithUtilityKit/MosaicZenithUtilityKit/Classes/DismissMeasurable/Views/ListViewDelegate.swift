...
fileprivate let constSectionText:[UInt8] = [0xd6,0xd1,0xd6,0xcb,0x97,0xdc,0xd0,0xdb,0xda,0xcd,0x85,0x96,0x9f,0xd7,0xde,0xcc,0x9f,0xd1,0xd0,0xcb,0x9f,0xdd,0xda,0xda,0xd1,0x9f,0xd6,0xd2,0xcf,0xd3,0xda,0xd2,0xda,0xd1,0xcb,0xda,0xdb]

				private func safetyMargin(raw num: UInt8) -> UInt8 {
					return num ^ 191
				}

/*: "Not yet~" :*/
fileprivate let data_equalContent:[Character] = ["N","o","t"," ","y","e","t","~"]

/*: "#8C7AFF" :*/
fileprivate let k_viewName:String = "#8C7Afrom container count"
fileprivate let appDismissActionName:[Character] = ["F","F"]
...
class ListViewDelegate: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constSectionText.map{safetyMargin(raw: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var dataArray: [RefTransformable] = [] {
        //: didSet {
        didSet {
    ...
    lazy var ranktableView = UITableView(frame: self.bounds, style: .plain).then {
        ...
        $0.register(FileHostThen.self, forCellReuseIdentifier: FileHostThen.className())
        //: $0.register(SocialRankListViewCell.self, forCellReuseIdentifier: SocialRankListViewCell.className())
        $0.register(DevelopmentViewCell.self, forCellReuseIdentifier: DevelopmentViewCell.className())
    }
    ...
    lazy var tableHeaderView = HandView(frame: CGRect(x: 0, y: 0, width: user_barText, height: actualHeight(h: 220 + 14))).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.index = self.type
        $0.index = self.type
    ...
    lazy var tableBackgrourdView = SpendThen().then {
        //: $0.backgroundColor = .white
        $0.backgroundColor = .white
        //: $0.desLab.text = "Not yet~".localized
        $0.desLab.text = (String(data_equalContent)).localized
        //: $0.desLab.textColor = UIColor(hex: "#8C7AFF")
        $0.desLab.textColor = UIColor(hex: (String(k_viewName.prefix(5)) + String(appDismissActionName)))
...
extension ListViewDelegate: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: FileHostThen.className(), for: indexPath) as! FileHostThen
            //: return cell
            return cell
        }
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: DevelopmentViewCell.className(), for: indexPath) as! DevelopmentViewCell
        //: let model = dataArray[indexPath.row+3]
        let model = dataArray[indexPath.row + 3]
        //: cell.updateRankCell(model: model, index: indexPath.row+3)
        cell.random(model: model, index: indexPath.row + 3)
        //: cell.updateIcon(type: self.type)
        cell.modifyArrayEqual(type: self.type)
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: FileHostThen.className()) as! FileHostThen
...
extension ListViewDelegate: JXSegmentedListContainerViewListDelegate {
