...
fileprivate let user_centerKey:[UInt8] = [0xba,0xbd,0xba,0xa7,0xfb,0xb0,0xbc,0xb7,0xb6,0xa1,0xe9,0xfa,0xf3,0xbb,0xb2,0xa0,0xf3,0xbd,0xbc,0xa7,0xf3,0xb1,0xb6,0xb6,0xbd,0xf3,0xba,0xbe,0xa3,0xbf,0xb6,0xbe,0xb6,0xbd,0xa7,0xb6,0xb7]

/*: "bg_medals_podium" :*/
fileprivate let noti_splitMsg:[Character] = ["b","g","_","m","e"]
fileprivate let mainColorPath:String = "blockals"

/*: "icon_rank_first_bg" :*/
fileprivate let appNameData:[Character] = ["i","c","o","n","_","r","a","n","k","_","f","i","r","s","t","_"]
fileprivate let main_imageMessage:String = "bmatch"

/*: "icon_rank_first" :*/
fileprivate let k_topTitle:[Character] = ["i","c","o","n","_","r","a","n","k"]
fileprivate let constStatusValue:[Character] = ["_","f","i","r","s","t"]

/*: "icon_ammount_points" :*/
fileprivate let user_modelTransformKey:String = "icon_ammake quit card message"
fileprivate let dataShareKey:String = "mount_view to player text true"

/*: "icon_lounge" :*/
fileprivate let constToPath:String = "kit add content for viewicon_l"

/*: "icon_rank_coin" :*/
fileprivate let notiUnderLabStr:String = "icpostn"
fileprivate let showViewGestureMainKey:String = "self hidden label background layerk_coin"

/*: "icon_rank_coin2" :*/
fileprivate let showUserTitle:[Character] = ["i","c","o","n","_","r","a","n","k","_","c","o"]
fileprivate let notiCloudUrl:String = "iname"

/*: "icon_rank_second" :*/
fileprivate let show_backMessage:String = "index a if bottom selficon_ra"
fileprivate let showLevelName:[Character] = ["n","k","_","s","e","c","o","n","d"]

/*: "icon_rank_third" :*/
fileprivate let dataKeyFemaleMessage:String = "itableon"
fileprivate let mainQuickManagerStr:String = "tshrinkr"
fileprivate let dataColorPath:String = "user"
...
class HandView: UIView {
    ...
    var dataArray: [RefTransformable] = [] {
        //: didSet {
        didSet {
    ...
    override init(frame: CGRect) {
        ...
        super.init(frame: frame)
        //: self.setupSubviews()
        self.externalBodyPartInButton()
        //: self.setupSubViewsConstraint()
        self.design()
        //: self.bindInteraction()
        self.doingCounteraction()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: user_centerKey.map{$0^211}, encoding: .utf8)!)
    }
    ...
    lazy var medalsView = UIImageView().then {
        ...
        $0.image = UIImage.snapIcon(name: (String(noti_splitMsg) + mainColorPath.replacingOccurrences(of: "block", with: "d") + "_podium"))
    }

    //: lazy var firstView = SocialRankHeaderItemView.init(frame: .zero, rankType: .firstRankType).then {
    lazy var firstView = TapValueThen(frame: .zero, rankType: .firstRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 0
        $0.tag = 0
        //: let tap = UITapGestureRecognizer.init(target: self, action: #selector(registTapAction))
        let tap = UITapGestureRecognizer(target: self, action: #selector(enableStatus))
        //: $0.addGestureRecognizer(tap)
        $0.addGestureRecognizer(tap)
    }

    //: lazy var secondView = SocialRankHeaderItemView.init(frame: .zero, rankType: .secondRankType).then {
    lazy var secondView = TapValueThen(frame: .zero, rankType: .secondRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 1
        $0.tag = 1
    ...
    lazy var thirdView = TapValueThen(frame: .zero, rankType: .thirdRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 2
        $0.tag = 2
...
extension HandView {
    ...
    private func externalBodyPartInButton() {
    ...
    private func design() {
        ...
        let firstWidth = (user_barText - actualWidth(w: 68)) / 3 + actualWidth(w: 10)
        //: let itemWidth = (ScreenWidth - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
        let itemWidth = (user_barText - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
...
class TapValueThen: UIView {
    ...
    public init(frame: CGRect, rankType: ColorWith) {
        //: super.init(frame: frame)
        super.init(frame: frame)
        //: self.setupItemViews(type: rankType)
        self.be(type: rankType)
    }
    ...
    lazy var headPicBgView = UIImageView().then {
        ...
        $0.image = UIImage.snapIcon(name: (String(appNameData) + main_imageMessage.replacingOccurrences(of: "match", with: "g")))
    }
    ...
    lazy var crownImgView = UIImageView().then {
        ...
        $0.image = UIImage.snapIcon(name: (String(k_topTitle) + String(constStatusValue)))
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .font(type: .Medium, fontSize: 18)
        //: $0.textAlignment = .center
        $0.textAlignment = .center
    }

    //: lazy var coinButton = TalkingButton().then {
    lazy var coinButton = GroupVideoControl().then {
        //: $0.isUserInteractionEnabled = false
        $0.isUserInteractionEnabled = false
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_ammount_points"), for: .normal)
        $0.setImage(UIImage.snapIcon(name: (String(user_modelTransformKey.prefix(7)) + String(dataShareKey.prefix(6)) + "points")), for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 13)
        $0.titleLabel?.font = .font(type: .Regular, fontSize: 13)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.snapIcon(name: (String(constToPath.suffix(6)) + "ounge"))
    }
}
...
extension TapValueThen {
    //: func updateItemWithModel(model: SocialRankItemModel?, type: Int) {
    func mTheoryPlayer(model: RefTransformable?, type: Int) {
        ...
        headPic.applicatorEqual(urlStr: model!.headPic, placeImg: UIImage.equalSex(sex: model!.sex))
        ...
        if type == 0 {
            ...
            coinButton.setImage(UIImage.snapIcon(name: (notiUnderLabStr.replacingOccurrences(of: "post", with: "o") + "_ran" + String(showViewGestureMainKey.suffix(6)))), for: .normal)
            //: } else {
        } else {
            //: coinButton.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin2"), for: .normal)
            coinButton.setImage(UIImage.snapIcon(name: (String(showUserTitle) + notiCloudUrl.replacingOccurrences(of: "name", with: "n2"))), for: .normal)
        }
    }
}
...
//        var topCrownMargin = actualHeight(h: 4)
        ...
        case .firstRankType:
            ...
            crownImgView.image = UIImage.snapIcon(name: (String(k_topTitle) + String(constStatusValue)))
            //: headPic.layer.borderColor = UIColor.RGBA(255, 224, 58, 1).cgColor
            headPic.layer.borderColor = UIColor.underReadingRgba(255, 224, 58, 1).cgColor
...
//            topCrownMargin = actualHeight(h: 10)
        ...
        case .secondRankType:
            ...
            crownImgView.image = UIImage.snapIcon(name: (String(show_backMessage.suffix(7)) + String(showLevelName)))
            //: headPic.layer.borderColor = UIColor.RGBA(182, 199, 254, 1).cgColor
            headPic.layer.borderColor = UIColor.underReadingRgba(182, 199, 254, 1).cgColor
        ...
        case .thirdRankType:
            ...
            crownImgView.image = UIImage.snapIcon(name: (dataKeyFemaleMessage.replacingOccurrences(of: "table", with: "c") + "_rank_" + mainQuickManagerStr.replacingOccurrences(of: "shrink", with: "hi") + dataColorPath.replacingOccurrences(of: "user", with: "d")))
            //: headPic.layer.borderColor = UIColor.RGBA(251, 166, 109, 1).cgColor
            headPic.layer.borderColor = UIColor.underReadingRgba(251, 166, 109, 1).cgColor
            //: headPic.layer.cornerRadius = actualWidth(w: 31)
            headPic.layer.cornerRadius = actualWidth(w: 31)
            //: break
        }
