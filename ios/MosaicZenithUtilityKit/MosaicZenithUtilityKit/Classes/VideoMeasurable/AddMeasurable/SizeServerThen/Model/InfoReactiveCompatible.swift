...
/*: "MF:LiveChatWelMsg" :*/
fileprivate let appLineItemText:String = "MF:Luser view false"
fileprivate let noti_scaleMsg:String = "atWelMsgparty make extension push"

/*: "MF:PartyChatWelMsg" :*/
fileprivate let const_cellKey:String = "MF:Parself conversation background as for"
fileprivate let show_subMsg:[Character] = ["t","y","C","h","a","t","W","e","l","M","s","g"]

/*: "MF:LiveChatPrizeMsg" :*/
fileprivate let constMakeTitle:String = "MF:Lin gift"
fileprivate let appSincePath:String = "rizeMsgkey reply"

/*: "MF:PartyChatPrizeMsg" :*/
fileprivate let const_shareText:[Character] = ["M","F",":","P","a","r","t"]
fileprivate let app_makeIconTitle:[Character] = ["y","C","h","a","t","P"]
fileprivate let appViewText:String = "rizeMsgself to value make here"

/*: "MF:LiveChatAttentionMsg" :*/
fileprivate let constLengthId:String = "var colorMF:Liv"
fileprivate let main_topAppName:String = "Attepublic row raw"

/*: "MF:PartyChatAttentionMsg" :*/
fileprivate let notiAllValue:String = "MF:Pas time succeed view"
fileprivate let noti_interactionStartMinMessage:String = "atAttstring manager filter"
fileprivate let showViewUrl:String = "ennighti"

/*: "#BAFBFF" :*/
fileprivate let app_makePath:String = "info moment#BAFBFF"

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let constCellKey:[Character] = ["<","a","t"]
fileprivate let dataIconPath:String = ">@[\\"
fileprivate let const_removeToEditPath:String = "make hidden player m?</at>"

/*: "#FFA4CF" :*/
fileprivate let appLoadKey:String = "#FFA4CFreturn left"

/*: "Here comes " :*/
fileprivate let noti_sumPath:String = "make false error party makeHere com"
fileprivate let constUserMsg:String = "empty extensiones "

/*: "#FFE386" :*/
fileprivate let k_makeText:[Character] = ["#","F","F","E","3","8","6"]

/*: "followed the anchor" :*/
fileprivate let user_cameraId:[Character] = ["f","o","l","l","o","w","e","d"]
fileprivate let showVideoMsg:String = "label height face height the "

/*: "party_seat_host" :*/
fileprivate let app_withMsg:String = "peffectt"
fileprivate let dataBackFormat:String = "at_hofalse location"
fileprivate let notiEditTitle:[Character] = ["s","t"]

/*: "icon_lounge" :*/
fileprivate let appUseUrl:String = "false path view name elseicon_"
fileprivate let data_styleName:String = "LOUNGE"
...
class InfoReactiveCompatible: NSObject {
    ...
    func statusName(model: TitleDanmuModel) -> TitleDanmuModel {
        //: self.model = model
        self.model = model
        /// 普通文本消息,礼物消息
        ...
        } else if self.model.MsgExtension == (String(appLineItemText.prefix(4)) + "iveCh" + String(noti_scaleMsg.prefix(8))) || self.model.MsgExtension == (String(const_cellKey.prefix(6)) + String(show_subMsg)) {
            //: caculateWelMsg()
            quantityName()
            /// 中奖消息
            //: } else if self.model.MsgExtension == "MF:LiveChatPrizeMsg" || self.model.MsgExtension == "MF:PartyChatPrizeMsg" {
        } else if self.model.MsgExtension == (String(constMakeTitle.prefix(4)) + "iveChatP" + String(appSincePath.prefix(7))) || self.model.MsgExtension == (String(const_shareText) + String(app_makeIconTitle) + String(appViewText.prefix(7))) {
        ...
        } else if self.model.MsgExtension == (String(constLengthId.suffix(6)) + "eChat" + String(main_topAppName.prefix(4)) + "ntionMsg") || self.model.MsgExtension == (String(notiAllValue.prefix(4)) + "artyCh" + String(noti_interactionStartMinMessage.prefix(5)) + showViewUrl.replacingOccurrences(of: "night", with: "t") + "onMsg") {
            //: caculAttionMsg()
            stockMarketIndex()
        }
        //: return self.model
    ...
    func permission() {
        ...
        let nameAttr = statusPlace(text: firstStr, font: UIFont.dismissSharedSize(fontSize: 14), color: UIColor(hex: (String(app_makePath.suffix(7))))!, isTap: true, atUid: String(self.model.user?.uid ?? 0))
        ...
        if self.model.msgInfo?.messageType == 1 {
            ...
            let content = statusPlace(text: self.model.msgInfo?.content ?? "", font: UIFont.dismissSharedSize(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
            ...
            let regex = try! NSRegularExpression(pattern: (String(constCellKey) + dataIconPath.capitalized + "S\\s]+" + String(const_removeToEditPath.suffix(6))), options: [])
            ...
            if matchRange?.length ?? 0 > 9 {
                ...
                let subStr = String.unicodeNowTo(str: text, from: (matchRange?.location ?? 0) + 4, to: (matchRange?.length ?? 0) - 5)
                //: let contentStr = String.substringByUnicodeIndex(str: text, from: (matchRange?.length ?? 0), to: text.utf16.count )
                let contentStr = String.unicodeNowTo(str: text, from: (matchRange?.length ?? 0), to: text.utf16.count)
                ...
                let atName = statusPlace(text: subStr, font: UIFont.dismissSharedSize(fontSize: 14), color: UIColor(hex: (String(app_makePath.suffix(7))))!, isTap: true, atUid: "\(atUid)")
                ...
                let content = statusPlace(text: contentStr, font: UIFont.dismissSharedSize(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
            ...
            let sentAttr = statusPlace(text: "Sent ".localized, font: UIFont.dismissSharedSize(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
            ...
            if let toUser = self.model.toUser { // 送礼接收人
                ...
                let nickNameAttr = statusPlace(text: "\(toUser.nickname) ", font: UIFont.font(type: .Medium, fontSize: 14), color: UIColor(hex: (String(appLoadKey.prefix(7))))!, isTap: true, atUid: String(toUser.uid))
            ...
            let giftAttr = statusPlace(text: giftnameStr, font: UIFont.font(type: .Medium, fontSize: 14), color: UIColor(hex: (String(appLoadKey.prefix(7))))!, isTap: false, atUid: nil)
            ...
            let imag = self.model.gift?.giftImg?.notAthlete(with: size)
            ...
            let giftIcon = NSMutableAttributedString.yy_attachmentString(withContent: imag, contentMode: .center, attachmentSize: size, alignTo: UIFont.dismissSharedSize(fontSize: 14), alignment: .center)
            //: giftIcon.yy_paragraphStyle = paragraphStyle()
            giftIcon.yy_paragraphStyle = changeDelete()
            ...
            let numAttr = self.statusPlace(text: "*\(self.model.gift?.num ?? 0)", font: UIFont.dismissSharedSize(fontSize: 14), color: UIColor(hex: (String(appLoadKey.prefix(7))))!, isTap: false, atUid: nil)
        ...
        appImage(attribText: self.model.msgAttribText)
    }

    /// 公告富文本
    //: func showAnnouncementMsg(model: TalkingLiveRoomDanmuModel) -> TalkingLiveRoomDanmuModel {
    func orderedSeries(model: TitleDanmuModel) -> TitleDanmuModel {
        //: self.model = model
        self.model = model
        // 内容
        //: let paraStyle = paragraphStyle()
        ...
        let content = statusPlace(text: self.model.msgInfo?.content ?? "", font: UIFont.dismissSharedSize(fontSize: 14), color: UIColor(hex: (String(app_makePath.suffix(7))))!, isTap: false, atUid: nil)
    ...
    func quantityName() {
        ...
        let strnor = (String(noti_sumPath.suffix(8)) + String(constUserMsg.suffix(3))).localized
        ...
        let content = statusPlace(text: strnor, font: UIFont.dismissSharedSize(fontSize: 14), color: UIColor(hex: (String(k_makeText)))!, isTap: false, atUid: nil)
        ...
        let attachText = statusPlace(text: strc, font: UIFont.dismissSharedSize(fontSize: 14), color: UIColor(hex: (String(app_makePath.suffix(7))))!, isTap: true, atUid: String(self.model.user?.uid ?? 0))
    ...
    func gestureLevelMsg() {
        ...
        let content = statusPlace(text: firstStr, font: UIFont.dismissSharedSize(fontSize: 14), color: UIColor.white, isTap: true, atUid: String(self.model.user?.uid ?? 0))
        //: textView.append(content)
        textView.append(content)
        //: self.model.msgAttribText = textView
        self.model.msgAttribText = textView
    ...
    func stockMarketIndex() {
        ...
        let strnor = (String(user_cameraId) + String(showVideoMsg.suffix(5)) + "anchor").localized
        //: let strc =  "\(self.model.user?.nickname ?? "") "
        let strc = "\(self.model.user?.nickname ?? "") "
        ...
        let content = statusPlace(text: strnor, font: UIFont.dismissSharedSize(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
        ...
        let nameText = statusPlace(text: strc, font: UIFont.dismissSharedSize(fontSize: 14), color: UIColor(hex: (String(app_makePath.suffix(7))))!, isTap: true, atUid: String(self.model.user?.uid ?? 0))
    ...
    func changeDelete() -> NSMutableParagraphStyle {
    ...
    func statusPlace(text: String, font: UIFont, color: UIColor, isTap: Bool, atUid: String?) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = changeDelete()
        ...
        if isTap == true {
            ...
            attribute.yy_setTextHighlight(range, color: color, backgroundColor: .clear, userInfo: nil) { _, _, _, _ in
                ...
                /// 不能@自己
                //: if atUid != ExamineedReactiveCompatible.share.loginUserMode.userID {
                if atUid != ExamineedReactiveCompatible.share.loginUserMode.userID {
    ...
    func appImage(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = MsgTableViewWidth - 16
        let maxWidth = app_indexUrl - 16
    ...
    private func giftmodel(giftmodel: MediaAddTransformable) -> String {
...
extension InfoReactiveCompatible {
    ...
    func result() -> NSMutableAttributedString {
        ...
        if ListThen.withShared().isParty, model.user?.uid == ListThen.withShared().partyModel.streamerInfo.uid {
            ...
            hostImgV.image = UIImage.snapIcon(name: (app_withMsg.replacingOccurrences(of: "effect", with: "ar") + "y_se" + String(dataBackFormat.prefix(5)) + String(notiEditTitle)))
            //: let hostAttr = getAttachText(imageView: hostImgV)
            let hostAttr = markTo(imageView: hostImgV)
        ...
        levelImgV.image = UIImage.snapIcon(name: "level_\(self.model.user?.level ?? 1)")
        //: let levelAttr = getAttachText(imageView: levelImgV)
        let levelAttr = markTo(imageView: levelImgV)
        ...
        if self.model.user?.loungePlus == true {
            ...
            vipImgV.image = UIImage.snapIcon(name: (String(appUseUrl.suffix(5)) + data_styleName.lowercased()))
            //: let vipAttr = getAttachText(imageView: vipImgV)
            let vipAttr = markTo(imageView: vipImgV)
    ...
    func markTo(imageView: UIImageView) -> NSMutableAttributedString {
        ...
        let attachText = NSMutableAttributedString.yy_attachmentString(withContent: imageView.image, contentMode: .scaleToFill, attachmentSize: imageView.size, alignTo: UIFont.easing(fontSize: 12), alignment: .center)
        //: attachText.yy_paragraphStyle = paragraphStyle()
        attachText.yy_paragraphStyle = changeDelete()
        //: return attachText
        return attachText
    }
}
