...
fileprivate let noti_licenseId:[UInt8] = [0x9e,0x99,0x9e,0x83,0xdf,0x94,0x98,0x93,0x92,0x85,0xcd,0xde,0xd7,0x9f,0x96,0x84,0xd7,0x99,0x98,0x83,0xd7,0x95,0x92,0x92,0x99,0xd7,0x9e,0x9a,0x87,0x9b,0x92,0x9a,0x92,0x99,0x83,0x92,0x93]

				private func viewShare(from num: UInt8) -> UInt8 {
					return num ^ 247
				}
...
@objc protocol ImageViewDelegate: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func service()

    //: @objc optional func endScroll()
    @objc optional func modelView()
}

//: class TalkingDanmuMsgListTableView: UIView {
class CommentPicView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_licenseId.map{viewShare(from: $0)}, encoding: .utf8)!)
    }
...
extension CommentPicView {
    ...
    func changeTimer() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(labWindow), userInfo: nil, repeats: true)
    }
...
extension CommentPicView {
    ...
    func changeModel(msgModel: TitleDanmuModel?) {
    ...
    func observerBottom() {
        ...
        if AllHeight > dataNoVersionValue {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < dataNoVersionValue {
                ...
                self.msgTableView.height = dataNoVersionValue
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = dataNoVersionValue - AllHeight
        ...
        bagAnimated(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func bagAnimated(animated: Bool) {
    ...
    func imageError(inPending: Bool) {
...
extension CommentPicView: UITableViewDelegate, UITableViewDataSource {
    ...
    func outGift(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.modelView?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(data_fileText)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = TapMediaViewCell.pastData(tableView: tableView, msg: msgModel, indexPath: indexPath)
