...
fileprivate let constValueMsg:[UInt8] = [0xf,0x8,0xf,0x12,0x4e,0x5,0x9,0x2,0x3,0x14,0x5c,0x4f,0x46,0xe,0x7,0x15,0x46,0x8,0x9,0x12,0x46,0x4,0x3,0x3,0x8,0x46,0xf,0xb,0x16,0xa,0x3,0xb,0x3,0x8,0x12,0x3,0x2]

				private func clubAction(show num: UInt8) -> UInt8 {
					return num ^ 102
				}

/*: "bth_live_follow_pre" :*/
fileprivate let show_equalValue:[Character] = ["b","t","h","_","l","i","v","e","_"]
fileprivate let mainCenterStr:String = "fotexttext"

/*: "btn_live_mini_nor" :*/
fileprivate let userSectionId:[Character] = ["b","t","n"]
fileprivate let kShadowValue:String = "_livetitle about"
fileprivate let const_toolAirFormat:String = "i_norgreet social view pic"

/*: "#2DF2FF" :*/
fileprivate let data_jawRefreshKey:String = "return model to image app#2DF2FF"

/*: "#FF506D" :*/
fileprivate let show_blockId:String = "#Ftime black"
fileprivate let const_imageData:String = "F506menu"

/*: "removeAttentionUid" :*/
fileprivate let dataCellMsg:String = "raddove"
fileprivate let notiContainerSizePath:[Character] = ["n","t","i","o","n","U","i","d"]

/*: "attentionUid" :*/
fileprivate let k_imageData:String = "A"
fileprivate let k_scaleText:[Character] = ["t","t","e","n","t","i","o","n","U","i","d"]

/*: "source" :*/
fileprivate let constFrontFormat:String = "sounextce"

/*: "btn_live_closed_nor" :*/
fileprivate let app_picFormat:[Character] = ["b","t","n","_","l","i","v","e","_","c","l","o","s","e","d","_","n","o","r"]
...
class SkipTopView: UIView {
    //: weak var weakVC: TalkingBaseViewController?
    weak var weakVC: ReloadViewController?
    // 右侧按钮点击Block
    ...
    var noFaceTime: Int = 0 {
        ...
        didSet {
            ...
            if noFaceTime >= ExamineedReactiveCompatible.share.appConfigMode.countdown1 { // 展示倒计时
                ...
                self.countdownLab.text = "\(kButtonKey - noFaceTime)s"
                // svga
                //: self.svgaView.isHidden = false
                self.svgaView.isHidden = false
                //: self.svgaView.refreshView(countdown: "\(MAX_NOFACETIME-noFaceTime)")
                self.svgaView.frameRangeCountdown(countdown: "\(kButtonKey - noFaceTime)")
                //: } else {
            } else {
                //: self.refreshLiveStatus()
                self.transactionEqual()
            }
        }
    }
    ...
    private var liveModel = AddModelType()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constValueMsg.map{clubAction(show: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(punchCard), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.snapIcon(name: (String(show_equalValue) + mainCenterStr.replacingOccurrences(of: "text", with: "l") + "ow_pre")), for: .normal)
        //: btn.addTarget(self, action: #selector(followButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(followClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var miniBtn: UIButton = {
        ...
        btn.setImage(UIImage.snapIcon(name: (String(userSectionId) + String(kShadowValue.prefix(5)) + "_min" + String(const_toolAirFormat.prefix(5)))), for: .normal)
        //: btn.addTarget(self, action: #selector(miniButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(resource), for: .touchUpInside)
    ...
    private lazy var liveStatusImgV: GroupVideoControl = {
        ...
        let image = UIImage.imageContent(color: UIColor(hex: (String(data_jawRefreshKey.suffix(7))))!, size: CGSize(width: 8, height: 8))
    ...
    private lazy var countdownView: UIView = {
        ...
        icon.backgroundColor = UIColor(hex: (String(show_blockId.prefix(2)) + const_imageData.replacingOccurrences(of: "menu", with: "D")))
    ...
    private lazy var countdownLab: UILabel = {
        ...
        lab.font = UIFont.dismissSharedSize(fontSize: 12)
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.text = " \(ExamineedReactiveCompatible.share.appConfigMode.countdown2)s"
        lab.text = " \(ExamineedReactiveCompatible.share.appConfigMode.countdown2)s"
    ...
    private lazy var onlineNumBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(indicatorClick), for: .touchUpInside)
...
extension SkipTopView {
    ...
    @objc func followCompletion(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(dataCellMsg.replacingOccurrences(of: "add", with: "em") + "Atte" + String(notiContainerSizePath))] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(k_imageData.lowercased() + String(k_scaleText))] = uid
            //: dict["source"] = "1"
            dict[(constFrontFormat.replacingOccurrences(of: "next", with: "r"))] = "1"
        }
        //: SizeRequestTool.req_atationTool(isAttention: !isAttention, params: dict) { succeed, result, errorModel in
...
extension SkipTopView {
    ...
    @objc private func indicatorClick() {
        //: self.onlineListView.showView(chatGroupId: liveModel.chatGroupId)
        self.onlineListView.alongEqualSend(chatGroupId: liveModel.chatGroupId)
    }
    ...
    @objc private func activity() {
        ...
        RefAudienceManager().sinceView(chatGroupId: liveModel.chatGroupId, completion: { succeed, result, _ in
    ...
    private func faceTimer() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 10, target: self, selector: #selector(activity), userInfo: nil, repeats: true)
    }

    //: func stopTimer() {
    func pullUpShortTimer() {
...
extension SkipTopView {
    ...
    func attributeModel(_ liveModel: AddModelType) {
        ...
        icon.applicatorEqual(urlStr: liveModel.streamerInfo.headPic, placeImg: UIImage.equalSex(sex: liveModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        ...
        if isAnchor {
            ...
            miniBtn.setImage(UIImage.snapIcon(name: (String(userSectionId) + String(kShadowValue.prefix(5)) + "_min" + String(const_toolAirFormat.prefix(5)))), for: .normal)
            //: refreshFollow(isAttention: true)
            addDown(isAttention: true)
            //: } else {
        } else {
            //: miniBtn.setImage(UIImage.BundleImageNamed(name: "btn_live_closed_nor"), for: .normal)
            miniBtn.setImage(UIImage.snapIcon(name: (String(app_picFormat))), for: .normal)
            //: refreshFollow(isAttention: self.liveModel.streamerInfo.isAttention)
            addDown(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
    ...
    @objc func resource() {
        ...
        if isAnchor { // 改为小窗口
            ...
            LiveManager.upShared().giftWindow(smallWindow: true)
            //: currentViewController()?.navigationController?.popViewController(animated: false)
            announcement()?.navigationController?.popViewController(animated: false)
        }
    }
    ...
    @objc private func punchCard() {
        ...
        underside(uid: String(LiveManager.upShared().liveRoomModel.streamerInfo.uid))
    }

    /// 展示个人资料卡
    //: private func showUserCard(uid: String) {
    private func underside(uid: String) {
        //: let userCard = TalkingLiveUserCardView(frame: .zero, uid: uid)
        let userCard = PushCardView(frame: .zero, uid: uid)
        //: userCard.delegate = weakVC as? any TalkingLiveUserCardViewDelegate
        userCard.delegate = weakVC as? any SizeObjectProtocol
        //: userCard.show()
        userCard.sizeRequest()
    }

    /// 刷新关注按钮
    //: private func refreshFollow(isAttention: Bool) {
    private func addDown(isAttention: Bool) {
    ...
    private func transactionEqual() {
        //: self.liveStatusImgV.isHidden = !TalkingLiveManager.shared().isLive
        self.liveStatusImgV.isHidden = !LiveManager.upShared().isLive
    ...
    @objc private func followClick() {
        ...
        followCompletion(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.addDown(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
}
...
extension SkipTopView {
    ...
    private func setupMagnitudeeraction() {
        //: self.frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: StatusBarHeight+actualWidth(w: 68))
        self.frame = CGRect(x: 0, y: 0, width: user_barText, height: app_versionItemStr + actualWidth(w: 68))
