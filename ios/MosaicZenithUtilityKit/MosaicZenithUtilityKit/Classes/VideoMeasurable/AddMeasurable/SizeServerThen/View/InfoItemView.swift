...
/*: "#ffffff" :*/
fileprivate let main_resultStr:String = "#fffffffor type if success"

/*: "/dist/loungePlus/index.html" :*/
fileprivate let user_sampleMessage:String = "/dist/custom gift model name"
fileprivate let showStatusName:String = "height if removeePlus"
fileprivate let notiModelKey:String = "x.htmlcamera video half angle app"
...
class InfoItemView: UIView {
    ...
    let Broadcast_TextColor = (String(main_resultStr.prefix(7)))
    //: let Broadcast_ContentFont: UIFont = UIFont.pingfangMediumFont(fontSize: 12)
    let Broadcast_ContentFont: UIFont = .dismissSharedSize(fontSize: 12)
    ...
    convenience init(model: LabMeasurable?) {
        //: self.init()
        self.init()
    ...
    private func zygotene() {
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            var mutableAtt = parse.thoughtImage(withFontSize: 12, insert: "", atIndex: 0)
            // 限制字符长度
    ...
    func keep(withUrl url: String) {
        //: self.backgroundImageView.setUrlImage(urlStr: url) {[weak self] result, image in
        self.backgroundImageView.applicatorEqual(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.Broadcast_HeightOfBackgroundImageView {
                    ...
                    newImage = image.notAthlete(with: CGSize(width: targetWidth, height: self.Broadcast_HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: LabMeasurable? {
        //: didSet {
        didSet {
            //: configModel()
            zygotene()
    ...
    var totalTime: CGFloat {
        ...
        return (user_barText + widthValue) * Broadcast_SpendTime / user_barText
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * Broadcast_LineSpendTime / ScreenWidth
        return widthValue * Broadcast_LineSpendTime / user_barText
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * Broadcast_SpendTime / ScreenWidth
        return widthValue * Broadcast_SpendTime / user_barText
    }
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(incursion(_:)))
        //: return t
        return t
        //: }()
    }()
    ...
    lazy var contentLabel: UILabel = {
        ...
        l.font = UIFont.dismissSharedSize(fontSize: 12)
        //: l.textColor = .white
        l.textColor = .white
        //: l.numberOfLines = 1
        l.numberOfLines = 1
...
extension InfoItemView {
    ...
    @objc func incursion(_: UITapGestureRecognizer) {
        ...
        //: case "1": // 个人中心
        case "1": // 个人中心
            //: AppPushManager.share.func__pushToUserDetailVC(uid: self.danumuMsgModel?.jumpKey)
            AppPushManager.share.enrich(uid: self.danumuMsgModel?.jumpKey)

        //: case "2": // 直播间
        case "2": // 直播间
            //: if let uid = self.danumuMsgModel?.jumpKey {
            if let uid = self.danumuMsgModel?.jumpKey {
                //: AppPushManager.share.func_audiencePushToLiveRoomVC(uid: uid, enterType: .unknown)
                AppPushManager.share.recordShow(uid: uid, enterType: .unknown)
            }
        ...
        case "3": // H5页面
            ...
            if let url = self.danumuMsgModel?.jumpKey {
                ...
                if url.contains((String(user_sampleMessage.prefix(6)) + "loung" + String(showStatusName.suffix(5)) + "/inde" + String(notiModelKey.prefix(6)))) {
                    //: AppPushManager.share.func__pushToSubscribePageWebVC()
                    AppPushManager.share.arm()
                    //: return
                    return
                }
                //: let httpsUrl = ExamineedReactiveCompatible.share.appConfigMode.urlH5Domain+url
                let httpsUrl = ExamineedReactiveCompatible.share.appConfigMode.urlH5Domain + url
                //: let httpUrl = httpsUrl.replacingOccurrences(of: "https://", with: "http://")
                let httpUrl = httpsUrl.replacingOccurrences(of: "https://", with: "http://")
                //: AppPushManager.share.func__pushToWebVC(urlStr: httpUrl)
                AppPushManager.share.makePic(urlStr: httpUrl)
            }
