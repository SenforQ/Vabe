...
fileprivate let data_appValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "btn_chat_send_nor" :*/
fileprivate let data_labPath:[Character] = ["b","t","n","_","c","h"]
fileprivate let main_touchFormat:String = "at_scolor open as"

/*: "btn_chat_send_dis" :*/
fileprivate let k_instanceContent:String = "btn_chato if model show"
fileprivate let appUserValue:[Character] = ["t","_","s","e","n","d","_","d","i","s"]

/*: "Say something...     " :*/
fileprivate let show_emptyData:String = "frame add cell like mainSay s"
fileprivate let userNeedFeatureFormat:String = "value leading let titleng..."
fileprivate let data_iconMsg:String = "     "

/*: "Can't send blank message" :*/
fileprivate let noti_resultVideoMsg:String = "Can\'t"
fileprivate let data_rowId:String = "self station selected blank"
fileprivate let const_withData:String = "GE"

/*: "length" :*/
fileprivate let showBackManagerKey:String = "LENGTH"

/*: "contentSize" :*/
fileprivate let main_endMessage:[Character] = ["c","o","n","t","e","n","t","S","i","z","e"]
...
class FileViewDelegate: UIView {
    //: open weak var delegate: TalkingLiveRoomInputViewDelegate?
    open weak var delegate: BackThen?
    ...
    override init(frame: CGRect) {
        ...
        confinementOf()
        //: bindInteraction()
        stickToWithinNumbererchange()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: data_appValue.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var sendBtn: UIButton = {
        ...
        btn.setBackgroundImage(UIImage.snapIcon(name: (String(data_labPath) + String(main_touchFormat.prefix(4)) + "end_nor")), for: .normal)
        //: btn.setBackgroundImage(UIImage.BundleImageNamed(name: "btn_chat_send_nor"), for: .highlighted)
        btn.setBackgroundImage(UIImage.snapIcon(name: (String(data_labPath) + String(main_touchFormat.prefix(4)) + "end_nor")), for: .highlighted)
        //: btn.setBackgroundImage(UIImage.BundleImageNamed(name: "btn_chat_send_dis"), for: .disabled)
        btn.setBackgroundImage(UIImage.snapIcon(name: (String(k_instanceContent.prefix(7)) + String(appUserValue))), for: .disabled)
    ...
    lazy var inputTextView: SearchedHasDelegate = {
        ...
        input.placeholderColor = UIColor.aftColor()
        //: input.placeholder = "Say something...     ".localized
        input.placeholder = (String(show_emptyData.suffix(5)) + "omethi" + String(userNeedFeatureFormat.suffix(5)) + data_iconMsg.capitalized).localized
...
extension FileViewDelegate {
    ...
    @objc func tableClick() {
        //: let textstr = self.handleSendText(text: inputTextView.attributedText)
        let textstr = self.wearer(text: inputTextView.attributedText)
        ...
        if temStr.count == 0 {
            ...
            heatherMixtureMoment(showMsg: (noti_resultVideoMsg + " send" + String(data_rowId.suffix(6)) + " messa" + const_withData.lowercased()).localized)
            //: return
            return
        }
        ...
        self.delegate?.viewLoad(msgStr: textstr, atUid: atStr)
        //: atDic = nil
        atDic = nil
    ...
    func makeUp() {
        ...
        if inputTextView.text.count == 0 {
            ...
            inputTextView.placeholder = (String(show_emptyData.suffix(5)) + "omethi" + String(userNeedFeatureFormat.suffix(5)) + data_iconMsg.capitalized).localized
        }
    }
    ...
    func coatButton() {
        ...
        self.delegate?.giftSendHeight(height: textHeight + 16)
    }

    //: @objc func keyboardWasShown(notification: NSNotification) {
    @objc func titleNotification(notification: NSNotification) {
        ...
        self.delegate?.notBind(heightToBottom: height)
    }
...
extension FileViewDelegate {
    ...
    func resumeAdd(range: NSRange?, dict: [String: Any]) {
        //: var atRange = range
        var atRange = range
        //: if atRange != nil {
        if atRange != nil { // 输入框@
        ...
        atInfo[(showBackManagerKey.lowercased())] = atStr.count
    ...
    private func wearer(text: NSAttributedString?) -> String {
...
extension FileViewDelegate: UITextViewDelegate {
...
extension FileViewDelegate {
    ...
    private func actionLoadSubviews() {
    ...
    private func stickToWithinNumbererchange() {
        //: NotificationCenter.default.addObserver(self, selector: #selector(keyboardWasShown(notification:)), name: UIResponder.keyboardWillShowNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(titleNotification(notification:)), name: UIResponder.keyboardWillShowNotification, object: nil)
        ...
        inputTextView.rx.observeWeakly(UITextView.self, (String(main_endMessage))).subscribe(onNext: { [weak self] _ in
