...
fileprivate let const_communicationValue:[UInt8] = [0xe,0x9,0xe,0x13,0x4f,0x4,0x8,0x3,0x2,0x15,0x5d,0x4e,0x47,0xf,0x6,0x14,0x47,0x9,0x8,0x13,0x47,0x5,0x2,0x2,0x9,0x47,0xe,0xa,0x17,0xb,0x2,0xa,0x2,0x9,0x13,0x2,0x3]

				private func styleModel(image num: UInt8) -> UInt8 {
					return num ^ 103
				}
...
@objc protocol PaintTheLilyObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func brand()

    //: @objc optional func endScroll()
    @objc optional func successAdd()
}
...
class TableReactiveCompatible: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(tableFollowing))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_communicationValue.map{styleModel(image: $0)}, encoding: .utf8)!)
    }
...
extension TableReactiveCompatible {
    ...
    func fillPost() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(rangeWarning), userInfo: nil, repeats: true)
    }
    ...
    @objc func rangeWarning() {
        //: tryToappendAndScrollToBottom()
        pickUpTheGauntlet()
    }

    //: @objc func tapAction() {
    @objc func tableFollowing() {
...
extension TableReactiveCompatible {
    ...
    func targetNew(msgModel: EasingMediaTransformable?) {
    ...
    func appendToHeel() {
        ...
        if AllHeight > app_errVideoName {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < app_errVideoName {
                ...
                self.msgTableView.height = app_errVideoName
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = app_errVideoName - AllHeight
        ...
        dataStatusClose(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func dataStatusClose(animated: Bool) {
...
extension TableReactiveCompatible: UITableViewDelegate, UITableViewDataSource {
    ...
    func loadEnableWith(_ scrollView: UIScrollView) {
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(user_keySaltTitle)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = DataFileThen.quantityimateLast(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension TableReactiveCompatible {
    ...
    func cur() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: show_intervalTitle, height: app_errVideoName)
    }
}
