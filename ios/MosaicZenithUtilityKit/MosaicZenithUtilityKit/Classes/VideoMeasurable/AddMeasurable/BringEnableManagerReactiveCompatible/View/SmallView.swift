...
fileprivate let mainClubName:[UInt8] = [0x84,0x89,0x84,0x8f,0x43,0x7e,0x8a,0x7f,0x80,0x8d,0x55,0x44,0x3b,0x83,0x7c,0x8e,0x3b,0x89,0x8a,0x8f,0x3b,0x7d,0x80,0x80,0x89,0x3b,0x84,0x88,0x8b,0x87,0x80,0x88,0x80,0x89,0x8f,0x80,0x7f]

				fileprivate func makeIt(image num: UInt8) -> UInt8 {
...
/*: "#FFD13A" :*/
fileprivate let k_withValue:String = "#FFD13Araw label kit progress button"

/*: "gift_gb_umnad_1" :*/
fileprivate let constAddMsg:String = "giafter"
fileprivate let noti_activityNaturalUrl:String = "count section camera name_gb_u"

/*: "icon_circle_pic_default" :*/
fileprivate let user_cardPath:String = "icon_csystem aspect selection let view"
fileprivate let const_cellData:String = "_pic_dpicture back app"

/*: "Open lucky blind-box" :*/
fileprivate let user_mainTitle:String = "manager frameOpen"
fileprivate let user_statusData:String = "model"
fileprivate let showShareFormat:[Character] = ["n","d","-","b","o","x"]

/*: "PingFangSC-Semibold" :*/
fileprivate let userMakeName:[Character] = ["P","i","n","g","F","a","n","g","S","C","-","S","e","m","i","b","o","l","d"]

/*: "Bombard" :*/
fileprivate let main_modelAppThanText:[Character] = ["B","o","m","b","a","r","d"]
...
class SmallView: UIView {
    ...
    var giftMsgModel = MediaAddTransformable()
    //: var giftSendItem = [TalkingGiftNumArrModel()]
    var giftSendItem = [EqualHandyJSON()]
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainClubName.map{makeIt(image: $0)}, encoding: .utf8)!)
    }
...
//        lb.numberOfLines = 1
        ...
        lb.textColor = UIColor(hex: (String(k_withValue.prefix(7))))
        //: lb.font = UIFont.pingfangRugularFont(fontSize: 13)
        lb.font = UIFont.easing(fontSize: 13)
        //: return lb
        return lb
        //: }()
    }()
    ...
    private lazy var pluralTip: RegisterReactiveCompatible = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = RegisterReactiveCompatible()
        //: lab.isHidden = true
        lab.isHidden = true
    ...
    private lazy var countLb: RegisterReactiveCompatible = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = RegisterReactiveCompatible()
...
extension SmallView {
    ...
    func pullUpShort(model: MediaAddTransformable) {
        ...
        self.bgImage.image = UIImage.snapIcon(name: (constAddMsg.replacingOccurrences(of: "after", with: "ft") + String(noti_activityNaturalUrl.suffix(5)) + "mnad_1"))

        //: let placeholder = UIImage.BundleImageNamed(name: "icon_circle_pic_default")
        let placeholder = UIImage.snapIcon(name: (String(user_cardPath.prefix(6)) + "ircle" + String(const_cellData.prefix(6)) + "efault"))

        //: if (model.showType == ChatGiftAnimatType.myStery.rawValue ) {
        if model.showType == ResumeVarArg.myStery.rawValue {
            //: self.nickLb.text = "Open lucky blind-box".localized
            self.nickLb.text = (String(user_mainTitle.suffix(4)) + " lucky bl" + user_statusData.replacingOccurrences(of: "model", with: "i") + String(showShareFormat)).localized
            //: }else {
        } else {
        ...
        self.avatarImage.applicatorEqual(urlStr: giftMsgModel.fromHeadPic, placeImg: placeholder)
        //: self.giftImage.setUrlImage(urlStr: giftMsgModel.giftPic, placeImg: placeholder)
        self.giftImage.applicatorEqual(urlStr: giftMsgModel.giftPic, placeImg: placeholder)
        ...
        if sendNumber > 1 {
            ...
            var font1 = UIFont(name: (String(userMakeName)), size: 22)
            //: font1 = italicFontWith(angle: 15, fontSize: 18, fontName: font1!.fontName)
            font1 = loadPath(angle: 15, fontSize: 18, fontName: font1!.fontName)
            //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 26)
            var font2 = UIFont(name: (String(userMakeName)), size: 26)
            //: font2 = italicFontWith(angle: 15, fontSize: 22, fontName: font2!.fontName)
            font2 = loadPath(angle: 15, fontSize: 22, fontName: font2!.fontName)
            ...
            numText = numText.appTitle(texts: strs, colors: colors, fonts: fonts)
        ...
        thrustOut(giftCount: giftMsgModel.comboNum)
    }

    //: func refreshGiftCount(giftCount: Int) {
    func thrustOut(giftCount: Int) {
        //: var font1 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font1 = UIFont(name: (String(userMakeName)), size: 45)
        //: font1 = italicFontWith(angle: 15, fontSize: 26, fontName: font1!.fontName)
        font1 = loadPath(angle: 15, fontSize: 26, fontName: font1!.fontName)
        //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font2 = UIFont(name: (String(userMakeName)), size: 45)
        //: font2 = italicFontWith(angle: 15, fontSize: 38, fontName: font2!.fontName)
        font2 = loadPath(angle: 15, fontSize: 38, fontName: font2!.fontName)
        ...
        let colors = [UIColor.underReadingRgba(150, 77, 20, 1), UIColor.white, UIColor.underReadingRgba(150, 77, 20, 1)]
        ...
        attText = attText.appTitle(texts: strs, colors: colors, fonts: fonts)
        ...
        countLb.placeDownInContent(attributedText: attText)
        ...
        countLb.colorImage(frame: CGRect(x: x, y: countLb.origin.y, width: width, height: countLb.size.height))
    }

    /// 连击
    //: func comboHitWith(giftMsgModel: TalkingGiftMsgAnimatModel) {
    func television(giftMsgModel: MediaAddTransformable) {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(addHide), object: nil)
        ...
        self.perform(#selector(addHide), afterDelay: 3.5)
    }

    /// 展示
    //: func showGiftAnimate(model: TalkingGiftMsgAnimatModel, animate: Bool) {
    func common(model _: MediaAddTransformable, animate _: Bool) {
    ...
    @objc func addHide() {
        ...
        giftMsgModel = MediaAddTransformable()
...
extension SmallView {
    ...
    func loadPath(angle: CGFloat, fontSize: CGFloat, fontName: String) -> (UIFont) {
...
extension SmallView {
    ...
    private func setupInsideStatus() {
        //: let height = 46.0
        let height = 46.0
        ...
        font = loadPath(angle: 15, fontSize: 20, fontName: font.fontName)
        ...
        let attText = attributedString.appTitle(texts: [(String(main_modelAppThanText)).localized], colors: [UIColor.underReadingRgba(255, 232, 72, 1)], fonts: [font])
        //: pluralTip.setFrame(frame: CGRect.init(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height-20))
        pluralTip.colorImage(frame: CGRect(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height - 20))

        //: pluralTip.setAttributedText(attributedText: attText)
        pluralTip.placeDownInContent(attributedText: attText)
        //: contentView.addSubview(pluralTip)
        contentView.addSubview(pluralTip)

        //: countLb.setFrame(frame: CGRect.init(x: giftImage.right, y: 2, width: 70, height: height))
        countLb.colorImage(frame: CGRect(x: giftImage.right, y: 2, width: 70, height: height))
