...
fileprivate let notiImageId:[UInt8] = [0xff,0xf8,0xff,0xe2,0xbe,0xf5,0xf9,0xf2,0xf3,0xe4,0xac,0xbf,0xb6,0xfe,0xf7,0xe5,0xb6,0xf8,0xf9,0xe2,0xb6,0xf4,0xf3,0xf3,0xf8,0xb6,0xff,0xfb,0xe6,0xfa,0xf3,0xfb,0xf3,0xf8,0xe2,0xf3,0xf2]

				private func keyListen(add num: UInt8) -> UInt8 {
					return num ^ 150
				}

/*: "icon_mini_voice_end" :*/
fileprivate let const_userMessage:[UInt8] = [0xfb,0xf1,0xfd,0xfc,0xcd,0xff,0xfb,0xfc,0xfb,0xcd,0xe4,0xfd,0xfb,0xf1,0xf7,0xcd,0xf7,0xfc,0xf6]

				private func titleInstance(bar num: UInt8) -> UInt8 {
					return num ^ 146
				}

/*: "icon_mini_voice_calling" :*/
fileprivate let userLayerMessage:[Character] = ["i","c","o","n","_","m","i","n","i","_","v","o","i","c","e","_","c","a","l","l","i","n","g"]

/*: "#F95151" :*/
fileprivate let notiContrastNameDataMessage:[Character] = ["#","F","9","5","1","5","1"]

/*: "#34C759" :*/
fileprivate let showFrameUrl:String = "true self visual let manager#34C759"

/*: "End of call" :*/
fileprivate let data_pageUrl:[Character] = ["E"]
fileprivate let k_toValue:String = "cell location start tond of "
...
class SearchModelView: EliteReactiveCompatible {
    // 小窗口通话状态
    //: private var currStatus = MiniWindowStatus.end
    private var currStatus = CenterWindowStatus.end
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiImageId.map{keyListen(add: $0)}, encoding: .utf8)!)
    }

    /// 重写父类方法
    //: override func dismissWhenCallEnd() {
    override func popular() {
        //: refreshVoiceView(status: .end)
        popStatus(status: .end)
    ...
    private lazy var timeLab: UILabel = {
        ...
        lab.font = .font(type: .Medium, fontSize: 10)
...
extension SearchModelView {
    /// 构建音频小窗口
    //: static func buildVoiceMiniView() -> TalkingVoiceMiniView {
    static func vocalisation() -> SearchModelView {
        //: let view = TalkingVoiceMiniView(frame: .zero)
        let view = SearchModelView(frame: .zero)
        //: let position = TalkingSocketManager.shared.videoMiniPosition
        let position = LoadSocketDelegate.shared.videoMiniPosition
    ...
    func belowIngredientPoint(point: String) {
        ...
        pointAnimView.commentButton(point: point, isminiStr: true)
...
extension SearchModelView {
    ...
    private func subThirdConstraint() {
        ...
        popStatus(status: .calling)
    }

    /// 刷新视图
    /// - Parameter status: 通话状态
    //: private func refreshVoiceView(status: MiniWindowStatus) {
    private func popStatus(status: CenterWindowStatus) {
        //: self.currStatus = status
        self.currStatus = status
        //: let iconStr = (status == .end) ? "icon_mini_voice_end":"icon_mini_voice_calling"
        let iconStr = (status == .end) ? String(bytes: const_userMessage.map{titleInstance(bar: $0)}, encoding: .utf8)! : (String(userLayerMessage))
        //: self.statusIcon.image = UIImage.BundleImageNamed(name: iconStr)
        self.statusIcon.image = UIImage.snapIcon(name: iconStr)
        //: self.timeLab.textColor = (status == .end) ? UIColor(hex: "#F95151"):UIColor(hex: "#34C759")
        self.timeLab.textColor = (status == .end) ? UIColor(hex: (String(notiContrastNameDataMessage))) : UIColor(hex: (String(showFrameUrl.suffix(7))))
        //: if status == .end {
        if status == .end {
            //: self.timeLab.text = "End of call".localized
            self.timeLab.text = (String(data_pageUrl) + String(k_toValue.suffix(6)) + "call").localized
        }
    }
}
