...
fileprivate let user_tempMsg:String = "TRTC 视频\u{901a}话"
fileprivate let showKitPath:String = "向SDK发送采appearance lab false tool string"
fileprivate let noti_externalFormat:[Character] = ["集","的","视","频","帧","\u{56fe}","片",":"," "]

/*: "LIVE 直播向SDK发送采集的视频帧图片:  :*/
fileprivate let showMakeOldFormat:String = "LIVE equal share case type var"
fileprivate let dataToolStr:String = "直model向"
fileprivate let kReloadFormat:String = "SDK发model cell field"
fileprivate let showFileMainLabelValue:String = "帧图net: "
...
class NameReactiveCompatible: UIView {
    ...
    private lazy var captureView: ErrorView = {
...
extension NameReactiveCompatible {
    ...
    func picSlenderLiveStartVanguardWindow(isSmallWindow: Bool, livePusher: TXLivePush? = nil) {
        ...
        if self.captureView.superview == nil {
            ...
            self.captureView.stCamera.start()
        }
        //: self.captureView.setupVideoConversationPreviewSize(isSmallWindow)
        self.captureView.curTelephoneConversation(isSmallWindow)
    }
...
extension NameReactiveCompatible: ParentDelegete {
    // TRTC推流
    //: func stVideoBeautyView(_ beautyView: STFilterView!, didProcessVideoSampleBuffer sampleBuffer: CVPixelBuffer!) {
    func filter(_: ErrorView!, capability sampleBuffer: CVPixelBuffer!) {
        ...
        if trtcCallCount >= logReportInterval {
            ...
            if let image = changeSearch(sampleBuffer) {
                //: UploadLogTool.writeLog(msg: "TRTC 视频通话向SDK发送采集的视频帧图片: \(image.size)")
                TitleSongLogTool.dataRender(msg: (user_tempMsg + String(showKitPath.prefix(7)) + String(noti_externalFormat)) + "\(image.size)")
            }
            //: trtcCallCount = 0
            trtcCallCount = 0
        }
    ...
    func exposure(_: ErrorView!, render sampleBuffer: CMSampleBuffer!) {
        //: if livePusher != nil {
        if livePusher != nil {
            ...
            if liveCallCount >= logReportInterval {
                ...
                if let imageBuffer = CMSampleBufferGetImageBuffer(sampleBuffer) {
                    ...
                    if let image = changeSearch(imageBuffer) {
                        //: UploadLogTool.writeLog(msg: "LIVE 直播向SDK发送采集的视频帧图片: \(image.size)")
                        TitleSongLogTool.dataRender(msg: (String(showMakeOldFormat.prefix(5)) + dataToolStr.replacingOccurrences(of: "model", with: "播") + String(kReloadFormat.prefix(4)) + "送采集的视频" + showFileMainLabelValue.replacingOccurrences(of: "net", with: "片")) + "\(image.size)")
                    }
                }
    ...
    func changeSearch(_ imageBuffer: CVImageBuffer) -> UIImage? {
