...
fileprivate let notiMaxText:[UInt8] = [0x92,0x97,0x92,0x9d,0x51,0x8c,0x98,0x8d,0x8e,0x9b,0x63,0x52,0x49,0x91,0x8a,0x9c,0x49,0x97,0x98,0x9d,0x49,0x8b,0x8e,0x8e,0x97,0x49,0x92,0x96,0x99,0x95,0x8e,0x96,0x8e,0x97,0x9d,0x8e,0x8d]

				fileprivate func socialError(model num: UInt8) -> UInt8 {
...
// __CLOSE_PRINT__
//
//  LimitedViewReactiveCompatible.swift
//  AbroadTalking
//
...
class LimitedViewReactiveCompatible: UIView {
    //: var callBlock: ((_ type: CallMenuType) -> Void)?
    var callBlock: ((_ type: VoicePriorityTarget) -> Void)?
    //: private var popView: TalkingPopView?
    private var popView: RecordPopView?
    //: private var content = "" // 展示内容
    private var content = "" // 展示内容
    //: private var type = CallMenuType.VideoCall
    private var type = VoicePriorityTarget.VideoCall // 类型

    //: init(type: CallMenuType, content: String) {
    init(type: VoicePriorityTarget, content: String) {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiMaxText.map{socialError(model: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var topIcon: UIImageView = {
        ...
        iamg.image = UIImage.snapIcon(name: "bg_vip")
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(executeRow), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension LimitedViewReactiveCompatible {
    //: func show() {
    func logWith() {
