...
/*: "#BAFBFF" :*/
fileprivate let mainMakeMessage:[Character] = ["#","B"]
fileprivate let notiLengthAddId:String = "AcontentFF"
...
// __CLOSE_PRINT__
//
//  HostessReactiveCompatible.swift
//
...
class HostessReactiveCompatible: NSObject {
    ...
    var model = EasingMediaTransformable()

    //: func caculateMsgHeight(model: TalkingVideoCallDanmuModel) -> TalkingVideoCallDanmuModel {
    func busy(model: EasingMediaTransformable) -> EasingMediaTransformable {
        //: self.model = model
        self.model = model
        /// 普通文本消息
    ...
    func trans(model: EasingMediaTransformable) -> EasingMediaTransformable {
        //: self.model = model
        self.model = model
        /// 普通文本消息
        //: if self.model.messageType == 1 {
    ...
    func basilarArtery() {
        ...
        let name = prolusion(text: firstStr, font: UIFont.dismissSharedSize(fontSize: 14), color: UIColor(hex: (String(mainMakeMessage) + notiLengthAddId.replacingOccurrences(of: "content", with: "FB")))!)
        // 内容
        //: let content = getAttributed(text: self.model.content, font: UIFont.pingfangMediumFont(fontSize: 14), color: UIColor.white)
        let content = prolusion(text: self.model.content, font: UIFont.dismissSharedSize(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        ...
        fromWith(attribText: self.model.msgAttribText)
    }
    ...
    func commentOf() {
        ...
        let content = prolusion(text: self.model.transContent, font: UIFont.dismissSharedSize(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        //: name.yy_paragraphStyle = paraStyle
        name.yy_paragraphStyle = paraStyle
    ...
    func requestStyle() -> NSMutableParagraphStyle {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = requestStyle()
        //: return attribute
        return attribute
    }
    ...
    func fromWith(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = VideoMsgTableViewWidth - 36
        let maxWidth = show_intervalTitle - 36
