...
/*: "call_bgm" :*/
fileprivate let appTextValue:[Character] = ["c","a","l","l","_","b","g","m"]

/*: "call_response_bgm" :*/
fileprivate let const_userText:String = "content size truecall_re"
fileprivate let showPriceFrameUrl:String = "modeonse"
...
// __CLOSE_PRINT__
//
//  BackTextReactiveCompatible.swift
//  MLProject
//
...
class BackTextReactiveCompatible: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = SectionQuickLookable.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func typeFirst(type: SectionQuickLookable) -> BackTextReactiveCompatible {
        //: let player = TalkingCallBGMPlayer()
        let player = BackTextReactiveCompatible()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.tick()
...
extension BackTextReactiveCompatible {
    //: func play() {
    func pointPlay() {
    ...
    private func bullyDoingTo(notify: Notification) {
    ...
    private func startSize(notify: Notification) {
    ...
    private func birthExternalBodyPart() -> Bool {
    ...
    private func tick() {
        ...
        var fileName = (String(appTextValue))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(const_userText.suffix(7)) + showPriceFrameUrl.replacingOccurrences(of: "mode", with: "sp") + "_bgm")
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = ValueReactiveCompatible.default.scaleOfMeasurementEqual(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension BackTextReactiveCompatible: TXVodPlayListener {
