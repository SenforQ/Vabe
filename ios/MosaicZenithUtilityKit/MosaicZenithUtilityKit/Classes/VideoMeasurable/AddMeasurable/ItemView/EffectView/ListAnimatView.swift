...
fileprivate let showThenName:[UInt8] = [0xf0,0xf7,0xf0,0xed,0xb1,0xfa,0xf6,0xfd,0xfc,0xeb,0xa3,0xb0,0xb9,0xf1,0xf8,0xea,0xb9,0xf7,0xf6,0xed,0xb9,0xfb,0xfc,0xfc,0xf7,0xb9,0xf0,0xf4,0xe9,0xf5,0xfc,0xf4,0xfc,0xf7,0xed,0xfc,0xfd]

				private func addGo(currency num: UInt8) -> UInt8 {
					return num ^ 153
				}

/*: "iosSvgaEffect" :*/
fileprivate let mainEyeValue:String = "image return main atiosS"
fileprivate let mainScreenValue:String = "guard withvgaEf"

/*: "iosEffect" :*/
fileprivate let constTextUrl:String = "iosEffectapp back announcement out"

/*: "entityId" :*/
fileprivate let data_imageName:String = "entityIdtitle icon will up"
...
class ListAnimatView: UIView {
    //: var currentEffectMsgModel: TalkingAnimatMsgModel?
    var currentEffectMsgModel: AddMeasurable?
    ...
    var animatedView: ItemView?

    //: open weak var delegate: AnimatViewStepProtocol?
    open weak var delegate: StepThen?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showThenName.map{addGo(currency: $0)}, encoding: .utf8)!)
    }
...
extension ListAnimatView {
    //: func playAnimation(infoDic: Dictionary<String, Any>) {
    func redModel(infoDic: [String: Any]) {
        //: let dict = infoDic
        let dict = infoDic
        //: if dict.keys.isEmpty == false {
        if dict.keys.isEmpty == false {
            //: var iosEffect: String = dict["iosSvgaEffect"] as! String
            var iosEffect: String = dict[(String(mainEyeValue.suffix(4)) + String(mainScreenValue.suffix(5)) + "fect")] as! String
            //: if iosEffect.isEmptyString {
            if iosEffect.isEmptyString {
                //: iosEffect = dict["iosEffect"] as! String
                iosEffect = dict[(String(constTextUrl.prefix(9)))] as! String
            }

            //: let entityId = dict["entityId"] as! Int
            let entityId = dict[(String(data_imageName.prefix(8)))] as! Int
            ...
            if iosEffect.isEmptyString == false {
                ...
                let successs = undo(msgModel: effectMsgModel)
            }
        }
    }
    ...
    func undo(msgModel: Any?) -> Bool {
        ...
        if msgModel is AddMeasurable {
            //: effectMsgModel = msgModel as? TalkingAnimatMsgModel
            effectMsgModel = msgModel as? AddMeasurable
            //: } else if msgModel is TalkingGiftMsgAnimatModel {
        } else if msgModel is MediaAddTransformable {
            //: let giftMsgModel = msgModel as! TalkingGiftMsgAnimatModel
            let giftMsgModel = msgModel as! MediaAddTransformable
            ...
            if giftMsgModel.animationTimes > 1 {
                ...
                tmpGiftMsgModel.pathTag(giftNum: 1) // 礼物个数设置为1，每次动效显示
                ...
                var repeatEfftect = AddMeasurable()
            ...
            if giftMsgModel.showType == ResumeVarArg.myStery.rawValue || giftMsgModel.showType == ResumeVarArg.king.rawValue {
        ...
        if self.currentEffectMsgModel != nil || paused {
            ...
            if effectMsgModel?.top ?? false && self.effectMsgModelArray.count > 0 {
                ...
                for i in 0 ..< self.effectMsgModelArray.count {
                    ...
                    let aModel: AddMeasurable = self.effectMsgModelArray[i] as! AddMeasurable
                    //: if aModel.top == false {
                    if aModel.top == false {
                        //: index = i
                        index = i
                ...
                if index == -1 {
                    ...
                    self.effectMsgModelArray.add(effectMsgModel ?? AddMeasurable())
                    //: } else {
                } else {
                    //: self.effectMsgModelArray.insert(effectMsgModel ?? TalkingAnimatMsgModel.init(), at: index)
                    self.effectMsgModelArray.insert(effectMsgModel ?? AddMeasurable(), at: index)
                }
            ...
            doTo(effectMsgModel: effectMsgModel!)
        }
...
extension ListAnimatView {
    ...
    func slow() {
        ...
        if self.currentEffectMsgModel == nil {
            ...
            if self.effectMsgModelArray.count > 0 {
                ...
                guard let removeModel = database() else { return }
                //: firstShow(effectMsgModel: removeModel)
                doTo(effectMsgModel: removeModel)
            }
            //: } else {
        } else {
            //: self.animatedView?.resumeAnimation()
            self.animatedView?.modelPlayRecord()
        }
    ...
    func database() -> AddMeasurable? {
        //: if (self.effectMsgModelArray.count == 0) {
        if self.effectMsgModelArray.count == 0 {
            //: return TalkingAnimatMsgModel.init()
            return AddMeasurable()
        }
        ...
        if effectMsgModel is AddMeasurable {
            //: let lastEffectMsgModel = effectMsgModel as! TalkingAnimatMsgModel
            let lastEffectMsgModel = effectMsgModel as! AddMeasurable
        ...
        return effectMsgModel as? AddMeasurable
    }
}
...
extension ListAnimatView {
    ...
    func doTo(effectMsgModel: AddMeasurable) {
        ...
        self.delegate?.straddleColor(effectView: self, obj: effectMsgModel)

        //: TalkingGiftAnimatTool.shared.loadEffectData(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
        ListAnimatTool.shared.prod(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
            ...
            self.detoxify(effectMsgModel: effectMsgModel, success: success, effectModel: giftModel ?? AnthemAnimatModel())
            //: })
        })
    }
    ...
    func detoxify(effectMsgModel: AddMeasurable, success: Bool, effectModel: AnthemAnimatModel) {
        // 资源下载完成
        //: self.delegate?.animatViewWithDidEndLoadingResource(effectView: self, obj: effectMsgModel, success: success)
        self.delegate?.glitter(effectView: self, obj: effectMsgModel, success: success)
        ...
        if success {
            ...
            if self.paused {
                ...
                if self.currentEffectMsgModel?.eid == effectMsgModel.eid, self.currentEffectMsgModel?.effectUrl == effectMsgModel.effectUrl {
                    ...
                    inputTo(effectMsgModel: effectMsgModel, model: effectModel)
                    //: return
                    return
                }
            }
    ...
    func skim(effectMsgModel: AddMeasurable, effectModel: AnthemAnimatModel) {
        //: if self.animatedView != nil {
        if self.animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.everyEqual()
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.cleanUpAnimating()
        ...
        className = ListAnimatTool.shared.inputNew(oldName: className ?? "")
        ...
        let realClass = AnimatedViewClass as! ItemView.Type
...
extension ListAnimatView: LightViewDelegate {
    ...
    func theoreticalAccount(effectItemView: ItemView, success: Bool) {
        ...
        self.delegate?.attitude(effectView: self, obj: effectItemView.effectMsgModel ?? AddMeasurable(), success: success)
    }
    ...
    func writeTarget(effectItemView: ItemView) {
        ...
        self.delegate?.devicePanoramicView(effectView: self, obj: effectMsgModel!)
    }
}
...
extension ListAnimatView {
    ...
    func totalCell() {
        ...
        if animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.everyEqual() // stop animation
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.cleanUpAnimating() // clean animation data
