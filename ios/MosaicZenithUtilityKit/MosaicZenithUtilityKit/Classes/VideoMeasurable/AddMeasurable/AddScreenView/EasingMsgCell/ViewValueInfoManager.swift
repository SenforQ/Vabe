...
/*: "serialQueue" :*/
fileprivate let k_giftPath:String = "SERI"
fileprivate let data_screenKey:String = "alQueuelet if"

/*: "fromUids" :*/
fileprivate let show_logMakeModelId:[UInt8] = [0x10,0x4,0x19,0x1b,0x23,0x1f,0x12,0x5]

//: Declare String End

// __DEBUG__
// __CLOSE_PRINT__
//  ViewValueInfoManager.swift
//  AbroadTalking
//
...
let userRecordExploreValue = 50

//: @objcMembers
@objcMembers
//: public class DBUserInfoManager: NSObject {
public class ViewValueInfoManager: NSObject {
    ...
    private static var _instance: ViewValueInfoManager?
    //: class func shared() -> DBUserInfoManager {
    class func sendFor() -> ViewValueInfoManager {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = DBUserInfoManager()
            _instance = ViewValueInfoManager()
            // 添加退出账号通知
            //: NotificationCenter.default.addObserver(_instance!,
            NotificationCenter.default.addObserver(_instance!,
                                                   //: selector: #selector(accountLogout),
                                                   selector: #selector(underErrorAccept),
    ...
    @objc private func underErrorAccept() {
        //: DBUserInfoManager.destroy()
        ViewValueInfoManager.paradigmError()
    }
}
...
extension ViewValueInfoManager {
    /// 获取用户消息
    /// - Parameters:
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: class func cache_getUserInfo(with userIdsArr: [String], completion: @escaping ([BivouacCacheModel]?, Error?) -> Void) {
    class func playWith(with userIdsArr: [String], completion: @escaping ([BivouacCacheModel]?, Error?) -> Void) {
        // 数量过多，wcdb的where语句会因过长而报错，所以做分页
        //: var newUserIdsArr = userIdsArr
        var newUserIdsArr = userIdsArr
        //: if DBUserInfoManager.shared().needReqUidsSet != nil {
        if ViewValueInfoManager.sendFor().needReqUidsSet != nil { // 合并去重
            //: newUserIdsArr = Array(DBUserInfoManager.shared().needReqUidsSet!.union(userIdsArr))
            newUserIdsArr = Array(ViewValueInfoManager.sendFor().needReqUidsSet!.union(userIdsArr))
            //: DBUserInfoManager.shared().needReqUidsSet = nil
            ViewValueInfoManager.sendFor().needReqUidsSet = nil
        }
        ...
        let pageSize = userRecordExploreValue
        //: repeat {
        repeat {
            ...
            goOff(with: subArr, completion: completion)
            //: location += len
            location += len
    ...
    class func substitute(with userArr: [String], completion: @escaping ([[String: Any]]?, Error?) -> Void) {
        ...
        let serialQueue = DispatchQueue(label: (k_giftPath.lowercased() + String(data_screenKey.prefix(7))))
        ...
        serialQueue.async {
            ...
            for item in finalArray {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                    //: SizeRequestTool.req_queryStatus(toUids: requestUidsStr) { _, list in
                    SizeRequestTool.queryCompletion(toUids: requestUidsStr) { _, list in
                        //: defer { sema.signal() }
                        defer { sema.signal() }
    ...
    public class func lineUp(message: V2TIMMessage) -> ImageLoadReactiveCompatible {
        //: let key = TUISocialChatManager.checkPrimaryKey(withMsgModel: message)
        let key = TitleChatManager.provenance(withMsgModel: message)
        //: var cacheWrap = WCDBVoiceMsgTable.db_getVoiceMsg(with: key)
        var cacheWrap = ImageLoadReactiveCompatible.infoSound(with: key)
        //: if cacheWrap == nil {
        if cacheWrap == nil {
            //: let info = TUISocialChatManager.packageDict(withTXMessage: message)
            let info = TitleChatManager.decorticate(withTXMessage: message)
            //: cacheWrap = WCDBVoiceMsgTable.db_inserVoiceMsgToDB(info)
            cacheWrap = ImageLoadReactiveCompatible.conversation(info)
        }
        //: return cacheWrap!
        return cacheWrap!
    }
...
extension ViewValueInfoManager {
    ...
    //: @discardableResult
    @discardableResult
    //: class func cache_getCachedUserInfo(targetId: String) -> BivouacCacheModel? {
    class func cache(targetId: String) -> BivouacCacheModel? {
        //: return WCDBUserInfoTable.db_getUserInfo(with: targetId)
        return FileInfoTable.warpath(with: targetId)
    }

    /// 更新数据库【用户模型】，并发送通知
    /// - Parameter userInfo: 用户信息model
    //: class func cache_updateMsgUserInfoAndPostNotif(with userInfo: BivouacCacheModel?) {
    class func userSize(with userInfo: BivouacCacheModel?) {
        ...
        let model = ViewValueInfoManager.cache(targetId: userInfo.uid)
        // 自定义字段需要赋值，防止被覆盖
        ...
        FileInfoTable.gestureStart(userInfo)
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: NotificationCenter.default.post(name: USER_INFO_MSG_LIST_NOTIFICATION, object: [userInfo.uid: userInfo])
            NotificationCenter.default.post(name: userMessageClickKey, object: [userInfo.uid: userInfo])
        }
    }
    ...
    class func submit(with targetId: String, intimate: Int, currTime: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = ViewValueInfoManager.cache(targetId: targetId) else { return }
    ...
    class func listFromInstance(with targetId: String, time: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = ViewValueInfoManager.cache(targetId: targetId) else { return }
    ...
    class func giftWith(with targetUid: String) {
        //: WCDBUserInfoTable.db_deleteUserInfo(with: targetUid)
        FileInfoTable.paintTheLilyWith(with: targetUid)
        //: DBUserInfoManager.shared().reqedUidsSet.remove(targetUid)
        ViewValueInfoManager.sendFor().reqedUidsSet.remove(targetUid)
    }
}
...
extension ViewValueInfoManager {
    ...
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: private class func inner_getUserInfo(with userIdsArr: [String], completion: @escaping ([BivouacCacheModel]?, Error?) -> Void) {
    private class func goOff(with userIdsArr: [String], completion: @escaping ([BivouacCacheModel]?, Error?) -> Void) {
        //: let userModels = WCDBUserInfoTable.db_getUserInfos(with: userIdsArr)
        let userModels = FileInfoTable.mainTargetPlace(with: userIdsArr)
        ...
        for uid in userIdsArr where !userModels.contains(where: { $0.uid == uid }) {
            ...
            if ViewValueInfoManager.sendFor().reqedUidsSet.contains(uid) == false {
        ...
        if !noDBUidArr.isEmpty {
            ...
            ViewValueInfoManager.pastResponse(with: noDBUidArr)
        }
    }
    ...
    private class func pastResponse(with userIdsArr: [String]) {
        ...
        ViewValueInfoManager.sendFor().reqedUidsSet = ViewValueInfoManager.sendFor().reqedUidsSet.union(userIdsArr)

        // 请求用户消息列表
        //: SizeRequestTool.req_refreshMsgUserListInfo(param: ["fromUids": requestUidsStr]) { succeed, dataArr in
        SizeRequestTool.imageKey(param: [String(bytes: show_logMakeModelId.map{$0^118}, encoding: .utf8)!: requestUidsStr]) { succeed, dataArr in
            //: guard succeed else {
            guard succeed else { // 失败还原数据
                //: DBUserInfoManager.shared().reqedUidsSet.subtract(userIdsArr)
                ViewValueInfoManager.sendFor().reqedUidsSet.subtract(userIdsArr)
                //: return
                return
            }
            // 更新数据库
            //: DBUserInfoManager.updateMsgUserInfoTable(userInfoArr: dataArr)
            ViewValueInfoManager.makeEvent(userInfoArr: dataArr)
            // 发送通知
            //: var retInfoDict: [String: BivouacCacheModel] = [:]
            var retInfoDict: [String: BivouacCacheModel] = [:]
            ...
            if !retInfoDict.isEmpty {
                ...
                DispatchQueue.main.async {
                    ...
                    NotificationCenter.default.post(name: userMessageClickKey, object: retInfoDict)
                }
            }
        }
    }
    ...
    //: private class func updateMsgUserInfoTable(userInfoArr: [BivouacCacheModel]) {
    private class func makeEvent(userInfoArr: [BivouacCacheModel]) {
        //: for aWrap in userInfoArr {
        for aWrap in userInfoArr {
            //: WCDBUserInfoTable.db_updateUserInfo(aWrap)
            FileInfoTable.gestureStart(aWrap)
        }
    }
}
