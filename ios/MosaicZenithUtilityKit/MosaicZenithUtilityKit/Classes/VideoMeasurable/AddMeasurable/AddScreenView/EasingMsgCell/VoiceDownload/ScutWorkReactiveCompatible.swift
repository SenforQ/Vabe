...
fileprivate let constShowUrl:[Character] = ["D","o","c","u","m"]
fileprivate let user_servicePath:[Character] = ["e","n","t","s","/","U","s","e","r","/","v"]
fileprivate let data_toName:String = "oice/edit lab value"

/*: "Documents/User/record/" :*/
fileprivate let dataContentMessage:String = "self withDocum"
fileprivate let data_pathName:String = "r/rectrue if make"

/*: "cancelled" :*/
fileprivate let const_randomName:String = "messagean"
fileprivate let kWithKey:String = "cetotoed"
...
// __CLOSE_PRINT__
//
//  ScutWorkReactiveCompatible.swift
...
typealias OneKeyGreetDownloadBlock = (_ model: FashionTaskModel) -> Void

//: protocol VoiceDownloadTaskManagerDelegate: NSObjectProtocol {
protocol LabManagerDelegate: NSObjectProtocol {
    /// 下载完成
    ...
    func seem(model: FashionTaskModel)
    /// 过期语音消息，服务器缓存时间为7天
...
extension ScutWorkReactiveCompatible {
    /// 添加下载任务
    //: func addDownloadTasks(_ tasksArray: [VoiceDownloadTaskModel]) {
    func appEnd(_ tasksArray: [FashionTaskModel]) {
        ...
        for model in filterArray {
            ...
            if model.zipFilePath.isEmpty {
                ...
                model.zipFilePath = model.listItem()
            }

            // 重编译，相对路径会变
            //: if model.zipFilePath.contains("Documents/User/voice/") {
            if model.zipFilePath.contains((String(constShowUrl) + String(user_servicePath) + String(data_toName.prefix(5)))) {
                //: model.zipFilePath = "\(DataCacheRedefineReactiveCompatible.getChatVoicePath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(DataCacheRedefineReactiveCompatible.currentSmall())\((model.zipFilePath as NSString).lastPathComponent)"
                //: } else if model.zipFilePath.contains("Documents/User/record/") {
            } else if model.zipFilePath.contains((String(dataContentMessage.suffix(5)) + "ents/Use" + String(data_pathName.prefix(5)) + "ord/")) {
                //: model.zipFilePath = "\(DataCacheRedefineReactiveCompatible.getChatRecordPath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(DataCacheRedefineReactiveCompatible.rid())\((model.zipFilePath as NSString).lastPathComponent)"
            }
            ...
            ImageLoadReactiveCompatible.diagonal(msgId: model.taskId, status: "\(GroupCustomReflectable.prepare.rawValue)", filePath: model.zipFilePath)
    ...
    func sendHandle(_ audioInfo: [String: Any], completeHandle: @escaping OneKeyGreetDownloadBlock) {
        //: var taskModel = VoiceDownloadTaskModel.getOneKeyGreetModel(audioInfo)
        var taskModel = FashionTaskModel.ane(audioInfo)
        ...
        if self.downloadingModelDic.keys.contains(taskModel.downloadUri) {
            ...
            taskModel = self.downloadingModelDic[taskModel.downloadUri] ?? FashionTaskModel()
        }

        // 重编译，相对路径会变
        //: if taskModel.zipFilePath.contains("Documents/User/voice/") {
        if taskModel.zipFilePath.contains((String(constShowUrl) + String(user_servicePath) + String(data_toName.prefix(5)))) {
            //: taskModel.zipFilePath = "\(DataCacheRedefineReactiveCompatible.getChatVoicePath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(DataCacheRedefineReactiveCompatible.currentSmall())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            //: } else if taskModel.zipFilePath.contains("Documents/User/record/") {
        } else if taskModel.zipFilePath.contains((String(dataContentMessage.suffix(5)) + "ents/Use" + String(data_pathName.prefix(5)) + "ord/")) {
            //: taskModel.zipFilePath = "\(DataCacheRedefineReactiveCompatible.getChatRecordPath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(DataCacheRedefineReactiveCompatible.rid())\((taskModel.zipFilePath as NSString).lastPathComponent)"
        }
...
extension ScutWorkReactiveCompatible: URLSessionDataDelegate {
    // 开始
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive _: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        ...
        let totalBytesWritten = FashionTaskModel.destination(path: taskModel.zipFilePath)
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        self.delegate?.seem(model: taskModel)
    }
    ...
    func urlSession(_: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if error != nil || errorCode != 200 {
            ...
            if let error = error, (error as NSError).userInfo[NSLocalizedDescriptionKey] as? String == (const_randomName.replacingOccurrences(of: "message", with: "c") + kWithKey.replacingOccurrences(of: "to", with: "l")) {
                ...
                if taskModel.taskStatus == .pause {
                    ...
                    self.pushUrl(url: task.taskDescription ?? "")
                }
                //: } else {
            } else {
            ...
            self.completeName(url: task.taskDescription ?? "")
        }
...
extension ScutWorkReactiveCompatible {
    ...
    func dangle(url: String, isSuspend: Bool) {
        ...
        case .chatRecord:
            ...
            let status = isSuspend ? "\(GroupCustomReflectable.pause.rawValue)" : "\(GroupCustomReflectable.error.rawValue)"
            //: WCDBVoiceMsgTable.db_updateVoiceMsg(msgId: taskModel.taskId, status: status)
            ImageLoadReactiveCompatible.diagonal(msgId: taskModel.taskId, status: status)
            //: if isSuspend == false {
            if isSuspend == false {
                //: self.delegate?.voiceDownloadError(model: taskModel)
                self.delegate?.exceptEventModel(model: taskModel)
            }
    ...
    func pushUrl(url: String) {
        ...
        case .chatRecord:
            ...
            ImageLoadReactiveCompatible.deleteControlId(msgId: taskModel.taskId)
            //: self.delegate?.voiceDownloadCancel(model: taskModel)
            self.delegate?.orientation(model: taskModel)
    ...
    func completeName(url: String) {
        ...
        let size = FashionTaskModel.destination(path: taskModel.zipFilePath)
        //: if size > 0 {
        if size > 0 {
            ...
            if taskModel.taskTypeId == .chatRecord {
                ...
                ImageLoadReactiveCompatible.diagonal(msgId: taskModel.taskId, status: "\(GroupCustomReflectable.finish.rawValue)")
            }
            ...
            self.colorCellSize(at: taskModel.zipFilePath, withKeyId: taskModel.taskId, withTypeId: taskModel.taskTypeId) { _ in
                //: switch taskModel.taskTypeId {
                switch taskModel.taskTypeId {
                //: case .chatRecord:
                case .chatRecord:
                    //: self.delegate?.voiceDownloadFinish(model: taskModel)
                    self.delegate?.receiveFromModel(model: taskModel)
            ...
            case .chatRecord:
                ...
                ImageLoadReactiveCompatible.diagonal(msgId: taskModel.taskId, status: "\(GroupCustomReflectable.expired.rawValue)")
                //: self.delegate?.voiceDownloadExpired(model: taskModel)
                self.delegate?.expiredLive(model: taskModel)
    ...
    func colorCellSize(at path: String, withKeyId keyId: String, withTypeId typeId: DownloadPriorityTarget, complete: @escaping (Error?) -> Void) {
            ...
            if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                ...
                if let handle = FileHandle(forWritingAtPath: newPath) {
                    ...
                    if typeId == .chatRecord {
                        ...
                        ImageLoadReactiveCompatible.diagonal(msgId: keyId, filePath: newPath)
                    }
