...
/*: "DBUserInfoTable" :*/
fileprivate let app_modelData:String = "image ofDBUs"
fileprivate let notiToIntervalInTitle:[Character] = ["o","T","a","b","l","e"]

/*: : 更新数据失败。error： :*/
fileprivate let noti_viewTitle:[Character] = [":"," ","更","\u{65b0}","数","据","失","\u{8d25}","。","e","r","r"]
fileprivate let kIndexStyleKey:[Character] = ["o","r","："]

/*: : 批量插入数据失败。error： :*/
fileprivate let appTimeTitleFormat:[Character] = [":"," ","批","量","插"]
fileprivate let kRawMainId:String = "入view失"
...
let data_viewMsg = (String(app_modelData.suffix(4)) + "erInf" + String(notiToIntervalInTitle))
//: final class WCDBUserInfoTable: TableCodable {
final class FileInfoTable: TableCodable {
    /// 用户Id【主键】
    //: var uid: String = ""
    var uid: String = ""
    /// 昵称
...
// MARK: - BivouacCacheModel和WCDBUserInfoTable转换

//: extension WCDBUserInfoTable {
extension FileInfoTable {
    /// BivouacCacheModel  =>  FileInfoTable
    //: private class func mapToDBUserInfoData(_ userInfo: BivouacCacheModel) -> WCDBUserInfoTable {
    private class func videoOffRange(_ userInfo: BivouacCacheModel) -> FileInfoTable {
        //: let dbData = WCDBUserInfoTable()
        let dbData = FileInfoTable()
    ...
    /// [FileInfoTable] => [BivouacCacheModel]
    //: private class func mapToUserInfoModels(_ dbDatas: [WCDBUserInfoTable]) -> [BivouacCacheModel] {
    private class func viewPop(_ dbDatas: [FileInfoTable]) -> [BivouacCacheModel] {
        //: return dbDatas.map { data in
        return dbDatas.map { data in
            //: let userInfo = BivouacCacheModel()
            let userInfo = BivouacCacheModel()
...
extension FileInfoTable {
    ...
    class func paintTheLilyWith(with userId: String) {
        ...
        try? SingingVoiceManager.shared.database?.run(transaction: { _ in
            ...
            let condition = FileInfoTable.Properties.uid.in(userId)
            //: try? WCDBManager.shared.database?.delete(fromTable: WCDBUserInfoTableName,
            try? SingingVoiceManager.shared.database?.delete(fromTable: data_viewMsg,
                                                     //: where: condition)
                                                     where: condition)
            //: })
        })
    ...
    /// - Parameter userId: 用户id
    /// - Returns: 模型
    //: class func db_getUserInfo(with userId: String) -> BivouacCacheModel? {
    class func warpath(with userId: String) -> BivouacCacheModel? {
        //: let userInfos = WCDBUserInfoTable.db_getUserInfos(with: [userId])
        let userInfos = FileInfoTable.mainTargetPlace(with: [userId])
    ...
    /// - Returns: 数据数组
    //: class func db_getUserInfos(with userIds: [String]) -> [BivouacCacheModel] {
    class func mainTargetPlace(with userIds: [String]) -> [BivouacCacheModel] {
        //: guard userIds.count > 0 else { return [] }
        guard userIds.count > 0 else { return [] }
        //: do {
        do {
            //: let condition = WCDBUserInfoTable.Properties.uid.in(userIds)
            let condition = FileInfoTable.Properties.uid.in(userIds)
            ...
            let userInfos: [FileInfoTable]? = try SingingVoiceManager.shared.database?.getObjects(on: FileInfoTable.Properties.all, fromTable: data_viewMsg, where: condition)
            //: if userInfos?.isEmpty == false {
            if userInfos?.isEmpty == false {
                //: let models = WCDBUserInfoTable.mapToUserInfoModels(userInfos!)
                let models = FileInfoTable.viewPop(userInfos!)
                //: return models
                return models
            }
            //: } catch {
    ...
    //: class func db_updateUserInfo(_ userInfo: BivouacCacheModel) {
    class func gestureStart(_ userInfo: BivouacCacheModel) {
        //: let dbData = WCDBUserInfoTable.mapToDBUserInfoData(userInfo)
        let dbData = FileInfoTable.videoOffRange(userInfo)
        //: try? WCDBManager.shared.database?.run(transaction: { _ in
        try? SingingVoiceManager.shared.database?.run(transaction: { _ in
            // 数据库中已存在，则更新数据
            //: if WCDBUserInfoTable.db_getUserInfo(with: dbData.uid) != nil {
            if FileInfoTable.warpath(with: dbData.uid) != nil {
                //: do {
                do {
                    //: let condition = WCDBUserInfoTable.Properties.uid == dbData.uid
                    let condition = FileInfoTable.Properties.uid == dbData.uid
                    //: try WCDBManager.shared.database?.update(table: WCDBUserInfoTableName,
                    try SingingVoiceManager.shared.database?.update(table: data_viewMsg,
                                                            //: on: WCDBUserInfoTable.Properties.all,
                                                            on: FileInfoTable.Properties.all,
                                                            //: with: dbData,
                                                            with: dbData,
                                                            //: where: condition)
                                                            where: condition)
                    ...
                    TitleSongLogTool.dataRender(msg: "WCDB表\(data_viewMsg)" + (String(noti_viewTitle) + String(kIndexStyleKey)) + "\(error).")
                }
                //: } else {
            } else {
                // 数据库中不存在，则插入数据
                //: WCDBUserInfoTable.db_insertUserInfo(dbData)
                FileInfoTable.toInfo(dbData)
            }
            //: })
        })
    }
    ...
    private class func toInfo(_ userInfo: FileInfoTable) {
        //: WCDBUserInfoTable.db_insertUserInfos([userInfo])
        FileInfoTable.frame([userInfo])
    }

    /// 插入多条数据
    /// - Parameter userInfos: 数据数组
    //: private class func db_insertUserInfos(_ userInfos: [WCDBUserInfoTable]) {
    private class func frame(_ userInfos: [FileInfoTable]) {
        //: try? WCDBManager.shared.database?.run(transaction: { _ in
        try? SingingVoiceManager.shared.database?.run(transaction: { _ in
            //: do {
            do {
                //: try WCDBManager.shared.database?.insert(userInfos, intoTable: WCDBUserInfoTableName)
                try SingingVoiceManager.shared.database?.insert(userInfos, intoTable: data_viewMsg)
                //: } catch {
            } catch {
                //: UploadLogTool.writeLog(msg: "WCDB表\(WCDBUserInfoTableName): 批量插入数据失败。error：\(error).")
                TitleSongLogTool.dataRender(msg: "WCDB表\(data_viewMsg)" + (String(appTimeTitleFormat) + kRawMainId.replacingOccurrences(of: "view", with: "数据") + "败。error：") + "\(error).")
            }
            //: })
        })
    }
