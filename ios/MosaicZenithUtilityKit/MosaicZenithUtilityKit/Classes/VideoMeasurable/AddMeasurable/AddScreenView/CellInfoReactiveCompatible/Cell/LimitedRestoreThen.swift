...
/*: "#ffffff" :*/
fileprivate let mainReplaceMessage:[Character] = ["#","f","f","f","f","f"]
fileprivate let show_ofStr:[Character] = ["f"]

/*: "earned" :*/
fileprivate let kAfterMsg:[Character] = ["e","a","r","n","e","d"]

/*: "Coins in" :*/
fileprivate let user_viewColorValue:[Character] = ["C","o","i","n","s"," "]
fileprivate let constLabManagerKey:[Character] = ["i","n"]

/*: "#C946D2" :*/
fileprivate let const_equalStr:String = "text image local var nor#C946D2"

/*: "btn_chat_floatS_userBK_nor" :*/
fileprivate let mainUserKey:[Character] = ["b","t","n","_","c","h","a","t","_","f","l","o","a","t","S","_","u","s","e","r","B","K","_","n","o"]
fileprivate let show_tableText:String = "icon"
...
class LimitedRestoreThen: UIView {
    ...
    private let TextColor = (String(mainReplaceMessage) + String(show_ofStr))

    //: var clickBlock: (() -> Void)?
    var clickBlock: (() -> Void)?

    //: convenience init(model: TalkingGameFloatingScreenModel?) {
    convenience init(model: SmallnessTransformable?) {
        //: self.init()
        self.init()
    ...
    private func element() {
        ...
        if danumuMsgModel.bgPic.count > 0 {
            ...
            titleTheory(withUrl: danumuMsgModel.bgPic)
        }
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            do {
                ...
                if let range = attrStr.string.range(of: (String(kAfterMsg))) {
                ...
                if let range = attrStr.string.range(of: (String(user_viewColorValue) + String(constLabManagerKey))) {
                    //: attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                    attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                }
                //: attrStr.addAttributes([.font: UIFont.pingfangFont(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                attrStr.addAttributes([.font: UIFont.font(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
    ...
    func titleTheory(withUrl url: String) {
        ...
        self.backgroundImageView.applicatorEqual(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.HeightOfBackgroundImageView {
                    ...
                    newImage = image.notAthlete(with: CGSize(width: targetWidth, height: self.HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: SmallnessTransformable? {
        //: didSet {
        didSet {
            //: configModel()
            element()
    ...
    var totalTime: CGFloat {
        ...
        return (user_barText + widthValue) * SpendTime / user_barText
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * LineSpendTime / ScreenWidth
        return widthValue * LineSpendTime / user_barText
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * SpendTime / ScreenWidth
        return widthValue * SpendTime / user_barText
    }
    ...
    lazy var headAnimatedImageView: UIImageView = {
        ...
        h.layer.borderColor = UIColor(hex: (String(const_equalStr.suffix(7))))?.cgColor
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var headBackImageView: UIImageView = {
        ...
        h.image = UIImage.snapIcon(name: (String(mainUserKey) + show_tableText.replacingOccurrences(of: "icon", with: "r")))
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(someAdd(_:)))
        //: return t
        return t
        //: }()
    }()
...
extension LimitedRestoreThen {
    /// 点击跳转事件
    //: @objc func tapAction(_ tapGesture: UITapGestureRecognizer) {
    @objc func someAdd(_: UITapGestureRecognizer) {
        //: guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        //: let httpsUrl = ExamineedReactiveCompatible.share.appConfigMode.urlH5Domain+gameUrl
        let httpsUrl = ExamineedReactiveCompatible.share.appConfigMode.urlH5Domain + gameUrl
...
extension LimitedRestoreThen {
    ...
    private func comfortableSubviews() {
