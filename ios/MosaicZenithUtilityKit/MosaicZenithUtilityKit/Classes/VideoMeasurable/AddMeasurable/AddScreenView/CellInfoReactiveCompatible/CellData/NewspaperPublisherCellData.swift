...
/*: "content" :*/
fileprivate let showPlayerTitle:String = "conbagenbag"

/*: "init(coder:) has not been implemented" :*/
fileprivate let const_enableIconReturnUrl:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "jumpKey" :*/
fileprivate let kEstimatedTitle:String = "jumpKemake type beauty"
fileprivate let app_scaleValue:String = "Y"
...
// __CLOSE_PRINT__
//
//  NewspaperPublisherCellData.swift
//  AbroadTalking
//
...
@objcMembers public class NewspaperPublisherCellData: PointReactiveCompatible {
    // 数据
    //: var extraJson: JSON {
    var extraJson: JSON {
    ...
    public lazy var titleAttrStr: NSMutableAttributedString = {
        ...
        let attributes = [.foregroundColor: UIColor.equalTarget(), .font: UIFont.font(type: .Medium, fontSize: 17)] as [NSAttributedString.Key: Any]
    ...
    public lazy var contentAttrStr: NSMutableAttributedString = {
        ...
        let attributes = [.foregroundColor: UIColor.componentColor(), .font: UIFont.font(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
        //: let attributeStr = NSMutableAttributedString(string: extraJson["content"].stringValue)
        let attributeStr = NSMutableAttributedString(string: extraJson[(showPlayerTitle.replacingOccurrences(of: "bag", with: "t"))].stringValue)
        //: attributeStr.addAttributes(attributes, range: NSRange(location: 0, length: attributeStr.length))
        attributeStr.addAttributes(attributes, range: NSRange(location: 0, length: attributeStr.length))
        //: return attributeStr
        return attributeStr
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_enableIconReturnUrl.reversed(), encoding: .utf8)!)
    }
    ...
    override public func fixing(_: CGFloat) -> CGFloat {
        ...
        if extraJson[(showPlayerTitle.replacingOccurrences(of: "bag", with: "t"))].stringValue.count > 0 {
            //: height += 4
            height += 4
        ...
        if extraJson[(String(kEstimatedTitle.prefix(6)) + app_scaleValue.lowercased())].stringValue.count > 0 {
            //: jumpHeight = 35
            jumpHeight = 35
            //: height += jumpHeight
            height += jumpHeight
