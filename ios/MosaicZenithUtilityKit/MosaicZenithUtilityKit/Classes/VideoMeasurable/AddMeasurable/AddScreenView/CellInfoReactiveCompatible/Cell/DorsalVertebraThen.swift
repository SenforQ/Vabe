...
fileprivate let appModelUrl:[UInt8] = [0x9b,0x9c,0x9b,0x86,0xda,0x91,0x9d,0x96,0x97,0x80,0xc8,0xdb,0xd2,0x9a,0x93,0x81,0xd2,0x9c,0x9d,0x86,0xd2,0x90,0x97,0x97,0x9c,0xd2,0x9b,0x9f,0x82,0x9e,0x97,0x9f,0x97,0x9c,0x86,0x97,0x96]

				private func gestureValue(path num: UInt8) -> UInt8 {
					return num ^ 242
				}

/*: "btn_intimate_send_bg_nor" :*/
fileprivate let noti_conversationPath:String = "conversation case indexbtn_in"
fileprivate let app_dataFrameStr:String = "_send_view type range"
...
class DorsalVertebraThen: VocalizationMsgCell {
    //: typealias SendBlock = () -> Void
    typealias SendBlock = () -> Void
    //: var sendBlock: SendBlock?
    var sendBlock: SendBlock?
    //: var giftData: CutCellData?
    var giftData: CutCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appModelUrl.map{gestureValue(path: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var sendBtn: GroupVideoControl = {
        //: let btn = TalkingButton.init()
        let btn = GroupVideoControl()
        //: btn.setBackgroundImage(UIImage.BundleImageNamed(name: "btn_intimate_send_bg_nor"), for: .normal)
        btn.setBackgroundImage(UIImage.snapIcon(name: (String(noti_conversationPath.suffix(6)) + "timate" + String(app_dataFrameStr.prefix(6)) + "bg_nor")), for: .normal)
...
extension DorsalVertebraThen {
    ...
    override func create(with data: PokeAtCellData) {
        ...
        self.indicator.stopAnimating()
        //: self.giftData = data as? CutCellData
        self.giftData = data as? CutCellData
