...
fileprivate let k_imageStatusTitle:[UInt8] = [0xd5,0xda,0xd5,0xe0,0x94,0xcf,0xdb,0xd0,0xd1,0xde,0xa6,0x95,0x8c,0xd4,0xcd,0xdf,0x8c,0xda,0xdb,0xe0,0x8c,0xce,0xd1,0xd1,0xda,0x8c,0xd5,0xd9,0xdc,0xd8,0xd1,0xd9,0xd1,0xda,0xe0,0xd1,0xd0]

				fileprivate func labelCurrent(report num: UInt8) -> UInt8 {
...
class ScriptModelMessageCell: VocalizationMsgCell {
    //: var giftData: TalkingGroupChatGiftMsgCellData?
    var giftData: ChatCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_imageStatusTitle.map{labelCurrent(report: $0)}, encoding: .utf8)!)
    }
...
extension ScriptModelMessageCell {
    ...
    override func create(with data: PokeAtCellData) {
        ...
        self.giftData = data as? ChatCellData
        ...
        self.giftImgView.inclose(urlStr: imgPreview)
        ...
        if giftData.msgModel.toUser.uid == "all" { // 送礼所有人
            ...
            self.toUserImgView.image = UIImage.snapIcon(name: giftData.msgModel.toUser.headPic)
            //: } else {
        } else {
            //: self.toUserImgView.setUrlImage(urlStr: giftData.msgModel.toUser.headPic)
            self.toUserImgView.inclose(urlStr: giftData.msgModel.toUser.headPic)
        }
    }
