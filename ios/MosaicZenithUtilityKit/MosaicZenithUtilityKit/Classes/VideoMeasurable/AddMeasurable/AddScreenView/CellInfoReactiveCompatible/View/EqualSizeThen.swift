...
fileprivate let main_viewFormat:[UInt8] = [0x9c,0x9b,0x9c,0x81,0xdd,0x96,0x9a,0x91,0x90,0x87,0xcf,0xdc,0xd5,0x9d,0x94,0x86,0xd5,0x9b,0x9a,0x81,0xd5,0x97,0x90,0x90,0x9b,0xd5,0x9c,0x98,0x85,0x99,0x90,0x98,0x90,0x9b,0x81,0x90,0x91]

/*: "icon_chats_banner_subtract" :*/
fileprivate let dataBottomRemoteTitle:String = "color file miniicon_"
fileprivate let dataLiveStr:[Character] = ["c","h","a","t","s","_","b","a","n","n","e","r","_","s"]
fileprivate let data_makeItemUrl:String = "ubtraequal"

/*: "icon_chats_banner_subtract_pre" :*/
fileprivate let show_userId:String = "image gendericon_c"
fileprivate let appDataText:String = "anner_subtable reading name head view"
fileprivate let main_touchData:String = "trsendc"
fileprivate let userElementStr:String = "t_preto to state"

/*: "bannerList" :*/
fileprivate let kListValue:String = "bablock"

/*: "JXBannerCellID" :*/
fileprivate let k_cellKey:String = "make"
fileprivate let k_filterName:String = "icon camera join button returnXBann"
fileprivate let show_transactionStr:String = "file viewllID"

/*: "#FFFFFF" :*/
fileprivate let mainTargetUrl:[Character] = ["#","F","F","F","F","F","F"]
...
class EqualSizeThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_viewFormat.map{$0^245}, encoding: .utf8)!)
    }
    ...
    private lazy var bannerListData: [LiveTransformable] = //: return Array<LiveTransformable>()
        .init()
    //: }()
    ...
    private lazy var bannerView: JXBanner = {
        ...
        banner.placeholderImgView.image = UIImage.anArti()
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.snapIcon(name: (String(dataBottomRemoteTitle.suffix(5)) + String(dataLiveStr) + data_makeItemUrl.replacingOccurrences(of: "equal", with: "ct"))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chats_banner_subtract_pre"), for: .selected)
        btn.setImage(UIImage.snapIcon(name: (String(show_userId.suffix(6)) + "hats_b" + String(appDataText.prefix(9)) + main_touchData.replacingOccurrences(of: "send", with: "a") + String(userElementStr.prefix(5)))), for: .selected)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(pushDismiss), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension EqualSizeThen {
    /// banner
    //: func reqBanner() {
    func vacate() {
        //: SocialRequestManager.init().req_adBanner(position: 3) { [weak self] succeed, result, errorModel in
        SkinColourRequestManager().female(position: 3) { [weak self] _, result, _ in
            ...
            let array = JSON(result[(kListValue.replacingOccurrences(of: "block", with: "nn") + "erList")] ?? []).arrayValue
...
extension EqualSizeThen {
    ...
    @objc private func pushDismiss() {
        ...
        self.delegate?.banner(isMiniView: isMiniView)
    }

    //: func resetView(isMiniView: Bool) {
    func beyondShared(isMiniView: Bool) {
        //: if isMiniView {
        if isMiniView {
...
extension EqualSizeThen: JXBannerDelegate, JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (k_cellKey.replacingOccurrences(of: "make", with: "J") + String(k_filterName.suffix(5)) + "erCe" + String(show_transactionStr.suffix(4))))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        tempCell.imageView.applicatorEqual(urlStr: url, placeImg: UIImage.parentArrange())
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(mainTargetUrl)))!
