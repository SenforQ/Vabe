...
fileprivate let constShareTitle:[UInt8] = [0x5f,0x64,0x5f,0x6a,0x1e,0x59,0x65,0x5a,0x5b,0x68,0x30,0x1f,0x16,0x5e,0x57,0x69,0x16,0x64,0x65,0x6a,0x16,0x58,0x5b,0x5b,0x64,0x16,0x5f,0x63,0x66,0x62,0x5b,0x63,0x5b,0x64,0x6a,0x5b,0x5a]

				fileprivate func beData(status num: UInt8) -> UInt8 {
...
class AddScreenView: UIView {
    ...
    var danumuMsgArray: [SmallnessTransformable] = []
    //: var danumuItemViewArray: [TalkingGameFloatingScreenItemView] = []
    var danumuItemViewArray: [LimitedRestoreThen] = []

    //: let TapAction: PublishSubject<TalkingGameFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<SmallnessTransformable?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constShareTitle.map{beData(status: $0)}, encoding: .utf8)!)
    }
    ...
    func imageSoundFemale(_ danumuMsgModel: SmallnessTransformable?) {
    ...
    @objc func cutBy(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(cutBy(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension AddScreenView: UIGestureRecognizerDelegate {
    ...
    func rebuild(for danumuMsgModel: SmallnessTransformable?, isFirst: Bool) {
        //: let itemView = TalkingGameFloatingScreenItemView(model: danumuMsgModel)
        let itemView = LimitedRestoreThen(model: danumuMsgModel)
        ...
        var frame = CGRect(x: user_barText, y: isFirst ? 7 : 50, width: itemView.widthValue, height: 24)
        //: var endFrame = frame
        var endFrame = frame
        ...
        if SizeLanguageManager.shared.direction == .rightToLeft {
            ...
            endFrame.origin.x = user_barText
        }
        //: addSubview(itemView)
        addSubview(itemView)
    ...
    func who(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                rebuild(for: model, isFirst: isFirst)
