...
fileprivate let dataAvailableName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#AAAAAA" :*/
fileprivate let data_serverMsg:String = "number first in if#AAAA"
fileprivate let const_addEventUrl:String = "viewview"

/*: "btn_chatsettings_unchoice" :*/
fileprivate let main_statusValue:String = "btn_view make"
fileprivate let const_makeText:String = "ettsectionng"
fileprivate let noti_tableQueryedValue:String = "liste"

/*: "btn_chatsettings_choiced" :*/
fileprivate let dataGiftName:[Character] = ["b","t","n","_","c","h","a"]
fileprivate let kBorderUrl:String = "dataetti"
fileprivate let main_changeUrl:String = "hoitemced"

/*: "Cancel" :*/
fileprivate let app_reasonName:[Character] = ["C","a","n","c","e","l"]
...
class SectionThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataAvailableName.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIView = {
        ...
        let height = actualWidth(w: 475) + dataShowText
        //: let v = UIView(frame: CGRect(x: 0, y: ScreenHeight-height, width: ScreenWidth, height: height))
        let v = UIView(frame: CGRect(x: 0, y: show_licenseData - height, width: user_barText, height: height))
        //: v.backgroundColor = .white
        v.backgroundColor = .white
        //: v.Corner(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        v.areaRecordRadii(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var textBgView: UIView = {
        ...
        v.backgroundColor = UIColor.addChoice()
    ...
    private lazy var textView: YYTextView = {
        ...
        tv.textColor = UIColor.equalTarget()
        //: tv.font = UIFont.pingfangFont(type: .Regular, fontSize: 16)
        tv.font = UIFont.font(type: .Regular, fontSize: 16)
    ...
    private lazy var limitLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(data_serverMsg.suffix(5)) + const_addEventUrl.replacingOccurrences(of: "view", with: "A")))
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var topBtn: GroupVideoControl = {
        ...
        btn.setImage(UIImage.snapIcon(name: (String(main_statusValue.prefix(4)) + "chats" + const_makeText.replacingOccurrences(of: "section", with: "i") + "s_uncho" + noti_tableQueryedValue.replacingOccurrences(of: "list", with: "ic"))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btn_chatsettings_choiced"), for: .selected)
        btn.setImage(UIImage.snapIcon(name: (String(dataGiftName) + kBorderUrl.replacingOccurrences(of: "data", with: "ts") + "ngs_c" + main_changeUrl.replacingOccurrences(of: "item", with: "i"))), for: .selected)
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((String(app_reasonName)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(placeTo), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension SectionThen: YYTextViewDelegate {
...
extension SectionThen {
    ...
    @objc private func writing() {
        ...
        let isTop = topBtn.isSelected ? 1 : 2
        //: SizeRequestTool.req_remark(content: textView.text, isTop: isTop, toUid: targetId) {
        SizeRequestTool.untilOf(content: textView.text, isTop: isTop, toUid: targetId) {
            //: succeed, _, _ in
            succeed, _, _ in
    ...
    func night(_ content: String, isTop: Int, toUid: String) {
        //: targetId = toUid
        targetId = toUid
...
extension SectionThen {
    ...
    private func ptolemaicSystemSubviews() {
