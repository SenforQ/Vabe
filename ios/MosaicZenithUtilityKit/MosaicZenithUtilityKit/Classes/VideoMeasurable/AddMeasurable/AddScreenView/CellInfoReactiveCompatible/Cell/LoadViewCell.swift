...
fileprivate let kRawStr:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_chat_translate_nor" :*/
fileprivate let constPlaceUrl:String = "icon_cof string"
fileprivate let dataVideoMessage:String = "racancelslate"
fileprivate let appAddStr:[Character] = ["_","n","o","r"]

/*: "icon_chat_translate_pre" :*/
fileprivate let constTopMsg:String = "self filter videoicon_"
fileprivate let app_labelPath:String = "_tranrange content manager"
fileprivate let main_centerClickName:String = "e_prbar create message"
fileprivate let notiCornerKindTitle:String = "user"

/*: "icon_chat_miss" :*/
fileprivate let appImageKey:[Character] = ["i","c","o","n","_","c","h","a"]
fileprivate let kLengthStr:[Character] = ["t","_","m","i","s","s"]

/*: "icon_chat_yes" :*/
fileprivate let k_makeText:String = "icon_native available collection let"
fileprivate let kDownId:String = "fail to_yes"
...
class LoadViewCell: VocalizationMsgCell {
    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?

    //: var textData: GrownupCellData?
    var textData: GrownupCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kRawStr.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var transBtn: UIButton = {
        ...
        btn.setImage(UIImage.snapIcon(name: (String(constPlaceUrl.prefix(6)) + "hat_t" + dataVideoMessage.replacingOccurrences(of: "cancel", with: "n") + String(appAddStr))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chat_translate_pre"), for: .highlighted)
        btn.setImage(UIImage.snapIcon(name: (String(constTopMsg.suffix(5)) + "chat" + String(app_labelPath.prefix(5)) + "slat" + String(main_centerClickName.prefix(4)) + notiCornerKindTitle.replacingOccurrences(of: "user", with: "e"))), for: .highlighted)
...
extension LoadViewCell {
    ...
    override func create(with data: PokeAtCellData) {
        ...
        //: let maxWidth = PicMacroDefine.getScreenWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        let maxWidth = PicMacroDefine.sharedObject() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
        ...
        if textData.translateStatus == .MsgTranslate_translating {
            ...
            self.transStatusImagV.image = UIImage.snapIcon(name: (String(appImageKey) + String(kLengthStr)))
            ...
            self.transStatusImagV.image = UIImage.snapIcon(name: (String(k_makeText.prefix(5)) + "chat" + String(kDownId.suffix(4))))
        }
    ...
    func downwards() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(playResponse(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func LognCellMessageAction(tap: UILongPressGestureRecognizer) {
    @objc func playResponse(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
