...
fileprivate let const_contentStr:[UInt8] = [0x4a,0x4f,0x4a,0x55,0x9,0x44,0x50,0x45,0x46,0x53,0x1b,0xa,0x1,0x49,0x42,0x54,0x1,0x4f,0x50,0x55,0x1,0x43,0x46,0x46,0x4f,0x1,0x4a,0x4e,0x51,0x4d,0x46,0x4e,0x46,0x4f,0x55,0x46,0x45]

				fileprivate func viewItem(main num: UInt8) -> UInt8 {
...
/*: "icon_chat_lock" :*/
fileprivate let mainEraseTopName:String = "icon_view pan request"
fileprivate let main_managerText:[Character] = ["l","o","c","k"]

/*: "icon_chat_burned" :*/
fileprivate let noti_observerFormat:String = "itablen"
fileprivate let noti_transformMessage:String = "neslide"

/*: "icon_chat_burned_shan" :*/
fileprivate let notiStateMediumValue:String = "icon_cspecial height var"
fileprivate let appMagnitudeeractionTitle:String = "burgifte"

/*: "Already burned" :*/
fileprivate let main_cellMessage:[Character] = ["A","l","r","e","a","d","y"," ","b","u","r","n"]
fileprivate let user_sizeKey:String = "count"
...
class ResponseViewCell: VocalizationMsgCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_contentStr.map{viewItem(main: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var lockImagV: UIImageView = {
        ...
        imag.image = UIImage.snapIcon(name: (String(mainEraseTopName.prefix(5)) + "chat_" + String(main_managerText)))
    ...
    lazy var burnedImagV: GroupVideoControl = {
        ...
        imgV.setBackgroundImage(UIImage.snapIcon(name: (noti_observerFormat.replacingOccurrences(of: "table", with: "co") + "_chat_bur" + noti_transformMessage.replacingOccurrences(of: "slide", with: "d"))), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.snapIcon(name: (String(notiStateMediumValue.prefix(6)) + "hat_" + appMagnitudeeractionTitle.replacingOccurrences(of: "gift", with: "n") + "d_shan")), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(main_cellMessage) + user_sizeKey.replacingOccurrences(of: "count", with: "ed")).localized, for: .normal)
        //: imgV.setTitleColor(.white, for: .normal)
        imgV.setTitleColor(.white, for: .normal)
        //: imgV.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 16)
        imgV.titleLabel?.font = UIFont.easing(fontSize: 16)
        //: return imgV
        return imgV
        //: }()
    }()
...
extension ResponseViewCell {
    //: override func fill(with data: TCommonCellData) {
    override func create(with data: PokeAtCellData) {
        //: super.fill(with: data)
        super.create(with: data)
        //: guard let imageCellData = data as? CompartmentCellData else { return }
        guard let imageCellData = data as? CompartmentCellData else { return }
        ...
        let intimatePhoto: RequestPermissionModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        ...
        if intimatePhoto != nil || imageCellData.msgModel.msgInfo.customSend.isBurn == "1" || Int(imageCellData.msgModel.msgInfo.customSend.giftId) ?? 0 > 0 { // 私密照片
            ...
            enableData(data: imageCellData, urlStr: imgUrlStr, isMosaic: imgIsMosaic)

            //: } else {
        } else { // 普通图片
            //: showImageView(data: imageCellData, urlStr: nil, isMosaic: false)
            enableData(data: imageCellData, urlStr: nil, isMosaic: false)
        }
    }
    ...
    ///   - urlStr: 图片网络地址
    ///   - isMosaic: 是否需要马赛克
    //: func showImageView(data: CompartmentCellData, urlStr: String?, isMosaic: Bool) {
    func enableData(data: CompartmentCellData, urlStr: String?, isMosaic: Bool) {
        ...
        filePath = (filePath as NSString).appendingPathComponent(notiDeviceName)
        ...
        if data.imageUrl!.count > 0, isExists {
            ...
            DispatchQueue.global().async {
                ...
                DispatchQueue.main.async {
                    ...
                    //: if isMosaic {
                    if isMosaic {
                        //: self.photoView.image = UIImage.mosaicSend(image, tillMessage: 90)
                        self.photoView.image = UIImage.mosaicSend(image, tillMessage: 90)
                        //: } else {
                    } else {
            ...
            swaddlingClothes(urlStr: urlStr ?? "", isMosaic: isMosaic)
            //: } else {
        } else { // 本地图片请求网络
            ...
            if data.imageData != nil {
                ...
                self.photoView.inclose(urlStr: data.imageUrl!)
            }
        }
    }
    ...
    func swaddlingClothes(urlStr: String, isMosaic: Bool) {
        //: self.photoView.setCookieToUrlImage(urlStr: urlStr) {[weak self] image in
        self.photoView.imbecility(urlStr: urlStr) { [weak self] image in
