...
fileprivate let notiKitName:[UInt8] = [0x14,0x13,0x14,0x9,0x55,0x1e,0x12,0x19,0x18,0xf,0x47,0x54,0x5d,0x15,0x1c,0xe,0x5d,0x13,0x12,0x9,0x5d,0x1f,0x18,0x18,0x13,0x5d,0x14,0x10,0xd,0x11,0x18,0x10,0x18,0x13,0x9,0x18,0x19]

				private func titleShow(selected num: UInt8) -> UInt8 {
					return num ^ 125
				}

/*: "icon_lounge" :*/
fileprivate let main_keyValue:[Character] = ["i","c","o","n","_"]
fileprivate let kModeId:[Character] = ["l","o","u","n","g","e"]

/*: "icon_home_v" :*/
fileprivate let constPositionText:String = "error observe toicon_"
fileprivate let noti_titleToId:String = "home_vrow with data data equal"

/*: "My album" :*/
fileprivate let app_withId:[Character] = ["M","y"," ","a","l","b","u","m"]

/*: "0A071F" :*/
fileprivate let noti_layerStr:[Character] = ["0","A","0","7","1","F"]

/*: "#C5BEFF" :*/
fileprivate let notiDataName:[Character] = ["#","C","5","B","E","F"]
fileprivate let app_toLeadingExtensionData:String = "image"

/*: "🎉congratulations! She may be very interested in you. She's not far from you. \nWhy don't you say hello to her?" :*/
fileprivate let userLocationTitle:[UInt8] = [0x91,0x40,0x2f,0x2a,0x4,0x10,0xf,0x8,0x13,0x2,0x15,0x16,0xd,0x2,0x15,0xa,0x10,0xf,0x14,0xc2,0xc1,0xf4,0x9,0x6,0xc1,0xe,0x2,0x1a,0xc1,0x3,0x6,0xc1,0x17,0x6,0x13,0x1a,0xc1,0xa,0xf,0x15,0x6,0x13,0x6,0x14,0x15,0x6,0x5,0xc1,0xa,0xf,0xc1,0x1a,0x10,0x16,0xcf,0xc1,0xf4,0x9,0x6,0xc8,0x14,0xc1,0xf,0x10,0x15,0xc1,0x7,0x2,0x13,0xc1,0x7,0x13,0x10,0xe,0xc1,0x1a,0x10,0x16,0xcf,0xc1,0xab,0xf8,0x9,0x1a,0xc1,0x5,0x10,0xf,0xc8,0x15,0xc1,0x1a,0x10,0x16,0xc1,0x14,0x2,0x1a,0xc1,0x9,0x6,0xd,0xd,0x10,0xc1,0x15,0x10,0xc1,0x9,0x6,0x13,0xe0]

				fileprivate func clickAdd(interval num: UInt8) -> UInt8 {
...
/*: "Why don't you say hello to her?" :*/
fileprivate let appToData:[UInt8] = [0x88,0xb7,0xa6,0xff,0xbb,0xb0,0xb1,0xf8,0xab,0xff,0xa6,0xb0,0xaa,0xff,0xac,0xbe,0xa6,0xff,0xb7,0xba,0xb3,0xb3,0xb0,0xff,0xab,0xb0,0xff,0xb7,0xba,0xad,0xe0]

				private func viewTo(camera num: UInt8) -> UInt8 {
					return num ^ 223
				}

/*: "🎉congratulations! He may be very interested in you. He's not far from you. \nWhy don't you say hello to him?" :*/
fileprivate let show_styleMessage:[UInt8] = [0x15,0xc4,0xb3,0xae,0x88,0x94,0x93,0x8c,0x97,0x86,0x99,0x9a,0x91,0x86,0x99,0x8e,0x94,0x93,0x98,0x46,0x45,0x6d,0x8a,0x45,0x92,0x86,0x9e,0x45,0x87,0x8a,0x45,0x9b,0x8a,0x97,0x9e,0x45,0x8e,0x93,0x99,0x8a,0x97,0x8a,0x98,0x99,0x8a,0x89,0x45,0x8e,0x93,0x45,0x9e,0x94,0x9a,0x53,0x45,0x6d,0x8a,0x4c,0x98,0x45,0x93,0x94,0x99,0x45,0x8b,0x86,0x97,0x45,0x8b,0x97,0x94,0x92,0x45,0x9e,0x94,0x9a,0x53,0x45,0x2f,0x7c,0x8d,0x9e,0x45,0x89,0x94,0x93,0x4c,0x99,0x45,0x9e,0x94,0x9a,0x45,0x98,0x86,0x9e,0x45,0x8d,0x8a,0x91,0x91,0x94,0x45,0x99,0x94,0x45,0x8d,0x8e,0x92,0x64]

				fileprivate func counternalRepresentation(commit num: UInt8) -> UInt8 {
...
/*: "Why don't you say hello to him?" :*/
fileprivate let k_viewInfoStr:[UInt8] = [0x3f,0x6d,0x69,0x68,0x20,0x6f,0x74,0x20,0x6f,0x6c,0x6c,0x65,0x68,0x20,0x79,0x61,0x73,0x20,0x75,0x6f,0x79,0x20,0x74,0x27,0x6e,0x6f,0x64,0x20,0x79,0x68,0x57]

/*: "#FFD13A" :*/
fileprivate let appRangeValue:[Character] = ["#","F","F","D","1","3","A"]

/*: "icon_male_default" :*/
fileprivate let mainModelCellContent:[UInt8] = [0xc,0x6,0xa,0xb,0x3a,0x8,0x4,0x9,0x0,0x3a,0x1,0x0,0x3,0x4,0x10,0x9,0x11]

				private func tableMedia(name num: UInt8) -> UInt8 {
					return num ^ 101
				}

/*: "icon_female_default" :*/
fileprivate let dataSearchName:String = "ICON"
fileprivate let main_sizeFlushId:String = "error gesture to fee_femal"
fileprivate let dataNoItemMessage:String = "adatet"

/*: "My interests：" :*/
fileprivate let mainGenerateImageErrorId:String = "value"
fileprivate let kTextCurName:[Character] = ["y"," ","i","n","t","e","r","e","s","t","s","："]

/*: "btn_me_edit" :*/
fileprivate let main_toMsg:[Character] = ["b","t","n","_","m"]
fileprivate let k_viewStr:[Character] = ["e"]
fileprivate let const_lockData:[Character] = ["_","e","d","i","t"]
...
class TableMsgCell: VocalizationMsgCell {
    //: var matchData: TalkingChatMatchMsgCellData?
    var matchData: PaperThen?
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        setupWith()
        //: bindInteraction()
        stickRunNumbererchange()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: notiKitName.map{titleShow(selected: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.snapIcon(name: (String(main_keyValue) + String(kModeId)))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    private lazy var authImgview: UIImageView = {
        ...
        imgV.image = UIImage.snapIcon(name: (String(constPositionText.suffix(5)) + String(noti_titleToId.prefix(6))))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var imgTitleLab: UILabel = {
        ...
        label.font = UIFont.dismissSharedSize(fontSize: 15)
        //: label.textColor = UIColor.appTitleColor()
        label.textColor = UIColor.equalTarget()
        //: label.text = "My album".localized
        label.text = (String(app_withId)).localized
        //: return label
        return label
        //: }()
    }()
    ...
    private lazy var bgView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(noti_layerStr)))?.withAlphaComponent(0.3)
        //: view.layer.cornerRadius = 12
        view.layer.cornerRadius = 12
        //: view.layer.masksToBounds = true
        view.layer.masksToBounds = true
        ...
        view.layer.borderColor = UIColor(hex: (String(notiDataName) + app_toLeadingExtensionData.replacingOccurrences(of: "image", with: "F")))?.cgColor
        //: return view
        return view
        //: }()
    }()
    ...
    private lazy var matchLab: UILabel = {
        ...
        var str = String(bytes: userLocationTitle.map{clickAdd(interval: $0)}, encoding: .utf8)!.localized
        //: var matchStr = "Why don't you say hello to her?".localized
        var matchStr = String(bytes: appToData.map{viewTo(camera: $0)}, encoding: .utf8)!.localized

        //: if ExamineedReactiveCompatible.share.loginUserMode.sex == "2" {
        if ExamineedReactiveCompatible.share.loginUserMode.sex == "2" {
            //: str = "🎉congratulations! He may be very interested in you. He's not far from you. \nWhy don't you say hello to him?".localized
            str = String(bytes: show_styleMessage.map{counternalRepresentation(commit: $0)}, encoding: .utf8)!.localized
            //: matchStr = "Why don't you say hello to him?".localized
            matchStr = String(bytes: k_viewInfoStr.reversed(), encoding: .utf8)!.localized
        }
        ...
        var attriString = NSMutableAttributedString(string: str as String, attributes: [.font: UIFont.dismissSharedSize(fontSize: 15), .foregroundColor: UIColor.white])
        ...
        if let range = str.range(of: matchStr) {
            ...
            attriString.addAttributes([NSAttributedString.Key.foregroundColor: UIColor(hex: (String(appRangeValue)))!], range: nsRange)
        }
...
extension TableMsgCell {
    ...
    override func create(with data: PokeAtCellData) {
        ...
        self.matchData = data as? PaperThen
        ...
        // 为空时必须初始化
        //: let userInfoModel = self.matchData?.msgModel.userInfoModel ?? BivouacCacheModel.init()
        let userInfoModel = self.matchData?.msgModel.userInfoModel ?? BivouacCacheModel()
        ...
        if self.vipBgView.isHidden == false {
            //: if let vipModel = ExamineedReactiveCompatible.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userInfoModel.vipSkinId}).first {
            if let vipModel = ExamineedReactiveCompatible.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userInfoModel.vipSkinId }).first {
                //: self.vipBgView.setUrlImage(urlStr: vipModel.vipChatSkin, placeImg: nil)
                self.vipBgView.inclose(urlStr: vipModel.vipChatSkin, placeImg: nil)
            }
        }
        //: self.headImgView.setUrlImage(urlStr: userInfoModel.headPic)
        self.headImgView.inclose(urlStr: userInfoModel.headPic)
        //: self.infoNameLab.textColor = userInfoModel.loungePlus ? .userVipColor():.appTitleColor()
        self.infoNameLab.textColor = userInfoModel.loungePlus ? .utilizerForBodyCovering() : .equalTarget()
        ...
        let imgStr = userInfoModel.sex == CommentRefCompatibleValue.male.rawValue ? String(bytes: mainModelCellContent.map{tableMedia(name: $0)}, encoding: .utf8)! : (dataSearchName.lowercased() + String(main_sizeFlushId.suffix(6)) + "e_def" + dataNoItemMessage.replacingOccurrences(of: "date", with: "ul"))
        //: sexBtn.setBackgroundImage(UIImage.BundleImageNamed(name: imgStr), for: .normal)
        sexBtn.setBackgroundImage(UIImage.snapIcon(name: imgStr), for: .normal)
        ...
        if userInfoModel.interest.count > 0 {
            ...
            let matchStr = (mainGenerateImageErrorId.replacingOccurrences(of: "value", with: "M") + String(kTextCurName)).localized
            ...
            let attributes = [.font: UIFont.dismissSharedSize(fontSize: 15), .foregroundColor: UIColor.titleDevice()] as [NSAttributedString.Key: Any]
            ...
            attrString.addAttributes([NSAttributedString.Key.foregroundColor: UIColor.equalTarget()], range: str.range(of: matchStr))
    ...
    func limb() {
        ...
        let maxCount = Int((user_barText - 60) / 62)
        ...
        for index in 0 ..< min(maxCount, matchData.msgModel.userInfoModel.picture.count) {
            ...
            imgV.inclose(urlStr: matchData.msgModel.userInfoModel.picture[index])
        ...
        if matchData.msgModel.userInfoModel.picture.count > maxCount {
            ...
            imgV.image = UIImage.snapIcon(name: (String(main_toMsg) + String(k_viewStr) + String(const_lockData)))
            //: self.pictureView.addSubview(imgV)
            self.pictureView.addSubview(imgV)
            //: imgV.snp.remakeConstraints { make in
            imgV.snp.remakeConstraints { make in
    ...
    override public func updateConstraints() {
        ...
        if userInfoModel.uid.isEmptyString {
            ...
            self.infoView.snp.remakeConstraints { make in
                ...
                make.width.equalTo(user_barText - 30)
            ...
            if userInfoModel.interest.count > 0 {
                ...
                let inTheight = self.interestsLab.sizeThatFits(CGSize(width: user_barText - 54, height: CGFLOAT_MAX)).height
                ...
                self.interestsLab.snp.remakeConstraints { make in
                    ...
                    make.width.equalTo(user_barText - 54)
        ...
        let labelSize = self.matchLab.sizeThatFits(CGSize(width: user_barText - 30 - 24, height: CGFLOAT_MAX))
...
extension TableMsgCell {
    ...
    private func stickRunNumbererchange() {
        ...
        let tapGes = UITapGestureRecognizer(target: self, action: #selector(rebate(_:)))
    ...
    @objc func rebate(_: UITapGestureRecognizer) {
