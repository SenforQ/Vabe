...
@implementation AggregationModel
...
@implementation RequestPermissionModel
...
+ (void)unwished:(RequestPermissionModel *)photoModel age:(NSString *)msgID {
    ...
    dict[msgID] = jsonString;
    //: NSString *path = [DataCacheRedefineReactiveCompatible getIntimatePhotoPath];
    NSString *path = [DataCacheRedefineReactiveCompatible writtenMatter];
...
+ (void)unwanted:(ClearSendModel *)videoModel mediaBy:(NSString *)msgID {
    ...
    //: dict[msgID] = jsonString;
    dict[msgID] = jsonString;
    //: NSString *path = [DataCacheRedefineReactiveCompatible getIntimateVideoPath];
    NSString *path = [DataCacheRedefineReactiveCompatible viewOf];
    //: [NSKeyedArchiver archiveRootObject:dict toFile:path];
    [NSKeyedArchiver archiveRootObject:dict toFile:path];
}
...
- (CGSize)session:(NSInteger)maxWidth{
    ...
    NSDictionary* attributes = @{NSFontAttributeName: [UIFont easingWithFontSize:12]};
    ...
    if ([self.renderType isEqualToString:@"gift"]) {
        ...
        NSString *str = [NSString stringWithFormat:@"%@: %@%@ x%ld",self.sendName,@"Sent ".localized,self.renderData.notSizeMessage, (long)self.renderData.giftNum];
        //: CGSize contentSize = [str boundingRectWithSize:CGSizeMake(1.7976931348623157e+308, 14) options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading) attributes:attributes context:nil].size;
        CGSize contentSize = [str boundingRectWithSize:CGSizeMake(1.7976931348623157e+308, 14) options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading) attributes:attributes context:nil].size;
        //: return CGSizeMake(((maxWidth) < (5 + contentSize.width) ? (maxWidth) : (5 + contentSize.width)) , 14);
        return CGSizeMake(((maxWidth) < (5 + contentSize.width) ? (maxWidth) : (5 + contentSize.width)) , 14);
...
- (NSString *)notSizeMessage{
    //: NSString *locName;
    NSString *locName;
    //: NSString *sysLangCode = [PicMacroDefine getSystemLangCode];
    NSString *sysLangCode = [PicMacroDefine confinementDoing];
...
+ (void)local:(NSString *)msgID {
    ...
    @synchronized (self) {
        ...
        NSMutableArray *arr = [self will];
        //: [arr addObject:msgID];
        [arr addObject:msgID];
        //: NSString *path = [DataCacheRedefineReactiveCompatible getAskForGiftPath];
        NSString *path = [DataCacheRedefineReactiveCompatible colorWith];
