...
fileprivate let dataEqualId:[UInt8] = [0xed,0xea,0xed,0xf0,0xac,0xe7,0xeb,0xe0,0xe1,0xf6,0xbe,0xad,0xa4,0xec,0xe5,0xf7,0xa4,0xea,0xeb,0xf0,0xa4,0xe6,0xe1,0xe1,0xea,0xa4,0xed,0xe9,0xf4,0xe8,0xe1,0xe9,0xe1,0xea,0xf0,0xe1,0xe0]

/*: "icon_chat_miss" :*/
fileprivate let const_skinStr:String = "labelcon"
fileprivate let user_buttonTitle:String = "t_misstyle self"
fileprivate let constMainId:String = "S"

/*: "icon_chat_yes" :*/
fileprivate let const_errorFormat:[Character] = ["i","c","o","n","_","c","h","a","t","_"]
fileprivate let const_myPathName:String = "ymax"
...
// __CLOSE_PRINT__
//
//  CellInfoReactiveCompatible.swift
//  AbroadTalking
//
...
class CellInfoReactiveCompatible: VocalizationMsgCell {
//    var transBlock: (() -> Void)?

    //: var textData: LoadCellData?
    var textData: LoadCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataEqualId.map{$0^132}, encoding: .utf8)!)
    }
    ...
    lazy var transLB: YYLabel = {
        ...
        label.font = UIFont.easing(fontSize: 16)
        //: return label
        return label
        //: }()
    }()
    ...
    override func create(with data: PokeAtCellData) {
        ...
        if let temTextData = data as? LoadCellData {
            ...
            if imgPreview.count > 0, temTextData.msgModel.msgInfo.messageType == .MessageTypeWelcome {
                ...
                giftImagV.inclose(urlStr: imgPreview)
            }
...
//                self.messageData?.translateStatus != .MsgTranslate_succeed && self.messageData?.msgModel.msgInfo.callCmd.count ?? 0 <= 0 && temTextData.msgModel.msgInfo.messageType != .MessageTypeWelcome && "\(temTextData.msgModel.user.uid)" != PicMacroDefine.getXiaoMiID()) { // 对方发的 && 开启了翻译功能 && 消息未翻译成功
...
//            }
            ...
            if temTextData.msgModel.quoteMsgInfo.tvMsg() {
            ...
            if self.textData?.translateStatus == .MsgTranslate_translating {
                ...
                self.transStatusImagV.image = UIImage.snapIcon(name: (const_skinStr.replacingOccurrences(of: "label", with: "i") + "_cha" + String(user_buttonTitle.prefix(5)) + constMainId.lowercased()))
                ...
                self.transStatusImagV.image = UIImage.snapIcon(name: (String(const_errorFormat) + const_myPathName.replacingOccurrences(of: "max", with: "es")))
            }
    ...
    func imageFile() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(fileAllocationTable(tap:)))
        ...
        self.container.addGestureRecognizer(containerTap)
        //: let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == PicMacroDefine.getXiaoMiID())
        let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == PicMacroDefine.tapId())
        //: if isOfficial == false {
        if isOfficial == false {
            //: addLongTagGes()
            requestDeadline()
...
//            }
    ...
    func requestDeadline() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(picTap(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func TouchCellMessageAction(tap: UITapGestureRecognizer?) {
    @objc func fileAllocationTable(tap _: UITapGestureRecognizer?) {
    ...
    @objc func picTap(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
...
//    }
    ...
    @objc func jumpOn() {
        ...
        self.delegate?.sendBy(cell: self)
    }
}
