...
fileprivate let notiModelTransactionStr:[UInt8] = [0x1d,0x1a,0x1d,0x0,0x5c,0x17,0x1b,0x10,0x11,0x6,0x4e,0x5d,0x54,0x1c,0x15,0x7,0x54,0x1a,0x1b,0x0,0x54,0x16,0x11,0x11,0x1a,0x54,0x1d,0x19,0x4,0x18,0x11,0x19,0x11,0x1a,0x0,0x11,0x10]

/*: "#0084FF" :*/
fileprivate let userTimeName:String = "#"
fileprivate let kTextId:String = "self else0084FF"

/*: "#FFEE4B" :*/
fileprivate let k_willId:String = "#FFEE4Binput layer"

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let const_intervalPath:String = "let text not voice<at>@"
fileprivate let noti_colorMessage:String = "s]+?</aself manager"
fileprivate let show_hungKey:String = "self view succeed imaget>"

/*: "jumpKey" :*/
fileprivate let app_signKey:String = "jumpKsucceed main for var"
fileprivate let k_marginModelYourPath:String = "equaly"

/*: "#16D073" :*/
fileprivate let dataBarMessage:String = "#16D07flag self be"
fileprivate let main_frameValue:String = "3"

/*: "#F95644" :*/
fileprivate let dataNameMessage:String = "name video#F956"
fileprivate let const_modeUrl:[Character] = ["4","4"]
...
//: @objcMembers public class GrownupCellData: TalkingChatMsgBaseCellData {
@objcMembers public class GrownupCellData: PointReactiveCompatible {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiModelTransactionStr.map{$0^116}, encoding: .utf8)!)
    }
    ...
    public lazy var contentAttrString: NSMutableAttributedString = {
        ...
        let attr = componentFormat(msgInfoModel: self.msgModel.msgInfo)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func componentFormat(msgInfoModel: AggregationModel) -> NSMutableAttributedString {
        ...
        let textColor = (self.direction == .MsgDirectionIncoming) ? UIColor.equalTarget() : .white
        //: let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: "#0084FF")! : UIColor(hex: "#FFEE4B")!
        let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: (userTimeName.capitalized + String(kTextId.suffix(6))))! : UIColor(hex: (String(k_willId.prefix(7))))!
        ...
        let regex = try! NSRegularExpression(pattern: (String(const_intervalPath.suffix(5)) + "[\\S\\" + String(noti_colorMessage.prefix(7)) + String(show_hungKey.suffix(2))), options: [])
        ...
        for (index, value) in rangeArr.enumerated() {
            ...
            if jumpInfo[(String(app_signKey.prefix(5)) + k_marginModelYourPath.replacingOccurrences(of: "equal", with: "e"))].stringValue == "user" {
    ...
    func behindAdd() -> NSMutableAttributedString {
        ...
        var color = UIColor(hex: (String(dataBarMessage.prefix(6)) + main_frameValue.capitalized))!
        //: if self.translateStatus == .MsgTranslate_fail {
        if self.translateStatus == .MsgTranslate_fail {
            //: color = UIColor(hex: "#F95644")!
            color = UIColor(hex: (String(dataNameMessage.suffix(5)) + String(const_modeUrl)))!
        }
