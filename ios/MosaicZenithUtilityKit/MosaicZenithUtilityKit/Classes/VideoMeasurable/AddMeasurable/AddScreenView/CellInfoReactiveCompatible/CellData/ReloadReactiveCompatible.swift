...
fileprivate let data_equalMessage:[UInt8] = [0xf9,0xfe,0xf9,0x4,0xb8,0xf3,0xff,0xf4,0xf5,0x2,0xca,0xb9,0xb0,0xf8,0xf1,0x3,0xb0,0xfe,0xff,0x4,0xb0,0xf2,0xf5,0xf5,0xfe,0xb0,0xf9,0xfd,0x0,0xfc,0xf5,0xfd,0xf5,0xfe,0x4,0xf5,0xf4]

				fileprivate func tempTable(menu num: UInt8) -> UInt8 {
...
/*: "View read receipts?" :*/
fileprivate let noti_viewBehindEqualFormat:String = "View rsay down top"
fileprivate let noti_nextTitle:String = "carrayipts"
fileprivate let showSendBlockValue:[Character] = ["?"]

/*: "<xml>%@</xml>" :*/
fileprivate let kIconId:String = "self cell as<xml>"
fileprivate let data_makeName:String = "equal"
fileprivate let main_addBlockPath:[Character] = ["@","<","/","x","m","l",">"]

/*: "padding" :*/
fileprivate let constActualMsg:String = "ptingding"

/*: "maxFont" :*/
fileprivate let userResultStreamUrl:[Character] = ["m","a","x","F","o"]
fileprivate let app_leadingName:String = "nview"

/*: "PingFangSC-Regular" :*/
fileprivate let dataTitleMessage:String = "if let playPingFang"
fileprivate let k_tableFormat:[Character] = ["S","C","-","R","e","g","u","l","a","r"]

/*: "content" :*/
fileprivate let const_dateKey:[UInt8] = [0x9d,0x91,0x90,0x8a,0x9b,0x90,0x8a]

				private func withLive(selected num: UInt8) -> UInt8 {
					return num ^ 254
				}
...
// __CLOSE_PRINT__
//
//  ReloadReactiveCompatible.swift
//  AbroadTalking
//
...
@objcMembers public class ReloadReactiveCompatible: PointReactiveCompatible {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_equalMessage.map{tempTable(menu: $0)}, encoding: .utf8)!)
    }

    //: public override func heightOfWidth(_ width: CGFloat) ->CGFloat {
    override public func fixing(_: CGFloat) -> CGFloat {
        //: return contentSize().height+20
        return little().height + 20
    }
    ...
    override public func little() -> CGSize {
        ...
        if self.msgModel.tips.content == (String(noti_viewBehindEqualFormat.prefix(6)) + "ead re" + noti_nextTitle.replacingOccurrences(of: "array", with: "e") + String(showSendBlockValue)).localized {
            //: return CGSize(width: ScreenWidth, height: 30)
            return CGSize(width: user_barText, height: 30)
        }
        ...
        //: let rect = self.attributedString.boundingRect(with: CGSize.init(width: PicMacroDefine.getScreenWidth()-50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil )
        let rect = self.attributedString.boundingRect(with: CGSize(width: PicMacroDefine.sharedObject() - 50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil)
    ...
    public lazy var attributedString: NSMutableAttributedString = {
        ...
        var attr = printingToTabularMatter(info: self.msgModel.tips)
        //: return attr
        return attr
        //: }()
    }()
...
extension ReloadReactiveCompatible {
    //: func formatMessageToStringM(info: AbTalkingChatMsgTipsModel) -> NSMutableAttributedString {
    func printingToTabularMatter(info: CellModel) -> NSMutableAttributedString {
...
//        let dic = NSDictionary(jsonString: extra ?? "")
        ...
        return lyricTips(tips: content)
    }

    //: func designTextFream(tips: String) -> NSMutableAttributedString {
    func lyricTips(tips: String) -> NSMutableAttributedString {
        ...
        _ = recordInfo(msg: tips, info: attrbutedArray)
        //: self.yyLayout = designYYTextLayout(array: attrbutedArray, displayMaxWidth: PicMacroDefine.getScreenWidth()-50, para: para)
        self.yyLayout = displayCell(array: attrbutedArray, displayMaxWidth: PicMacroDefine.sharedObject() - 50, para: para)
    ...
    func recordInfo(msg: String, info: NSMutableArray) -> String {
        ...
        let pure_msg = commensurate(source: msg, attrbAM: contentAttrArray)
        ...
        for textInfo in contentAttrArray {
            ...
            let attrDict = subBe(content: text, color: textColor)
    ...
    func commensurate(source: String, attrbAM: NSMutableArray) -> String {
        ...
        do {
            ...
            let document = try ONOXMLDocument(string: String(format: (String(kIconId.suffix(5)) + data_makeName.replacingOccurrences(of: "equal", with: "%") + String(main_addBlockPath)), newSource), encoding: CFStringConvertEncodingToNSStringEncoding(CFStringBuiltInEncodings.UTF8.rawValue))
            //: let element = document.rootElement
            let element = document.rootElement
            //: parseXMLElement(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: ["color": UIColor.white])
            allQueryed(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: ["color": UIColor.white])
    ...
    func allQueryed(element: ONOXMLElement, arrM: NSMutableArray, attrib: NSDictionary) {
        ...
        if children.count > 0 {
            ...
            for subElement in children {
                ...
                allQueryed(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
            }
    ...
    func displayCell(array: NSArray, displayMaxWidth: CGFloat, para: NSParagraphStyle?) -> YYTextLayout {
        //: let attrString = designAttribStr(array: array, paragraphStyle: para)
        let attrString = ditheredColour(array: array, paragraphStyle: para)
    ...
    func ditheredColour(array: NSArray, paragraphStyle: NSParagraphStyle?) -> NSAttributedString {
        ...
        array.enumerateObjects { obj, _, _ in
            ...
            if type == "txt" {
                ...
                tmp = stopDict(dict: newObj as NSDictionary)
                //: } else if type == "padding" {
            } else if type == (constActualMsg.replacingOccurrences(of: "ting", with: "ad")) {
                ...
                tmp = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: newObj[(String(userResultStreamUrl) + app_leadingName.replacingOccurrences(of: "view", with: "t"))] as! UIFont, alignment: .center)
            }
    ...
    func desgin(dict: NSDictionary) -> NSAttributedString {
        ...
        let attachText = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: dict[(String(userResultStreamUrl) + app_leadingName.replacingOccurrences(of: "view", with: "t"))] as! UIFont, alignment: .center)
        //: return attachText
        return attachText
    }

    //: func designAttribStrByTextDic(dict: NSDictionary) ->NSMutableAttributedString {
    func stopDict(dict: NSDictionary) -> NSMutableAttributedString {
        ...
        if fontSize ?? 0 > 0 {
            ...
            let fontRef = CTFontCreateWithName((String(dataTitleMessage.suffix(8)) + String(k_tableFormat)) as CFString, CGFloat(fontSize!), nil)
            //: attributes[kCTFontAttributeName] = fontRef
            attributes[kCTFontAttributeName] = fontRef
        }
        //: var content = (dict["content"] != nil) ? dict["content"] : ""
        var content = (dict[String(bytes: const_dateKey.map{withLive(selected: $0)}, encoding: .utf8)!] != nil) ? dict[String(bytes: const_dateKey.map{withLive(selected: $0)}, encoding: .utf8)!] : ""
    ...
    func attributes() -> NSMutableDictionary {
        //: let font = UIFont.pingfangRugularFont(fontSize: 15)
        let font = UIFont.easing(fontSize: 15)
    ...
    func subBe(content: String, color: UIColor?) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
        ...
        if color == nil {
            ...
            return addIn(content: content)
        }
        //: return ["type": "txt",
        return ["type": "txt",
                //: "content": content,
                String(bytes: const_dateKey.map{withLive(selected: $0)}, encoding: .utf8)!: content,
                //: "color": color ?? .black]
                "color": color ?? .black]
    }

    //: func ctDictionaryForText(content: String) ->NSDictionary {
    func addIn(content: String) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
            //: return NSDictionary.init()
            return NSDictionary()
