...
fileprivate let constShareMsg:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_chat_start" :*/
fileprivate let showControlTextPath:[Character] = ["i","c","o","n","_","c","h","a","t","_","s"]
fileprivate let noti_expressValue:String = "locationrt"

/*: "#EBEBEB" :*/
fileprivate let data_aPathIfMsg:String = "#EBEBEBfilter share of user result"

/*: "icon_talk_reply_voive" :*/
fileprivate let notiShowValue:String = "icon_tself user image make view"
fileprivate let main_positionFormat:String = "reply_var equal make ting if"

/*: "#DCDCDC" :*/
fileprivate let noti_thumbMaxId:[Character] = ["#","D","C","D","C","D","C"]
...
class EasingQuoteView: UIButton {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constShareMsg.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var playView: UIImageView = {
        ...
        img.image = UIImage.snapIcon(name: (String(showControlTextPath) + noti_expressValue.replacingOccurrences(of: "location", with: "ta")))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    lazy var audioView: UIView = {
        ...
        v.backgroundColor = UIColor(hex: (String(data_aPathIfMsg.prefix(7))))
    ...
    lazy var audioImg: UIImageView = {
        ...
        img.image = UIImage.snapIcon(name: (String(notiShowValue.prefix(6)) + "alk_" + String(main_positionFormat.prefix(6)) + "voive"))
        //: return img
        return img
        //: }()
    }()
    ...
    var direction: SizeExpressionConvertible = .MsgDirectionIncoming {
        //: didSet {
        didSet {
            //: if direction == .MsgDirectionIncoming {
            if direction == .MsgDirectionIncoming {
                //: self.lineView.backgroundColor = UIColor.appValueDetailColor()
                self.lineView.backgroundColor = UIColor.aftColor()
                //: self.nameLab.textColor = UIColor.appValueDetailColor()
                self.nameLab.textColor = UIColor.aftColor()
                //: }else{
            } else {
                //: self.lineView.backgroundColor = UIColor(hex: "#DCDCDC")
                self.lineView.backgroundColor = UIColor(hex: (String(noti_thumbMaxId)))
                //: self.nameLab.textColor = UIColor(hex: "#DCDCDC")
                self.nameLab.textColor = UIColor(hex: (String(noti_thumbMaxId)))
            }
        }
    }
    ...
    var quoteModel: CancelModel? {
        ...
        didSet {
            ...
            if quoteModel.renderType == "text" {
                ...
                self.nameLab.text = quoteModel.sendName + ": " + "Sent ".localized + quoteModel.renderData.notSizeMessage() + " x\(quoteModel.renderData.giftNum)"
                ...
                if quoteModel.renderData.previewUrl.count > 0 && isExists {
                    ...
                    self.imgView.inclose(urlStr: quoteModel.renderData.previewUrl)
                }
                //: spaceRight = 34
                spaceRight = 34
                ...
                self.imgView.inclose(urlStr: quoteModel.renderData.coverUrl)
                //: spaceRight = 34
                spaceRight = 34
            }
...
extension EasingQuoteView {
    ...
    private func radiogramSubviews() {
