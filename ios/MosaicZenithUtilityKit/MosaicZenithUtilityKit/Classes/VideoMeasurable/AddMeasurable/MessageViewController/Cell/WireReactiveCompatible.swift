...
fileprivate let show_regularTablePath:String = "lab false device page replyTSys"
fileprivate let noti_timeVoiceTitle:String = "hidden text equal rank returnsageCe"
fileprivate let user_subStr:String = "post key caseuseId"

/*: "init(coder:) has not been implemented" :*/
fileprivate let const_styleContent:[UInt8] = [0xbb,0xbc,0xbb,0xa6,0xfa,0xb1,0xbd,0xb6,0xb7,0xa0,0xe8,0xfb,0xf2,0xba,0xb3,0xa1,0xf2,0xbc,0xbd,0xa6,0xf2,0xb0,0xb7,0xb7,0xbc,0xf2,0xbb,0xbf,0xa2,0xbe,0xb7,0xbf,0xb7,0xbc,0xa6,0xb7,0xb6]

/*: "icon_talk_notic" :*/
fileprivate let kLabStr:[Character] = ["i","c","o","n","_","t","a","l","k","_","n"]
fileprivate let k_infoTitle:String = "texttic"
...
@objc public protocol CharacteristicRootOfASquareMatrixDelegate: NSObjectProtocol {
    /**
     *  长按消息回调
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onLongPressMessage(cell: WireReactiveCompatible)
    func cellDown(cell: WireReactiveCompatible)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onRetryMessage(cell: WireReactiveCompatible)
    func postView(cell: WireReactiveCompatible)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessage(cell: WireReactiveCompatible)
    func theModel(cell: WireReactiveCompatible)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessageAvatar(cell: WireReactiveCompatible)
    func bigCell(cell: WireReactiveCompatible)

    /**
     *  点击消息单元中引用消息
     *  @param cell 委托者，消息单元
     */
    //: func onReplyMessage(cell: WireReactiveCompatible)
    func sendBy(cell: WireReactiveCompatible)
}

//: @objcMembers public class WireReactiveCompatible: TCommonTableViewCell {
@objcMembers public class WireReactiveCompatible: SkipViewCell {
    /**
     *  信息数据类。
     *  存储了该massageCell中所需的信息。包括发送者 ID，发送者头像、信息发送状态、信息气泡图标等。
     *  messageData 详细信息请参考：Section\Chat\CellData\SideCellData.h
     */
    //: public var messageData: SideCellData?
    public var messageData: SideCellData?

    /// 复用cell标识符
    //: @objc public static let TSystemMessageCell_ReuseId = "TSystemMessageCell_ReuseId"
    public static let TSystemMessageCell_ReuseId = (String(show_regularTablePath.suffix(4)) + "temMes" + String(noti_timeVoiceTitle.suffix(6)) + "ll_Re" + String(user_subStr.suffix(5)))

    /**
     *  协议委托
     *  负责实现 CharacteristicRootOfASquareMatrixDelegate 协议中的功能。
     */
    //: public weak var delegate: TMessageCellDelegate?
    public weak var delegate: CharacteristicRootOfASquareMatrixDelegate?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_styleContent.map{$0^210}, encoding: .utf8)!)
    }
    ...
    override public func create(with data: PokeAtCellData) {
        //: super.fill(with: data)
        super.create(with: data)
        //: let newData: SideCellData = data as! SideCellData
        let newData: SideCellData = data as! SideCellData
    ...
    override public func updateConstraints() {
        ...
        let csize = self.messageData?.little()
    ...
    lazy var avatarView: UIImageView = {
        ...
        let tap1 = UITapGestureRecognizer(target: self, action: #selector(checkGift(recognizer:)))
    ...
    lazy var nameLabel: UILabel = {
        ...
        lb.font = UIFont.dismissSharedSize(fontSize: 13)
        //: lb.textColor = .gray
        lb.textColor = .gray
    ...
    public lazy var container: UIView = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(alongButtonUser(recognizer:)))
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(colorTab(recognizer:)))
    ...
    lazy var retryView: UIImageView = {
        ...
        imagV.image = UIImage.snapIcon(name: (String(kLabStr) + k_infoTitle.replacingOccurrences(of: "text", with: "o")))
        ...
        let resendTap = UITapGestureRecognizer(target: self, action: #selector(videoSendImage(recognizer:)))
...
//: extension WireReactiveCompatible {
extension WireReactiveCompatible {
    //: @objc func onSelectMessageAvatar(recognizer: UIGestureRecognizer) {
    @objc func checkGift(recognizer _: UIGestureRecognizer) {
        //: self.delegate?.onSelectMessageAvatar(cell: self)
        self.delegate?.bigCell(cell: self)
    }

    //: @objc func onLongPress(recognizer: UIGestureRecognizer) {
    @objc func colorTab(recognizer: UIGestureRecognizer) {
