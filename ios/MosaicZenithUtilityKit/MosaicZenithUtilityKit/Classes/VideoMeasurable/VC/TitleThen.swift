...
fileprivate let show_modelFormat:[UInt8] = [0xf5,0xfa,0xf5,0x0,0xb4,0xef,0xfb,0xf0,0xf1,0xfe,0xc6,0xb5,0xac,0xf4,0xed,0xff,0xac,0xfa,0xfb,0x0,0xac,0xee,0xf1,0xf1,0xfa,0xac,0xf5,0xf9,0xfc,0xf8,0xf1,0xf9,0xf1,0xfa,0x0,0xf1,0xf0]

				fileprivate func artificialLanguageValue(direction num: UInt8) -> UInt8 {
...
/*: "icon_photo_giftbg" :*/
fileprivate let dataInviteFormat:[Character] = ["i","c","o","n","_","p","h","o","t","o","_","g","i","f","t","b","g"]

/*: "nav_back_black_nor" :*/
fileprivate let data_appViewKey:String = "nav_bmake block path"
fileprivate let kVoiceKey:String = "ack_norvar equal equal"

/*: "giftId" :*/
fileprivate let const_cellUrl:[Character] = ["g","i","f","t","I","d"]

/*: "giftNum" :*/
fileprivate let const_managerToMsg:[Character] = ["g","i","f","t","N","u","m"]

/*: "unlockMsgId" :*/
fileprivate let dataValueMessage:String = "size shareunlockMsg"
fileprivate let showSampleTitle:String = "Idself make"

/*: "Need a %@ (%@ Gold coins) to unlock" :*/
fileprivate let data_toUrl:[UInt8] = [0x6b,0x63,0x6f,0x6c,0x6e,0x75,0x20,0x6f,0x74,0x20,0x29,0x73,0x6e,0x69,0x6f,0x63,0x20,0x64,0x6c,0x6f,0x47,0x20,0x40,0x25,0x28,0x20,0x40,0x25,0x20,0x61,0x20,0x64,0x65,0x65,0x4e]

/*: "icon_photo_lock" :*/
fileprivate let k_fromTitle:[Character] = ["i","c","o","n","_","p","h","o","t"]
fileprivate let dataObjectUrl:[Character] = ["o","_","l","o","c","k"]

/*: "Click on the screen to view 15s" :*/
fileprivate let k_picMsg:[UInt8] = [0xe9,0xc6,0xc3,0xc9,0xc1,0x8a,0xc5,0xc4,0x8a,0xde,0xc2,0xcf,0x8a,0xd9,0xc9,0xd8,0xcf,0xcf,0xc4,0x8a,0xde,0xc5,0x8a,0xdc,0xc3,0xcf,0xdd,0x8a,0x9b,0x9f,0xd9]

				private func showView(name num: UInt8) -> UInt8 {
					return num ^ 170
				}

/*: "icon_photo_burn" :*/
fileprivate let kTopUrl:[Character] = ["i","c","o","n","_","p"]
fileprivate let notiBagId:[Character] = ["h","o","t","o","_","b","u","r","n"]

/*: "Already burned" :*/
fileprivate let notiAvailableMessage:[Character] = ["A","l","r","e","a"]
fileprivate let constFrameFormat:[Character] = ["d","y"," ","b","u","r","n","e","d"]
...
typealias IntimatePhotoBlock = (_ model: RequestPermissionModel) -> Void

//: class TalkingIntimatePhotoPreviewVC: TalkingBaseViewController {
class TitleThen: ReloadViewController {
    ...
    //: private var imageTotalData = CompartmentCellData(direction: .MsgDirectionOutgoing)
    private var imageTotalData = CompartmentCellData(direction: .MsgDirectionOutgoing) // 图片消息数据
    //: private var photoModel = AbTalkingChatMsgIntimatePhotoModel()
    private var photoModel = RequestPermissionModel() // 当前私密照片模型
    ...
    ///   - imageCellData: 图片模型
    ///   - toUid: 对方Id
    //: init(imageCellData: CompartmentCellData, toUid: String) {
    init(imageCellData: CompartmentCellData, toUid: String) {
        ...
        //: targetId = toUid
        targetId = toUid
        //: isMySelf = (imageTotalData.msgModel.user.uid == Int(ExamineedReactiveCompatible.share.loginUserMode.userID))
        isMySelf = (imageTotalData.msgModel.user.uid == Int(ExamineedReactiveCompatible.share.loginUserMode.userID))
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_modelFormat.map{artificialLanguageValue(direction: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var giftBgView: UIImageView = {
        ...
        img.image = UIImage.snapIcon(name: (String(dataInviteFormat)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftIcon: UIImageView = {
        ...
        img.inclose(urlStr: photoModel.giftIcon)
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftSendBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(cameraEqualContent), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var backBtn: UIButton = {
        ...
        let img = UIImage.snapIcon(name: (String(data_appViewKey.prefix(5)) + "ack_bl" + String(kVoiceKey.prefix(7)))).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
        //: btn.addTarget(self, action: #selector(backButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(methodIn), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension TitleThen {
    ...
    func saveer(giftId: String, giftNum: String) {
        ...
        params[(String(const_cellUrl))] = giftId
        //: params["giftNum"] = giftNum
        params[(String(const_managerToMsg))] = giftNum
        //: params["unlockMsgId"] = self.imageTotalData.msgModel.msgInfo.msgId
        params[(String(dataValueMessage.suffix(9)) + String(showSampleTitle.prefix(2)))] = self.imageTotalData.msgModel.msgInfo.msgId
...
extension TitleThen {
    ...
    @objc private func cameraEqualContent() {
        //: req_sendGiftMsg(giftId: photoModel.giftId, giftNum: "1")
        saveer(giftId: photoModel.giftId, giftNum: "1")
    }
    ...
    @objc private func remote() {
        ...
        self.imageDetail(urlStr: photoModel.originUrl, isMosaic: false) { [weak self] isSucceed in
            //: ProgressHUD.dismiss()
            ...
            self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
                ...
                self.progressView.plot(totalTime: 150, currTime: time)
                //: time -= 1
                time -= 1
                //: }, repeats: true)
            }, repeats: true)
...
extension TitleThen {
    ...
    private func totaleractionPost() {
    ...
    private func buttonAcross() {
        ...
        if photoModel.lockStatus == 1 { // 锁定状态，需要送礼解锁
            ...
            self.desLab.text = String(bytes: data_toUrl.reversed(), encoding: .utf8)!.equalKey(photoModel.giftName, photoModel.giftPrice)
            ...
            if photoModel.burnStatus == 0 { // 未销毁
                ...
                self.intimateStatusImgV.image = UIImage.snapIcon(name: (String(k_fromTitle) + String(dataObjectUrl)))
                //: self.desLab.text = "Click on the screen to view 15s".localized
                self.desLab.text = String(bytes: k_picMsg.map{showView(name: $0)}, encoding: .utf8)!.localized
                // 添加手势
                //: let tap = UITapGestureRecognizer(target: self, action: #selector(burnPhotoTapGesture))
                let tap = UITapGestureRecognizer(target: self, action: #selector(remote))
                ...
                self.intimateStatusImgV.image = UIImage.snapIcon(name: (String(kTopUrl) + String(notiBagId)))
                //: self.desLab.text = "Already burned".localized
                self.desLab.text = (String(notiAvailableMessage) + String(constFrameFormat)).localized
            }
        ...
        self.imageDetail(urlStr: imgUrlStr, isMosaic: imgIsMosaic)
    }
    ...
    private func imageDetail(urlStr: String, isMosaic: Bool, completionHandler: ((_ isSucceed: Bool) -> Void)? = nil) {
