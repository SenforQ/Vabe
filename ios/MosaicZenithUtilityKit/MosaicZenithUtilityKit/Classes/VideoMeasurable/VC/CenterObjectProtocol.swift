...
/*: "New friends" :*/
fileprivate let show_withAddName:[Character] = ["N","e","w"," ","f","r","i","e","n"]
fileprivate let user_normalStr:[Character] = ["d","s"]

/*: "icon_yidu_pre" :*/
fileprivate let show_reportValue:String = "let view listicon_yidu"
fileprivate let userIconMsg:String = "_premore frame icon"

/*: "You've got no message yet." :*/
fileprivate let mainToolContent:String = "You\'ve"
fileprivate let constProgressTitle:String = "gift block height no m"
fileprivate let showImageKey:String = "row lock listen app camerae ye"
fileprivate let main_leadingUserId:String = "of mode top shared layert."

/*: "icon_kong_kong_default" :*/
fileprivate let constModelStr:String = "var send openicon_k"
fileprivate let mainSharedKey:String = "ong_defachange choice index"
fileprivate let userSucceedUrl:String = "ulname"

/*: "Cancel" :*/
fileprivate let k_sectionData:[Character] = ["C","a","n","c","e","l"]

/*: "Do you want to mark all messages as read?" :*/
fileprivate let kLabKey:[UInt8] = [0x14,0x3f,0x70,0x29,0x3f,0x25,0x70,0x27,0x31,0x3e,0x24,0x70,0x24,0x3f,0x70,0x3d,0x31,0x22,0x3b,0x70,0x31,0x3c,0x3c,0x70,0x3d,0x35,0x23,0x23,0x31,0x37,0x35,0x23,0x70,0x31,0x23,0x70,0x22,0x35,0x31,0x34,0x6f]

				private func dismissClick(message num: UInt8) -> UInt8 {
					return num ^ 80
				}

/*: "MoreMessages一键已读失败：code: :*/
fileprivate let app_blockRecordKey:String = "action nowMoreMe"
fileprivate let dataReadingId:[Character] = ["s","一","键","已","\u{8bfb}","失","\u{8d25}","\u{ff1a}","c","o","d","e",":"]

/*: "btn_message_report_nor" :*/
fileprivate let user_upStr:String = "btn_mpath in to effect center"
fileprivate let const_norMessage:[Character] = ["e","p","o","r","t","_","n","o","r"]

/*: "#FF935D" :*/
fileprivate let k_managerKey:String = "#FF93let collection value"
fileprivate let main_labStr:[Character] = ["5","D"]

/*: "btn_message_block_nor" :*/
fileprivate let mainHeadFormat:String = "btn_mesucceed newspaper"
fileprivate let data_cellName:String = "self any usere_b"
fileprivate let appPresentId:String = "lock_norreturn model model"

/*: "#C179F9" :*/
fileprivate let constReloadFormat:String = "#Cbottom no succeed want"
fileprivate let userStatusHeadValue:[Character] = ["1","7","9","F","9"]

/*: "btn_message_delete_nor" :*/
fileprivate let userMakeData:String = "app can time hidden wantbtn_m"
fileprivate let dataFeatureText:[Character] = ["e","s","s"]
fileprivate let noti_ofUrl:[Character] = ["a","g","e","_","d","e","l","e","t","e","_","n","o","r"]

/*: "#FF506D" :*/
fileprivate let appKitValue:[Character] = ["#"]
fileprivate let user_backgroundSizeLetStr:String = "FF506link"
...
class CenterObjectProtocol: ReloadViewController {
    //: override func viewDidLoad() {
    override func viewDidLoad() {
        //: super.viewDidLoad()
        super.viewDidLoad()
        //: self.title = "New friends".localized
        self.title = (String(show_withAddName) + String(user_normalStr)).localized
        ...
        btn.setImage(UIImage.snapIcon(name: (String(show_reportValue.suffix(9)) + String(userIconMsg.prefix(4)))), for: .normal)
        //: btn.addTarget(self, action: #selector(clearBadgeButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(badgeClickButtonUnambiguous), for: .touchUpInside)
    ...
    private lazy var emptyView: EmptyView = {
        ...
        style.TipsTitle = (mainToolContent + " got" + String(constProgressTitle.suffix(5)) + "essag" + String(showImageKey.suffix(4)) + String(main_leadingUserId.suffix(2))).localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (String(constModelStr.suffix(6)) + "ong_k" + String(mainSharedKey.prefix(8)) + userSucceedUrl.replacingOccurrences(of: "name", with: "t"))
...
extension CenterObjectProtocol {
    /// 刷新表格
...
extension CenterObjectProtocol {
    ...
    func distributionList(conversationModel: ImageDisappearReactiveCompatible) {
        ...
        //: CenterV2Listener.shared.func__removeConversation(targetId: conversationModel.targetId, dataType: .moreList)
        CenterV2Listener.shared.commerce(targetId: conversationModel.targetId, dataType: .moreList)
    ...
    func addStep(conversationModel: ImageDisappearReactiveCompatible) {
        // 拉黑/解除拉黑
        ...
        AlertThen.implement(title: nil,
                                   ...
                                   leftBtnTitle: (String(k_sectionData)).localized,
                                   ...
                                   rightBtnTitle: "OK".localized)
        {
            ...
            LittleRequestManager.execute(uid: conversationModel.userID, isBlack: true) { succeed, _, _ in
                ...
                self.distributionList(conversationModel: conversationModel)
    ...
    func gestureModel(conversationModel: ImageDisappearReactiveCompatible) {
    ...
    @objc private func badgeClickButtonUnambiguous() {
        ...
        AlertThen.upForQuantity(message: String(bytes: kLabKey.map{dismissClick(message: $0)}, encoding: .utf8)!.localized, leftBtnTitle: (String(k_sectionData)).localized, rightBtnTitle: "OK".localized, leftBlock: {
            //: TalkingAlertShow.hideAlert()
            AlertThen.statusElement()
            ...
            guard self.manager.forEachEnablee() > 0 else { return }
            ...
            self.manager.moreMsgArr.forEach { model in
                ...
                V2TIMManager.sharedInstance().markC2CMessage(asRead: model.userID) {
                    ...
                    TitleSongLogTool.dataRender(msg: (String(app_blockRecordKey.suffix(6)) + "ssage" + String(dataReadingId)) + "\(code), desc:\(String(describing: desc))")
                }
            }
...
extension CenterObjectProtocol {
    ...
    func putIn() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(fulfillNotification(notification:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(dataName(notif:)),
    ...
    @objc func fulfillNotification(notification: NSNotification) {
    ...
    @objc private func dataName(notif: Notification) {
...
extension CenterObjectProtocol: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var model = self.manager.timeBy(indexPath: indexPath)
    ...
    func tableView(_: UITableView, canEditRowAt indexPath: IndexPath) -> Bool {
        ...
        guard let model = self.manager.timeBy(indexPath: indexPath) else {
            //: return false
            return false
        }
    ...
    func tableView(_: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        guard let model = self.manager.timeBy(indexPath: indexPath) else { return nil }
        ...
        let reportBtn = UIContextualAction(style: .normal, title: nil) { _, _, completionHandler in
            ...
            self.gestureModel(conversationModel: model)
            //: completionHandler(false)
            completionHandler(false)
        }
        //: if let reportBtnTrashcan = UIImage.BundleImageNamed(name: "btn_message_report_nor").cgImage {
        if let reportBtnTrashcan = UIImage.snapIcon(name: (String(user_upStr.prefix(5)) + "essage_r" + String(const_norMessage))).cgImage {
        ...
        reportBtn.backgroundColor = UIColor(hex: (String(k_managerKey.prefix(5)) + String(main_labStr)))
        ...
        let blockBtn = UIContextualAction(style: .normal, title: nil) { _, _, completionHandler in
            ...
            self.addStep(conversationModel: model)
            //: completionHandler(false)
            completionHandler(false)
        }
        //: if let blockBtnTrashcan = UIImage.BundleImageNamed(name: "btn_message_block_nor").cgImage {
        if let blockBtnTrashcan = UIImage.snapIcon(name: (String(mainHeadFormat.prefix(6)) + "ssag" + String(data_cellName.suffix(3)) + String(appPresentId.prefix(8)))).cgImage {
        ...
        blockBtn.backgroundColor = UIColor(hex: (String(constReloadFormat.prefix(2)) + String(userStatusHeadValue)))
        ...
        let deleteBtn = UIContextualAction(style: .normal, title: nil) { _, _, completionHandler in
            ...
            self.distributionList(conversationModel: model)
            //: completionHandler(false)
            completionHandler(false)
        }
        //: if let deleteBtnTrashcan = UIImage.BundleImageNamed(name: "btn_message_delete_nor").cgImage {
        if let deleteBtnTrashcan = UIImage.snapIcon(name: (String(userMakeData.suffix(5)) + String(dataFeatureText) + String(noti_ofUrl))).cgImage {
        ...
        deleteBtn.backgroundColor = UIColor(hex: (String(appKitValue) + user_backgroundSizeLetStr.replacingOccurrences(of: "link", with: "D")))
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        //: AppPushManager.share.func__pushToPriveteChatVC(chatID: model.targetId) { vc in
        AppPushManager.share.logDown(chatID: model.targetId) { vc in
...
extension CenterObjectProtocol: BackObjectProtocol {
    //: func onRefreshConversationList(data: [TalkingConversationModel]) {
    func holeOutCallithumpData(data _: [ImageDisappearReactiveCompatible]) {
        //: self.manager.req_moreMsgInitData()
        self.manager.goOutPush()
        //: refreshTableView()
        statusAcross()
