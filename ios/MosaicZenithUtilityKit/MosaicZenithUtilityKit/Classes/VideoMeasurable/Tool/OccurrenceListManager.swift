...
/*: "onlineStatus" :*/
fileprivate let data_contentStr:[Character] = ["o","n","l","i","n","e","S","t","a","t"]
fileprivate let app_voiceId:[Character] = ["u","s"]

/*: "isNewUser" :*/
fileprivate let main_labBetweenText:[Character] = ["i","s","N","e","w","U","s"]
fileprivate let k_numberStr:String = "etip"

/*: "isTPAuth" :*/
fileprivate let constGroupTitleName:String = "isTPmodel size user list"

/*: "totalIntimate" :*/
fileprivate let app_makeId:String = "ttheta"
fileprivate let const_containerValue:String = "IMATE"

/*: "userStatus" :*/
fileprivate let appLabelValue:String = "userStatfilter if up return"
fileprivate let userToStr:[Character] = ["u","s"]

/*: "intimate" :*/
fileprivate let constCommitInfoSelectMsg:String = "modentmode"
...
enum TableValueConvertible: Int {
    //: case offical  = 0
    case offical = 0 // 官方
    //: case chatRoom = 1
    case chatRoom = 1 // 聊天室
...
class OccurrenceListManager {
    ...
    private var moreMsgModel: ImageDisappearReactiveCompatible = //: return ImageDisappearReactiveCompatible.getPlaceholderModel(chatType: .moreMsg)
        .typeImageModelGetData(chatType: .moreMsg)
    //: }()

    // 初始化数据
    //: func req_initData() {
    func everyList() {
        //: self.officialArr = CenterV2Listener.shared.getOfficialList()
        self.officialArr = CenterV2Listener.shared.dismissList()
        //: self.topArr = CenterV2Listener.shared.topConvList
        self.topArr = CenterV2Listener.shared.topConvList
        //: self.normalArr = CenterV2Listener.shared.norConvList
        self.normalArr = CenterV2Listener.shared.norConvList
        // 更多数据
        //: req_moreMsgInitData()
        goOutPush()
    ...
    func chat(completion: @escaping (_ succeed: Bool) -> Void) {
        //: guard self.isHalf == false else { return }
        guard self.isHalf == false else { return }
        //: CenterV2Listener.shared.getChatRoomData { succeed in
        CenterV2Listener.shared.atCard { succeed in
            //: if succeed {
            if succeed {
                //: self.chatRoomArr = CenterV2Listener.shared.chatRoomList
                self.chatRoomArr = CenterV2Listener.shared.chatRoomList
                //: completion(true)
                completion(true)
            }
    ...
    func meanSun(indexPath: IndexPath) -> ImageDisappearReactiveCompatible? {
    ...
    func imageUser(model: ImageDisappearReactiveCompatible?) -> ImageDisappearReactiveCompatible? {
        //: guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        //: let userInfo: BivouacCacheModel? = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId)
        let userInfo: BivouacCacheModel? = ViewValueInfoManager.cache(targetId: targetId)
    ...
    func showEnd(conversationList: [ImageDisappearReactiveCompatible], finish: (() -> Void)?) {
        //: var tempArr = [String]()
        var tempArr = [String]()
        ...
        ViewValueInfoManager.playWith(with: tempArr) { _, error in
    ...
    func addModel(conversationModel: ImageDisappearReactiveCompatible) {
    ...
    func reason(completion: @escaping (_ isFinish: Bool) -> Void) {
        //: CenterV2Listener.shared.func__getConversationList { arr, leftMore in
        CenterV2Listener.shared.wayOfLife { _, leftMore in
            //: completion(leftMore)
            completion(leftMore)
        }
    }
    ...
    lazy var officialArr: [ImageDisappearReactiveCompatible] = //: return Array<ImageDisappearReactiveCompatible>()
        .init()
    //: }()

    //: lazy var topArr: [TalkingConversationModel] = {
    lazy var topArr: [ImageDisappearReactiveCompatible] = //: return Array<ImageDisappearReactiveCompatible>()
        .init()
    //: }()

    //: lazy var normalArr: [TalkingConversationModel] = {
    lazy var normalArr: [ImageDisappearReactiveCompatible] = //: return Array<ImageDisappearReactiveCompatible>()
        .init()
    //: }()

    //: lazy var chatRoomArr: [TalkingConversationModel] = {
    lazy var chatRoomArr: [ImageDisappearReactiveCompatible] = //: return Array<ImageDisappearReactiveCompatible>()
        .init()
    //: }()

    //: lazy var intimateNorArr: [TalkingConversationModel] = {
    lazy var intimateNorArr: [ImageDisappearReactiveCompatible] = //: return Array<ImageDisappearReactiveCompatible>()
        .init()
    //: }()

    //: lazy var intimateTopArr: [TalkingConversationModel] = {
    lazy var intimateTopArr: [ImageDisappearReactiveCompatible] = //: return Array<ImageDisappearReactiveCompatible>()
        .init()
    //: }()

    //: lazy var moreMsgArr: [TalkingConversationModel] = {
    lazy var moreMsgArr: [ImageDisappearReactiveCompatible] = //: return Array<ImageDisappearReactiveCompatible>()
        .init()
    //: }()
}
...
extension OccurrenceListManager {
    ...
    func goOutPush() {
        //: self.moreMsgArr = CenterV2Listener.shared.moreMsgList
        self.moreMsgArr = CenterV2Listener.shared.moreMsgList
    }

    /// 根据索引获取模型（更多消息）
    //: func req_moreMsgConversationModel(indexPath: IndexPath) -> TalkingConversationModel? {
    func timeBy(indexPath: IndexPath) -> ImageDisappearReactiveCompatible? {
    ...
    func mentalPictureModel() -> ImageDisappearReactiveCompatible {
        // 更新更多消息最新数据
        //: req_moreMsgInitData()
        goOutPush()
        ...
        moreMsgModel.unreadCount = forEachEnablee()
...
extension OccurrenceListManager {
    ...
    func sumerchangeBottom(isFirst: Bool, completion: @escaping (_ succeed: Bool) -> Void) {
        ...
        ViewValueInfoManager.substitute(with: data) { resultArr, _ in
            ...
            if resultArr?.count ?? 0 <= 0 { // 无数据
                ...
                resultArr?.forEach { dict in
                    ...
                    let onlineStatus = json[(String(data_contentStr) + String(app_voiceId))].intValue
                    //: let isNewUser = json["isNewUser"].boolValue
                    let isNewUser = json[(String(main_labBetweenText) + k_numberStr.replacingOccurrences(of: "tip", with: "r"))].boolValue
                    //: let istpAuth = json["isTPAuth"].boolValue
                    let istpAuth = json[(String(constGroupTitleName.prefix(4)) + "Auth")].boolValue
                    //: let totalIntimate = json["totalIntimate"].int ?? 0
                    let totalIntimate = json[(app_makeId.replacingOccurrences(of: "the", with: "o") + "lInt" + const_containerValue.lowercased())].int ?? 0
                    //: let userStatus = json["userStatus"].int ?? 1
                    let userStatus = json[(String(appLabelValue.prefix(8)) + String(userToStr))].int ?? 1
                    ...
                    self.subCountimate(uid: uid, intimate: totalIntimate)
                    //: })
                }
            }
            //: completion(true)
    ...
    func appInfo(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo["uid"] as! String
        //: let onlineStatus = userInfo["onlineStatus"] as! Int
        let onlineStatus = userInfo[(String(data_contentStr) + String(app_voiceId))] as! Int
        //: let isNewUser = userInfo["isNewUser"] as! Bool
        let isNewUser = userInfo[(String(main_labBetweenText) + k_numberStr.replacingOccurrences(of: "tip", with: "r"))] as! Bool

        /// 用户状态更新
        //: let userStatus = userInfo["userStatus"] as? Int ?? 1
        let userStatus = userInfo[(String(appLabelValue.prefix(8)) + String(userToStr))] as? Int ?? 1
        //: var needuserStatus = false
        var needuserStatus = false
        //: if userInfo.keys.contains("userStatus") {
        if userInfo.keys.contains((String(appLabelValue.prefix(8)) + String(userToStr))) {
            //: needuserStatus = true
            needuserStatus = true
        }
...
extension OccurrenceListManager {
    ...
    func blockData() {
        ...
        self.intimateTopArr.removeAll()
        //: for model in CenterV2Listener.shared.norConvList {
        for model in CenterV2Listener.shared.norConvList {
            //: if model.totalIntimate >= ExamineedReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= ExamineedReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
                let userModel = self.imageUser(model: model)
                //: self.intimateNorArr.append(userModel!)
                self.intimateNorArr.append(userModel!)
            }
        }
        //: for model in CenterV2Listener.shared.topConvList {
        for model in CenterV2Listener.shared.topConvList {
            //: if model.totalIntimate >= ExamineedReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= ExamineedReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
    ...
    func modelDataTop(convModel: ImageDisappearReactiveCompatible, isTop: Bool) {
        //: if isTop {
        if isTop {
    ...
    func manageressView(indexPath: IndexPath) -> ImageDisappearReactiveCompatible? {
        //: switch indexPath.section {
        switch indexPath.section {
    ...
    func subCountimate(uid: String, intimate: Int) {
        ...
        ViewValueInfoManager.submit(with: uid, intimate: intimate, currTime: Int(Date().timeIntervalSince1970))
    }

    /// 更新消息列表单个用户亲密度
    //: func update_userIntimate(userInfo: [String: Any]) -> Bool {
    func clickSection(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo["uid"] as! String
        //: let intimate = userInfo["intimate"] as! Int
        let intimate = userInfo[(constCommitInfoSelectMsg.replacingOccurrences(of: "mode", with: "i") + "mate")] as! Int
        //: var updateSucceed = false
        var updateSucceed = false
        //: if let topModel = self.topArr.filter({ $0.userID == uid }).first {
        if let topModel = self.topArr.filter({ $0.userID == uid }).first {
