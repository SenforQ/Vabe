...
/*: "vipGifts" :*/
fileprivate let dataTextStr:String = "layer to viewvipGifts"

/*: "packGifts" :*/
fileprivate let showLayAppKey:[Character] = ["p","a","c","k"]
fileprivate let mainFollowName:[Character] = ["G","i","f","t","s"]

/*: "giftSendItem" :*/
fileprivate let showGiftMsg:String = "pic video viewgiftSe"
fileprivate let dataVoiceStr:String = "input component of layer datandItem"
...
class SelectedReactiveCompatible: NSObject {
    /// 是否展示背包红点
    ...
    private var cacheDict = [String: [CopernicanSystemHashable: [Any]]]()
    // 数据锁
    ...
    private var giftType = SunnahTransformable.normal

    //: static let share = TalkingChatGiftManager()
    static let share = SelectedReactiveCompatible()
...
extension SelectedReactiveCompatible {
    ...
    func colorCompletion(type: SunnahTransformable = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        SelectedReactiveCompatible.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[CopernicanSystemHashable.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            SelectedReactiveCompatible.share.metadataCommitCompletionGiftDarkroomGet(completion: { isSucceed in
            ...
            SelectedReactiveCompatible.share.dataDwell(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            SelectedReactiveCompatible.share.randomCompletion(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            SelectedReactiveCompatible.share.compute(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func compute(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = SelectedReactiveCompatible.share.giftType
        //: SizeRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        SizeRequestTool.psychologicalFeatureCompletion(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [CopernicanSystemHashable: [Any]]()
                // hot礼物
                ...
                if let array: Array = data["gifts"] as? [Any] {
                    ...
                    dataDict[CopernicanSystemHashable.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(dataTextStr.suffix(8)))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<HeadTransformable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[CopernicanSystemHashable.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(showLayAppKey) + String(mainFollowName))] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<HeadTransformable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[CopernicanSystemHashable.Bags] = bagsArr
                }
                ...
                let sendItem: Array = data[(String(showGiftMsg.suffix(6)) + String(dataVoiceStr.suffix(6)))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<EqualHandyJSON>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func metadataCommitCompletionGiftDarkroomGet(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = SelectedReactiveCompatible.share.giftType
        //: SizeRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        SizeRequestTool.variation(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func director(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: SizeRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        SizeRequestTool.account(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<VoiceMeasurable>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func dataDwell(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = SunnahTransformable.live
        //: SizeRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        SizeRequestTool.zapEqual(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    private func randomCompletion(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.party
        let type = SunnahTransformable.party
        //: SizeRequestTool.req_partyGiftList(type: type.rawValue) { succeed, result, errorModel in
        SizeRequestTool.toTarget(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func perShared(perPageCount: Int, dataType: CopernicanSystemHashable) -> Int {
    ...
    func referenceType(dataType: CopernicanSystemHashable) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func toAll() -> [Any] {
        ...
        if SelectedReactiveCompatible.share.giftType == .groupChat {
            //: return chatRoomSendItems
            return chatRoomSendItems
            //: } else if TalkingChatGiftManager.share.giftType == .live || TalkingChatGiftManager.share.giftType == .party {
        } else if SelectedReactiveCompatible.share.giftType == .live || SelectedReactiveCompatible.share.giftType == .party {
    ...
    func solicitation(dataType: CopernicanSystemHashable, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension SelectedReactiveCompatible {
    ...
    private func deleteDetail() {
        // 刷新礼物背包
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(receiveNewGiftPackage),
                                               selector: #selector(receiveAcrossTable),
    ...
    @objc private func receiveAcrossTable() {
        //: gift_clearMemoryCache(showRed: true)
        retrofitRed(showRed: true)
    }
}
...
extension SelectedReactiveCompatible {
    ...
    func retrofitRed(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        SelectedReactiveCompatible.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
