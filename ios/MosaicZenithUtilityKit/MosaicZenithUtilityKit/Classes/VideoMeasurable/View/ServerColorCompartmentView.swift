...
fileprivate let userMakeValue:[UInt8] = [0x6d,0x72,0x6d,0x78,0x2c,0x67,0x73,0x68,0x69,0x76,0x3e,0x2d,0x24,0x6c,0x65,0x77,0x24,0x72,0x73,0x78,0x24,0x66,0x69,0x69,0x72,0x24,0x6d,0x71,0x74,0x70,0x69,0x71,0x69,0x72,0x78,0x69,0x68]

				fileprivate func backgroundLocation(model num: UInt8) -> UInt8 {
...
/*: "#30D00B" :*/
fileprivate let kLastUrl:String = "bag action collection gift play#30D00B"

/*: "icon_receivedcalls" :*/
fileprivate let noti_imageName:String = "icon_license text"
fileprivate let show_pairData:String = "IVEDCALLS"

/*: "Received calls" :*/
fileprivate let data_removeMessage:[Character] = ["R","e","c","e","i","v","e","d"," ","c","a","l","l","s"]

/*: "icon_dialedcalls" :*/
fileprivate let data_modeText:String = "ictextn"
fileprivate let noti_equalUrl:String = "normaldcalls"

/*: "Dialed calls" :*/
fileprivate let show_storeStr:[Character] = ["D","i","a","l","e","d"," ","c","a","l","l","s"]
...
// __CLOSE_PRINT__
//
//  ServerColorCompartmentView.swift
//  AbroadTalking
//
...
class ServerColorCompartmentView: UITableViewCell {
    //: var currenModel = TalkingVideoCallRecordModel()
    var currenModel = InstanceMeasurable()
    //: var seleteIndex: IndexPath?
    var seleteIndex: IndexPath?
    //: open weak var delegate: WhoLikeDelegate?
    open weak var delegate: DisappearThen?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userMakeValue.map{backgroundLocation(model: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var iconBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(iconographyByClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var durationLB: UILabel = {
        ...
        label.font = .font(type: .Medium, fontSize: 10)
        //: label.textColor = .appValueDetailColor()
        label.textColor = .aftColor()
    ...
    lazy var onlineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(kLastUrl.suffix(7))))
...
extension ServerColorCompartmentView {
    //: func setVideoCallCell(model: TalkingVideoCallRecordModel) {
    func removeModel(model: InstanceMeasurable) {
        //: currenModel = model
        currenModel = model
        ...
        if !model.headPicFrame.isEmptyString {
            ...
            iconBorderBtn.showUp(urlStr: model.headPicFrame)
            //: } else {
        } else {
        ...
        if model.callStatus == 0 {
            ...
            rcordImg.image = UIImage.snapIcon(name: (String(noti_imageName.prefix(5)) + "rece" + show_pairData.lowercased()))
            //: rcordLB.text = "Received calls".localized
            rcordLB.text = (String(data_removeMessage)).localized
            //: } else {
        } else {
            //: rcordImg.image = UIImage.BundleImageNamed(name: "icon_dialedcalls")
            rcordImg.image = UIImage.snapIcon(name: (data_modeText.replacingOccurrences(of: "text", with: "o") + "_dial" + noti_equalUrl.replacingOccurrences(of: "normal", with: "e")))
            //: rcordLB.text = "Dialed calls".localized
            rcordLB.text = (String(show_storeStr)).localized
        }
