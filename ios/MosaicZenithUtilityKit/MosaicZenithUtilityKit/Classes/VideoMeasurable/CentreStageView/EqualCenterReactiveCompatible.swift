...
// __CLOSE_PRINT__
//
//  EqualCenterReactiveCompatible.swift
//  AbroadTalking
//
...
class EqualCenterReactiveCompatible: UICollectionViewFlowLayout {

    //: var sectionArr = [GiftDataType]()  
    var sectionArr = [CopernicanSystemHashable]()  // collection有多少个分区
    //: var hotPage = 0                    
    var hotPage = 0                    // hot页数
    ...
    private var GiftItemHeight  = Int(userBarUrl/2) // 每个礼物的高度
    ...
    override func prepare() {
        ...
        layoutContentSize = CGSize(width: Int(user_barText-10) * totalPage, height: 2*GiftItemHeight)
    }
    ...
    override func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        if indexPath.section == CopernicanSystemHashable.Vip.rawValue {
            //: page += hotPage
            page += hotPage
        //: } else if indexPath.section == GiftDataType.Bags.rawValue {
        } else if indexPath.section == CopernicanSystemHashable.Bags.rawValue {
        ...
        let itemWidth = Int(user_barText-10) / GiftItemColumn
        //: let x = page * Int(ScreenWidth-10) + pageCol * itemWidth
        let x = page * Int(user_barText-10) + pageCol * itemWidth
