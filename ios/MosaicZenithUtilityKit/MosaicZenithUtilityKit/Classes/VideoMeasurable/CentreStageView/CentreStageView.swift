...
fileprivate let kFieldKey:[Character] = ["设","置","普","通","状","态","的","文","字","\u{989c}","\u{8272}","时"," ","请","\u{4f7f}","用","R","G","B","\u{7a7a}"]
fileprivate let user_toolName:[Character] = ["间","的","\u{989c}","色","值"]

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let notiOnName:String = "设\u{7f6e}选中"
fileprivate let user_contentText:String = "时 请使"
fileprivate let userNameValue:String = "间的颜色color"

/*: "init(coder:) has not been implemented" :*/
fileprivate let data_propertyFormat:[UInt8] = [0x1c,0x1b,0x1c,0x1,0x5d,0x16,0x1a,0x11,0x10,0x7,0x4f,0x5c,0x55,0x1d,0x14,0x6,0x55,0x1b,0x1a,0x1,0x55,0x17,0x10,0x10,0x1b,0x55,0x1c,0x18,0x5,0x19,0x10,0x18,0x10,0x1b,0x1,0x10,0x11]

				private func toAdd(layer num: UInt8) -> UInt8 {
					return num ^ 117
				}

/*: "设置的下标不合法!!" :*/
fileprivate let app_pathName:String = "设置的下标\u{4e0d}\u{5408}"
fileprivate let appShareName:[Character] = ["法","!","!"]
...
open class CentreStageView: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: PictureSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.shade(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(kFieldKey) + String(user_toolName)))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.shade(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((notiOnName + "状态\u{7684}文字\u{989c}色" + user_contentText.capitalized + "用RGB空" + userNameValue.replacingOccurrences(of: "color", with: "值")))
        }
    ...
    fileprivate func shade(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: PictureSegmentStyle, titles: [String]) {
    ...
    @objc func cellClick(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? GroupCustomLabel else { return }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_propertyFormat.map{toAdd(layer: $0)}, encoding: .utf8)!)
    }
}
...
public extension CentreStageView {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func contentAnimated(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (app_pathName + String(appShareName)))
    ...
    func locationRoute(_ titles: [String]) {
        // 移除所有的scrollView子视图
        ...
        contentAnimated(0, animated: true)
    }
}
...
//: extension ScrollSegmentView {
extension CentreStageView {
    // 根据Titles添加相应的控件
    //: fileprivate func setupTitles() {
    fileprivate func pushGesture() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = GroupCustomLabel(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.cellClick(_:)))
    ...
    fileprivate func betweenMain() {
        ...
        if let firstLabel = labelsArray[0] as? GroupCustomLabel {
    ...
    fileprivate func achromaticColour() {
    ...
    func counterval(_: Bool) {
        ...
        if currentIndex == titles.firstIndex(of: "Bags".localized) {
            ...
            SelectedReactiveCompatible.share.showBagsRed = false
        }

        //: guard currentIndex != oldIndex else { return }
        guard currentIndex != oldIndex else { return }
        //: let oldLabel = labelsArray[oldIndex] as! CustomLabel
        let oldLabel = labelsArray[oldIndex] as! GroupCustomLabel
        //: let currentLabel = labelsArray[currentIndex] as! CustomLabel
        let currentLabel = labelsArray[currentIndex] as! GroupCustomLabel
        // 让选中标签居中显示
        //: adjustTitleOffSetToCurrentIndex(currentIndex)
        withObserver(currentIndex)
        // 动画效果
    ...
    public func withObserver(_ currentIndex: Int) {
...
open class GroupCustomLabel: UILabel {
    /// 用来记录当前label的缩放比例
