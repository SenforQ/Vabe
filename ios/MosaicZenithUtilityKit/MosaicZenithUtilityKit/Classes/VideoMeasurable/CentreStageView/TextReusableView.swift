...
fileprivate let dataTopTitle:[UInt8] = [0x93,0x98,0x93,0x9e,0x52,0x8d,0x99,0x8e,0x8f,0x9c,0x64,0x53,0x4a,0x92,0x8b,0x9d,0x4a,0x98,0x99,0x9e,0x4a,0x8c,0x8f,0x8f,0x98,0x4a,0x93,0x97,0x9a,0x96,0x8f,0x97,0x8f,0x98,0x9e,0x8f,0x8e]

				fileprivate func quantityroductionIcon(user num: UInt8) -> UInt8 {
...
/*: "icon_coinxxx" :*/
fileprivate let k_inviteContent:String = "icpath"
fileprivate let main_strengthValue:String = "nitemitemitem"

/*: "icon_gift_num_bg" :*/
fileprivate let noti_screenData:[Character] = ["i","c","o","n","_","g","i","f","t","_","n"]
fileprivate let const_toPath:[Character] = ["u","m","_","b","g"]
...
class TextReusableView: UICollectionViewCell {
    //: var model = TalkingRoomGiftModel()
    var model = HeadTransformable()
    //: let checked = false
    let checked = false
    //: let giftNum = 0
    let giftNum = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataTopTitle.map{quantityroductionIcon(user: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nameLB: UILabel = {
        ...
        lb.font = UIFont.font(type: .Regular, fontSize: 10)
        //: lb.textColor = .white
        lb.textColor = .white
    ...
    lazy var priceBtn: GroupVideoControl = {
        //: let btn = TalkingButton.init()
        let btn = GroupVideoControl()
        //: btn.setTitleColor(UIColor.white, for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 10)
        btn.titleLabel?.font = UIFont.font(type: .Regular, fontSize: 10)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_coinxxx"), for: .normal)
        btn.setImage(UIImage.snapIcon(name: (k_inviteContent.replacingOccurrences(of: "path", with: "on") + "_coi" + main_strengthValue.replacingOccurrences(of: "item", with: "x"))), for: .normal)
    ...
    lazy var numBgView: UIImageView = {
        ...
        let imag = UIImage.snapIcon(name: (String(noti_screenData) + String(const_toPath)))
        //: let sizex = imag.size
        let sizex = imag.size
    ...
    lazy var numLb: UILabel = {
        ...
        lb.font = UIFont.font(type: .Regular, fontSize: 9)
        //: return lb
        return lb
        //: }()
    }()
...
extension TextReusableView {
    ...
    func drawChecked(checked: Bool) {
        ...
        if checked {
            ...
            bgView.layer.borderColor = UIColor.titleDevice().cgColor
    ...
    func drownAdd(currenmodel: HeadTransformable, giftType: CopernicanSystemHashable) {
        //: model = currenmodel
        model = currenmodel
        //: giftIcon.setUrlImage(urlStr: model.imgPreview ?? "")
        giftIcon.inclose(urlStr: model.imgPreview ?? "")
        //: priceBtn.setTitle("\(model.price)", for: .normal)
        priceBtn.setTitle("\(model.price)", for: .normal)
        //: selectedImageView.setUrlImage(urlStr: model.cornerMark, placeImg: nil)
        selectedImageView.inclose(urlStr: model.cornerMark, placeImg: nil)
        ...
        let attributes = [NSAttributedString.Key.font: UIFont.font(type: .Regular, fontSize: 11),
...
extension TextReusableView {
    ...
    func plugTotalo() {
        ...
        bgView.backgroundColor = UIColor.phaseOfTheMoon()
