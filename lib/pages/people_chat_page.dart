...
class _PeopleChatPageState extends State<PeopleChatPage> {
  ...
  Future<void> _loadChatHistory() async {
    try {
      ...
      final characterId = widget.character['VabeUserName'] ?? 'unknown';
      final historyKey = 'chat_history_$characterId';
      
      final historyJson = prefs.getString(historyKey);
      if (historyJson != null) {
        final List<dynamic> historyList = jsonDecode(historyJson);
        final List<ChatMessage> loadedMessages = historyList.map((item) {
  ...
  Future<void> _saveChatHistory() async {
    try {
      ...
      final historyList = _messages.map((message) => message.toJson()).toList();
  ...
  void _addWelcomeMessage() {
    ...
    if (_messages.isEmpty) {
      _messages.add(ChatMessage(
        text: 'Hello! I\'m ${_getCleanCharacterName()}. How can I help you today?',
        isUser: false,
  ...
  /// 获取清理后的角色名称（只取空格前的部分）
  String _getCleanCharacterName() {
    final rawName = widget.character['VabeUserName'] ?? 'your AI companion';
                ...
                GestureDetector(
                  onTap: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => PeopleVideoPage(
                          character: widget.character,
                        ),
                      ),
                    );
  ...
  void _showBlockConfirmDialog() {
    showCupertinoDialog(
      ...
      builder: (BuildContext context) => CupertinoAlertDialog(
        ...
        content: Text(
          'Are you sure you want to block ${_getCleanCharacterName()}? You will no longer be able to chat with this user.',
          style: const TextStyle(
            fontSize: 16,
  ...
  void _showMuteConfirmDialog() {
    showCupertinoDialog(
      ...
      builder: (BuildContext context) => CupertinoAlertDialog(
        ...
        content: Text(
          'Are you sure you want to mute ${_getCleanCharacterName()}? You will no longer be able to chat with this user.',
          style: const TextStyle(
            fontSize: 16,
            color: Color(0xFF666666),
          ),
  ...
  Future<void> _confirmBlockUser() async {
    final userId = _getCleanCharacterName();
